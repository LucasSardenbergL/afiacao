...
echo "        PEND=\$(mktemp -t pend)   # único por invocação: /tmp/pend.json colide entre worktrees"
echo "        bun scripts/pendencias-deploy.ts --json > \"\$PEND\""
echo "        bun scripts/pendencias-pacote.ts - < \"\$PEND\""
