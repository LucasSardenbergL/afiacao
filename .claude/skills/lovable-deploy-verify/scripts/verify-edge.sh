...
if [ -z "${SUPABASE_PAT:-}" ]; then
  ...
  echo "    rastro do deploy .... commit do bot na main, e a MENSAGEM É VARIÁVEL: o padrão"
  echo "                          dominante é genérico ('Lovable update', 'Work in progress'),"
  echo "                          e o explícito varia de forma ('Changes', 'Deployed snapshot"
  echo "                          edge func', 'Deployou … verbatim' — este em pt-BR NÃO casa um"
  echo "                          grep por 'deployed'). Logo grep de mensagem só CONFIRMA; como"
  echo "                          negativa ele FABRICA veredito — em 2026-08-26 a omie-vendas-sync"
  echo "                          foi lida como não-deployada estando NO AR (bot commitou 4x como"
  echo "                          'Lovable update'/'Work in progress'). Prova que UM deploy"
  echo "                          rodou, não QUAL versão; ausência dele não prova o contrário"
