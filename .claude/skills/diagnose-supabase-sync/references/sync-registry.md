...
> ⚠️ **Orquestrador `omie-cron-diario` (cron 52): `STEP_TIMEOUT_MS=25s` é abort do CLIENTE, não morte da edge.** Ele chama as 5 edges em série com `AbortController` de 25s; um step que estoura vira `{ok:false,"signal has been aborted",duracao_ms≈25002}` no `resultados` — **FALSO-NEGATIVO**. As edges commitam por página/item e seguem em **background** server-side até o **guard interno** (nfes ~130s, sku-items ~50s, pedidos provado), 1–2min **além** do abort. **Nenhuma das 5 loga em `fin_sync_log`** → zero rastreabilidade independente; prove SÓ por **efeito**: histograma de `updated_at` por segundo na janela `:15–:18` (cauda além de `:16:50`) ou **span > 25s** no writer-único `sku_leadtime_history` = background vivo. ⚠️ `net._http_response` aqui só vê o cron→orquestrador (1 linha), **não** os 5 `fetch` internos do orquestrador→edges → inútil pra atribuir o step; use o efeito. Provado 2026-06-27 (gap de completude ≈0). Mesmo fenômeno da nota de backfill em `docs/agent/sync.md` ("INSERE POR PÁGINA e roda em BACKGROUND além do timeout — valide pela CONTAGEM").
