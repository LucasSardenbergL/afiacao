...
valores reais de `status_titulo` e a data do último sync. Há uma armadilha conhecida — o
front filtra por strings que **não existem** no banco (`'ATRASADO'`, `'A VENCER'`); o que o
sync grava é MAIÚSCULO (`ABERTO`/`VENCIDO`/`PAGO`/`PARCIAL`/`CANCELADO`). Confirme antes de
confiar em qualquer filtro de status. Detalhes em `references/schema-financeiro.md`.
...
A fonte canônica é o engine `fin-cashflow-engine` (rodando na tela `/financeiro/capital-giro`,
tab "Fluxo 13s"). O SQL aqui é um **cross-check** independente: entradas por `data_vencimento`
de CR aberto, saídas por CP aberto + folha recorrente, saldo inicial = Σ `saldo_atual` das
...
- Ressalva: o engine aplica taxa de inadimplência sobre as entradas; o cross-check SQL é
  bruto (não desconta inadimplência). Se o SQL mostra folga mas o engine mostra aperto, a
  diferença é a inadimplência esperada — vale checar.
...
> Calcule o aging por **data** (`CURRENT_DATE − data_vencimento`) sobre títulos com `saldo > 0`
> e `data_recebimento IS NULL`, **não** pelo `status_titulo` (que pode estar dessincronizado).
