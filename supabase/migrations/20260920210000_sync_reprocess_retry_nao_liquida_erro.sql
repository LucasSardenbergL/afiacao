-- ============================================================================================
-- sync_reprocess_saude: um RETRY em voo deixa de liquidar um ERRO terminal
--
-- ACHADO (challenge Codex retroativo, 2026-09-20 — o #2514 subiu com `REVISÃO INDEPENDENTE
-- PENDENTE` porque a cota estava esgotada; esta é a revisão que faltava, e ela encontrou um furo).
--
-- O veredito por chave lia a ULTIMA LINHA, qualquer que fosse. Sequência real de um sync que erra e
-- o cron re-tenta:
--     10h00  complete
--     12h00  error
--     12h29  running   (o retry começou)
--     12h30  watchdog avalia
-- A última linha já não era `error`; o `running` era recente (< 2h, logo não-órfã); e o sucesso das
-- 10h ainda cabia no SLA de 4h. Resultado: **`ok`** — e o watchdog faz dismiss AUTOMÁTICO, que só
-- acontece com `ok` explícito. O alerta do erro das 12h morria antes de o retry sequer completar,
-- e ninguém ficava sabendo. Reproduzido por EXECUÇÃO antes de consertar (o assert correspondente
-- ficou vermelho em `db/test-data-health-sync-reprocess.sh`); não foi aceito no papel.
--
-- O CONSERTO: separar o último RESULTADO da tentativa EM VOO. `u` passa a ler só linhas terminais
-- (`status IS DISTINCT FROM 'running'`) e `r` lê a última `running`. Um início POSTERIOR não
-- liquida um erro terminal; um SUCESSO posterior liquida, porque vira o novo `u`. O teste de órfã
-- passa a usar `r`, e só quando ela é posterior ao último resultado.
--
-- Recria SÓ o `_data_health_compute`: o CONJUNTO de checks não muda, então o `v_sources` do
-- watchdog e a IN-list do heartbeat seguem válidos (a regra do trio acoplado em docs/agent/sync.md
-- é sobre mudar o conjunto de sources, não a lógica interna de um bloco). Corpo partido do
-- `pg_get_functiondef` da PROD lida hoje.
-- ============================================================================================
...
DO $post$
DECLARE v_n int; v_ndist int; v_status text;
BEGIN
  SELECT count(*), count(DISTINCT source) INTO v_n, v_ndist FROM public._data_health_compute();
  IF v_n <> v_ndist OR v_ndist <> 30 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: compute devolveu % linhas / % sources (esperado 30/30) — '
                    'source duplicado cega o laco inteiro do watchdog.', v_n, v_ndist;
  END IF;
  SELECT status INTO v_status FROM public._data_health_compute() WHERE source='sync_reprocess_saude';
  IF v_status IS NULL OR v_status NOT IN ('ok','stale','broken','unknown') THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude com status %, fora do vocabulario '
                    'que o watchdog aceita (22023).', COALESCE(v_status,'<NULL>');
  END IF;
  -- o source tem de seguir nas DUAS pontas; este apply nao as recria, entao confere que nao regrediu
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
     ...
     OR NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                  WHERE n.nspname='public' AND p.proname='fin_sync_heartbeat'
                    AND pg_get_functiondef(p.oid) LIKE '%''sync_reprocess_saude''%') THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude sumiu de uma das duas pontas '
                    '(v_sources do watchdog / IN-list do heartbeat) — o check existiria sem nunca '
                    'ser avaliado.';
  END IF;
  RAISE NOTICE 'OK: 30 sources, sync_reprocess_saude=%', v_status;
END
$post$;
