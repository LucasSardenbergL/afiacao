-- ═══════════════════════════════════════════════════════════════════════════
-- Tintométrico — ALLOWLIST no `preco_csv_legado` (challenge retroativo Codex
-- xhigh 2026-07-20 sobre #1469/#1479, Achado 1 [P1 latente]; decisão do
-- founder 2026-07-21).
--
-- O que fecha: a 20260722100002 fixou "canônica SL → max só de linhas NÃO-SL"
-- (blocklist). Mas o conjunto "não-SL" inclui personalizadas (subcolecao_id
-- NULL) e qualquer subcoleção futura ≠'SL' — e o rótulo do balcão ("Tabela
-- (versão anterior)", #1479) promete proveniência da geração CONGELADA '1',
-- não "qualquer não-SL". Latente hoje (psql-ro 2026-07-21: 0 linhas SL ativas
-- com CSV; 756 personalizadas ativas com CSV, ZERO delas em chave com SL
-- ativa; só '1' e 'SL' existem em tint_subcolecoes) — nada mente no dado
-- atual; é blindagem semântica pura.
--
-- Regra nova (só o ramo SL muda DE NOVO; o não-SL segue a 2b verbatim):
--   • canônica SL     → max SÓ entre linhas ATIVAS da subcoleção '1'
--                       (allowlist). Personalizada e subcoleção futura NUNCA
--                       alimentam o rótulo "versão anterior" sem decisão
--                       explícita — fail-closed, precisão > recall.
--   • canônica não-SL → max de TODAS as ativas (intacto desde a 2b).
--   • sem CSV elegível na chave → NULL (fonte "tabela" some do balcão).
--
-- Baseline pré-apply (psql-ro 2026-07-21): interseção "personalizada ativa
-- com CSV" × "chave com SL ativa" = 0 ⇒ NO-OP no dado atual.
--
...
-- Prova: db/test-tint-canonica.sh (aplica a cadeia de 4 migrations na ordem
-- de prod; C16 = 2ª SL com CSV fora do max [mata o mutante g2.id <> f.id];
-- C17 = personalizada fora [fixa a decisão]; C18 = desativada fora;
-- falsificações dirigidas F8/F9/F10).
-- ═══════════════════════════════════════════════════════════════════════════
...
SELECT
  ...
  rf.tem_receita,
  rf.receita_valida,
  -- CSV da "versão anterior da tinta": canônica SL → SÓ linhas da geração
  -- congelada '1' contam (allowlist — personalizada e subcoleção futura ficam
  -- fora); canônica não-SL → max de todas as ativas (comportamento da
  -- 20260718233000, preservado verbatim).
...
COMMENT ON VIEW public.v_tint_formula_canonica IS
  ...
  'Fase 2b (allowlist fixada 2026-07-21, pós-challenge Codex): preco_csv_legado '
  '= max CSV das linhas ativas da chave; quando a canônica é SL, SÓ linhas da '
  'geração congelada ''1'' contam (allowlist) — personalizada e subcoleção '
  'futura nunca alimentam a fonte "Tabela (versão anterior)". '
