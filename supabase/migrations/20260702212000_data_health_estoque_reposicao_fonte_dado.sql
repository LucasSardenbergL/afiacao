-- ============================================================================================
-- Migration — Sentinela: check `estoque_reposicao` v3 — fonte de frescor VOLTA ao DADO REAL
-- (2026-07-02 · incidente 30/06-02/07 · complemento server-side do PR #1142)
-- ============================================================================================
-- CONTEXTO: o sync omie-sync-estoque (alimenta sku_estoque_atual OBEN, que o motor de compra
-- gerar_pedidos_sugeridos_ciclo lê) ficou 2+ dias parado em silêncio (30/06-02/07, instabilidade
-- da plataforma Lovable). O Sentinela NÃO gritou porque o check estoque_reposicao (v2, marcador)
-- estava 'broken' PERMANENTE desde ~15/06: ele vigia os markers sync_state
-- reposicao_estoque_full/reposicao_pendente_po que NUNCA existiram em prod — o passo-edge do
-- desenho FONTE-ÚNICA (#809 passo 2) não foi implementado (a edge não grava markers; a RPC
-- aplicar_snapshot_pendente, que gravaria o segundo, nunca é chamada). Com o alerta
-- data_health_estoque_reposicao preso ATIVO em fin_alertas desde 15/06, o ON CONFLICT
-- (company,tipo) DO NOTHING do watchdog nunca re-emitiu e-mail => surdez total (17 dias).
--
-- FIX: trocar a fonte do check de volta pro DADO (max(ultima_sincronizacao) OBEN, allowlist
-- fonte_sync='ListarPosEstoque'), thresholds da v1 (4h janela comercial BRT / 16h fora / 30h ou
-- nunca = broken). Backstop de EFEITO NO DADO: teria pego ESTE incidente em 16h. Quando o sync
-- voltar a rodar, o próprio watchdog dismissa o alerta preso (transição ->ok) e o ciclo re-arma.
--
-- O CONJUNTO de checks NÃO muda (mesmos 23 sources) => data_health_watchdog e fin_sync_heartbeat
-- ficam INTACTOS (IN-lists já contêm estoque_reposicao) — só o _data_health_compute é recriado
-- (superfície mínima, regra multi-sessão de docs/agent/sync.md §Sentinela).
--
-- ⚠️ GUARD ANTI-DRIFT (dentro da transação): esta migration foi gerada do pg_get_functiondef VIVO
...
DECLARE
  ...
  v_md5 text;
BEGIN
  v_def := pg_get_functiondef('public._data_health_compute()'::regprocedure);
  v_md5 := md5(v_def);
  IF v_md5 <> '51f336c399c21efde222698142d74c3e' AND v_def NOT LIKE '%estoque frescor v3%' THEN
    RAISE EXCEPTION USING message =
      'PRE-FLIGHT ABORTOU: _data_health_compute vivo (md5 ' || v_md5 || ') não é a base esperada '
      || '(20260626150000 / dump 2026-07-02, md5 51f336c399c21efde222698142d74c3e) nem contém a v3. '
      || 'Outra migration recriou a função depois que esta foi gerada — NÃO aplicar: rebasear sobre '
      || 'pg_get_functiondef atual (rito docs/agent/sync.md §Sentinela) e re-gerar o arquivo.';
  END IF;
END
$guard$;
...
-- Validação pós-apply (read-only — rode no MESMO SQL Editor após o COMMIT):
SELECT 'MIGRATION estoque_reposicao v3 OK' AS resultado,
  (SELECT count(*) FROM public._data_health_compute()) AS total_checks,                  -- esperado: 23
  (SELECT freshness_basis FROM public._data_health_compute()
    WHERE source='estoque_reposicao') AS fonte_do_check,                                 -- esperado: max(sku_estoque_atual...) v3
  (SELECT status FROM public._data_health_compute()
    WHERE source='estoque_reposicao') AS status_agora,                                   -- broken/stale enquanto o sync não voltar; ok depois
  (SELECT to_char(max(ultima_sincronizacao) AT TIME ZONE 'America/Sao_Paulo','DD/MM HH24:MI')
     FROM public.sku_estoque_atual
    WHERE empresa='OBEN' AND fonte_sync='ListarPosEstoque') AS ultimo_sync_real;
