-- ============================================================================================
-- Migration — Sentinela: VIGIA da PROVENIÊNCIA de custo na FONTE (2026-06-23) · follow-up #1019
-- ============================================================================================
-- Adiciona 2 checks ao _data_health_compute (20→22), domain 'estoque', AMBOS promovidos ao PUSH
-- (watchdog 14→16 + heartbeat 15→17). Detecção-na-FONTE de proveniência forjada em product_costs —
-- o complemento do #1019 (que blindou o CONSUMO: resolverCustoCockpit ganhou `|| !sourceReal`, o
-- cockpit de valor degrada a margem quando o source não é real). A fonte cobre TODOS os consumidores
-- de uma vez (resolverCustoConfiavel + espelhos Deno recommend/algorithm-a-audit, cockpit, ranking,
-- relatórios), não só os dois resolvers. Desenho eu + Codex (gpt, reasoning high, 2026-06-23):
--   • custos_proxy_conf_alta (I1 · PUSH): proxy (source ∉ COST_SOURCES_REAIS) com cost_final>0 e
--     cost_confidence>=0.7. O motor de custo inflou a confiança de um source não-real. Hoje teto de
--     proxy = conf 0.5 (headroom 0.2). NASCE VERDE (0 hoje, pre-flight psql-ro 2026-06-23).
--   • custos_product_cost_revivido (I2 · PUSH): qualquer linha cost_source=PRODUCT_COST (normalizado
--     upper/btrim). A escada cost-ladder.ts REMOVEU PRODUCT_COST da operação (lavagem de proveniência,
--     classe #977); a escada nunca o emite. NASCE VERDE (0 hoje).
--
-- ⚠️ DOIS sources, NÃO um combinado (decisão do Codex): o watchdog insere fin_alertas com
--   ON CONFLICT (company,tipo) WHERE dismissed_at IS NULL DO NOTHING e só emaila no IF FOUND. Um source
--   único ('custos_proveniencia') faria a 2ª classe de violação ficar SILENCIOSA enquanto a 1ª estivesse
--   ativa (mesmo company+tipo ⇒ DO NOTHING ⇒ sem e-mail). Sources separados ⇒ cada classe alerta sozinha.
--
-- ⚠️ CONTRADIÇÃO SAUDÁVEL (documentar p/ futuros mantenedores não "simplificarem"): PRODUCT_COST está na
--   whitelist consumer-real de I1 (cost-source.ts:22 — para NÃO nulificar um custo real legítimo se um dia
--   voltar por um writer AUDITÁVEL) E é proibido na escrita por I2. Os dois layers são intencionais:
--   "consumer-real" (quem o lê confia) × "writer-forbidden" (quem escreve não pode emitir).
--
-- ⚠️ PARIDADE SQL→TS: a whitelist de I1 espelha COST_SOURCES_REAIS de src/lib/custos/cost-source.ts:22
--   ({PRODUCT_COST,CMC,CMC_MARGEM_ATIPICA}). Source REAL novo lá ⇒ atualizar a whitelist do I1 aqui.
--
-- ⚠️ PRE-FLIGHT OBRIGATÓRIO no SQL Editor ANTES de aplicar (drift repo×prod, §10/database.md §2):
--   1) pg_get_functiondef('public._data_health_compute()'::regprocedure) — a DEF VIVA deve bater com a
--      base desta migration (corpo da 20260615130000). Divergiu? Rebasear sobre o corpo vivo.
--   2) SELECT count(*) FROM public._data_health_compute(); — esperado 20 (≠20 = drift do conjunto).
--   3) Os 2 backlogs (I1, I2) — ESPERADO 0/0; se >0, os checks nascem 'stale' (não verde): investigue o
--      writer ANTES (o e-mail do 1º push seria legítimo, mas confirme que não é ruído pré-existente).
--
-- ⚠️ ANTI-CASCATA (_data_health_compute já reverteu checks 5×): as 3 funções são CREATE OR REPLACE com
--   corpo VERBATIM da def VIVA (pg_get_functiondef 2026-06-23, == 20260615130000) + APENAS: os 2 UNION ALL
--   novos (antes do alert_channel) e os 2 sources nos IN-lists (watchdog push + heartbeat resumo). NÃO
--   editar mais nada. PRESERVADOS: tint_vinculo_omie (dashboard-only, fora do push), estoque_reposicao,
--   alert_channel, e o tratamento especial de vendas_familia_ausente no watchdog. Envolto em BEGIN/COMMIT.
-- Contagem-alvo pós-apply: compute 20→22; watchdog IN 14→16; heartbeat IN 15→17.
-- PG17: db/test-data-health-custos-proveniencia.sh.
