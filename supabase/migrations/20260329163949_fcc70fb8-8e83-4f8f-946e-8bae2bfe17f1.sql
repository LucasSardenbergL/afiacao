...
BEGIN
  ...
  -- Compare staging formulas vs official formulas
  -- 1) Items in staging
  FOR r IN
  ...
  LOOP
    ...
    ELSE
      DECLARE
        v_diffs text[] := '{}';
      BEGIN
        IF COALESCE(r.stg_vol,0) != COALESCE(r.off_vol,0) THEN v_diffs := v_diffs || 'volume_final_ml'; END IF;
        IF COALESCE(r.stg_preco,0) != COALESCE(r.off_preco,0) THEN v_diffs := v_diffs || 'preco_final'; END IF;
        IF COALESCE(r.nome_cor,'') != COALESCE(r.off_nome,'') THEN v_diffs := v_diffs || 'nome_cor'; END IF;
  ...
  -- Compare staging corantes vs official
  FOR r IN
  ...
  LOOP
    ...
    ELSE
      DECLARE v_diffs2 text[] := '{}';
      BEGIN
        IF COALESCE(r.stg_preco,0) != COALESCE(r.off_preco,0) THEN v_diffs2 := v_diffs2 || 'preco_litro'; END IF;
        IF COALESCE(r.stg_desc,'') != COALESCE(r.off_desc,'') THEN v_diffs2 := v_diffs2 || 'descricao'; END IF;
