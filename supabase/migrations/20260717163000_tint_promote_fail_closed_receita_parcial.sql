...
BEGIN
  ...
  SELECT DISTINCT staging_formula_id FROM (
    ...
    HAVING bool_and(COALESCE(si.qtd_ml, 0) <= 0)
  ) q;
  ...
  WHERE fl.produto_id IS NOT NULL AND fl.base_id IS NOT NULL
    AND fl.volume_final_ml IS NOT NULL AND fl.volume_final_ml > 0
    AND e.volume_ml IS NOT NULL AND e.volume_ml > 0
    -- Guard 4 (Fase 1): fórmula com corante presente sem dose válida NÃO expande → não entra em
    -- _promoted → o DELETE de itens (USING _promoted) não a toca → receita anterior preservada.
    AND NOT EXISTS (SELECT 1 FROM _fl_corrompida fc WHERE fc.staging_formula_id = fl.staging_formula_id);
