-- ============================================================
-- Fix de fuso nas MENSAGENS dos checks de saúde de dados
-- ============================================================
-- Irmão do bug do heartbeat (20260528130000): _data_health_compute() monta as
-- mensagens dos checks com to_char(<timestamptz>, 'DD/MM HH24:MI') SEM
-- "AT TIME ZONE", então os horários saem no fuso da SESSÃO do Postgres (UTC no
-- Supabase). Essas mensagens aparecem (a) na tela/badge Saúde de Dados (via
-- get_data_health) e (b) nos emails de check degradado (data_health_watchdog) —
-- mostrando os horários ~3h à frente. (Não afeta o heartbeat diário, que só
-- exibe "source: status".)
--
-- Fix: AT TIME ZONE 'America/Sao_Paulo' em CADA to_char de coluna timestamptz
-- (cr/cp.updated_at, sync_pedidos completed_at, ip.synced_at, fcs.calculated_at,
-- pc.updated_at, omie_clientes/omie_products.updated_at). As 2 colunas DATE
-- (fin_contas_correntes.saldo_data, pedido_compra_sugerido.data_ciclo) ficam
-- INTACTAS — date não tem fuso e AT TIME ZONE nelas mudaria o tipo/semântica.
-- Mudança é cosmética: só o texto de `message`; status/source/severity (a lógica
-- de alerta) seguem idênticos. Corpo = verbatim da 20260527260000 + os AT TIME ZONE.
