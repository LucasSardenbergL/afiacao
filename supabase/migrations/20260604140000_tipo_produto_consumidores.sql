-- Migration 2a — consumidores money-path leem a COLUNA tipo_produto + RPC fail-closed
-- ============================================================================
-- Liga a guarda "não comprar Produto Acabado (04)" à coluna dedicada (Migration 1).
-- Corpos VERBATIM da 20260531160000 (confirmada = produção via pg_get_functiondef em
-- 2026-06-04). Mudanças cirúrgicas:
--   (RPC) 1. fail-closed: recusa gerar compra se o sinal de classificação está morto
--            (0 produtos com tipo_produto na empresa) — nunca tratar fabricado como comprável.
--         2. guarda lê COALESCE(coluna, metadata) — ponte de transição (metadata é legado vazio).
--   (VIEW) 3. cold-start lê COALESCE(coluna, metadata, '') — NULL/'00' seguem compráveis.
-- NÃO toca o LEFT JOIN account-blind (op) — isolado como follow-up (não dá p/ validar a
-- cardinalidade fora de prod; a guarda '04' já é account-aware via subquery). Spec 2026-06-04.
-- Idempotente. Aplicar manual via SQL Editor do Lovable APÓS o sync ter populado a coluna.
-- ============================================================================

-- ─── PARTE A — RPC de sugestão de pedidos: fail-closed + guarda lê a coluna ───
...
-- ─── PARTE B — cold-start: a view de candidatos lê a COLUNA tipo_produto ───
-- VERBATIM da 20260531160000 PARTE C (ordem de coluna idêntica à prod), exceto o filtro '04'.
...
elegiveis AS (
  ...
  WHERE v.status_sugestao = 'AGUARDANDO_SEGUNDA_ORDEM' AND v.demanda_media_diaria > 0
    ...
    -- [04-fabricado] esconde Produto Acabado ('04', fabricado). Lê a COLUNA (ponte ao metadata). NULL/'00' = comprável.
...
-- Validação (NÃO regenera o ciclo aqui — isso é o Checkpoint B, consciente). Só confirma que a
-- RPC nova compila e que a guarda lê a coluna.
SELECT 'MIGRATION 2a OK' AS status,
  (SELECT count(*) FROM pg_proc WHERE proname='gerar_pedidos_sugeridos_ciclo') AS rpc,
  (SELECT count(*) FROM pg_views WHERE viewname='v_sku_candidatos_primeira_compra') AS view_cold_start;
