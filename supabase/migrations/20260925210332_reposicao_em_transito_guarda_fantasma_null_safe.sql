-- ============================================================
-- Reposição — a guarda [FANTASMA] da CTE em_transito deixa de ser NULL-blind
-- Money-path (compras). Recria gerar_pedidos_sugeridos_ciclo. ⚠️ NÃO auto-aplica (nome custom) —
-- colar no SQL Editor do Lovable. Pré-flight: pg_get_functiondef da PROD em 2026-09-25 = byte-a-byte
-- o corpo da 20260904232555 (base deste arquivo). ACL preservado por CREATE OR REPLACE.
--
-- Defeito (LATENTE): a 20260802120000 pôs na em_transito
--     AND NOT (status = 'aprovado_aguardando_disparo' AND status_envio_portal = 'erro_nao_retentavel'
--              AND portal_protocolo IS NULL AND omie_pedido_compra_numero IS NULL)
-- "=" é NULL-blind: com status_envio_portal NULL o predicado interno vira NULL, NOT(NULL) é NULL, o
-- AND externo vira NULL e a linha SAI da em_transito. Um pedido SAUDÁVEL aprovado-aguardando-disparo
-- deixaria de contar como estoque a caminho → o motor recompraria o que já está vindo. Compra dupla
-- em escala — o inverso exato do #1276 que a guarda veio consertar.
--
-- Por que latente: medido na PROD (psql-ro, 2026-09-22 e 2026-09-25): status_envio_portal tem
-- DEFAULT 'nao_aplicavel' e ZERO NULLs (589 linhas). Mas a coluna é NULLABLE — INSERT com NULL
-- explícito, backfill ou rota de escrita nova reintroduz o problema sem aviso.
--
-- O que muda: "=" → IS NOT DISTINCT FROM (nunca NULL; NULL → false → NOT(false) → segue contando:
-- fail-CLOSED). NADA mais do corpo é tocado. Mesmo conserto que o #2536 fez na função irmã
-- gerar_pedidos_oportunidade_ciclo (20260922225449).
--
-- Prova: db/test-em-transito-erro-terminal.sh (PG17; controle com a versão da PROD vazando o S7 →
-- esta versão segurando; falsificação F5 troca de volta por "=" e exige o vazamento).
-- Fixture viva: db/embalagem-motor-rpc.sql (paridade: embalagem-motor-paridade.test.ts).
-- Rollback: reaplicar a 20260904232555 (a anterior a recriar esta função).
-- ============================================================
...
BEGIN
  ...
  END IF;
  RAISE NOTICE 'OK: guarda [FANTASMA] NULL-safe + multiplo de embalagem preservado';
END
$post$;
