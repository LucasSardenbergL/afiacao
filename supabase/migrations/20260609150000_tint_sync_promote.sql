...
-- Promoção staging→oficial do sync SayerSystem (regra de 3 + preço reproduzido pág 9 +
-- recálculo por insumo + keys-snapshot com blast-radius guard + desativação soft).
-- Spec: docs/superpowers/specs/2026-06-09-tint-sync-sayersystem-design.md §6.2/§11
-- Oráculo (espelhado VERBATIM aqui; PG17 db/test-tint-promote.sh prova a equivalência):
--   src/lib/tint/sync-promote.ts → expandirFormula / precoFinalSayer / validarSnapshotKeys.
--   - fator = vol_destino / vol_formulacao
--   - preco = custo×(1+imp/100)×(1+marg/100) + Σ(qtd_expandida × custo_corante/volume_corante)
--   - round(x::numeric, 2); insumo faltando → NULL (NUNCA 0); custo_base=0 é VÁLIDO (≠ ausente)
--   - thresholds blast radius: snapshot < 50% do oficial ativo OU desativaria > 20% → aborta
-- Idempotente (CREATE OR REPLACE / IF NOT EXISTS). Migration manual via SQL Editor (§5 CLAUDE.md).
...
DROP POLICY IF EXISTS "Staff can view tint_staging_precos_base" ON public.tint_staging_precos_base;
...
CREATE INDEX IF NOT EXISTS idx_tsp_precos_chave
  ON public.tint_staging_precos_base (account, cod_produto, id_base, id_embalagem, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_tint_formulas_ativas
  ON public.tint_formulas (account) WHERE desativada_em IS NULL;
...
DROP POLICY IF EXISTS "Staff can view tint_keys_snapshots" ON public.tint_keys_snapshots;
...
BEGIN
  ...
  FOR r IN SELECT * FROM _formulas_latest LOOP
    ...
    LOOP
      ...
      v_preco := tint_calc_preco_final(
        v_account, r.cod_produto, r.id_base,
  ...
  LOOP
    v_preco := tint_recalc_preco_oficial(v_account, r.formula_id, r.cod_produto, r.id_base, r.id_emb);
...
-- Stub de corante: garante a linha oficial e retorna o id (ensureCorante do CSV → volume 1000).
CREATE OR REPLACE FUNCTION public.tint_ensure_corante_stub(p_account text, p_id_corante text)
...
BEGIN
  ...
  RETURN v_id;
END $$;
REVOKE EXECUTE ON FUNCTION public.tint_ensure_corante_stub(text, text) FROM anon, authenticated, PUBLIC;
...
CREATE OR REPLACE FUNCTION public.tint_calc_preco_final(
  p_account text, p_cod_produto text, p_id_base text, p_id_embalagem text,
...
DECLARE
  ...
  v_cor        record;
BEGIN
  -- Insumo da base p/ ESTA embalagem de destino (latest staging precos_base).
  SELECT custo, imposto_pct, margem_pct INTO v_base
  FROM tint_staging_precos_base
  WHERE account = p_account AND cod_produto = p_cod_produto AND id_base = p_id_base AND id_embalagem = p_id_embalagem
  ...
  LOOP
    ...
    WHERE account = p_account AND id_corante_sayersystem = it.id_corante
    ORDER BY created_at DESC, id DESC
    LIMIT 1;
...
END $$;
REVOKE EXECUTE ON FUNCTION public.tint_calc_preco_final(text, text, text, text, uuid, numeric) FROM anon, authenticated, PUBLIC;
...
CREATE OR REPLACE FUNCTION public.tint_recalc_preco_oficial(
  p_account text, p_formula_id uuid, p_cod_produto text, p_id_base text, p_id_embalagem text)
RETURNS numeric LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_base       record;
  v_preco_base numeric;
...
BEGIN
  ...
  RETURN round((v_preco_base + v_soma)::numeric, 2);
END $$;
REVOKE EXECUTE ON FUNCTION public.tint_recalc_preco_oficial(text, uuid, text, text, text) FROM anon, authenticated, PUBLIC;
...
BEGIN
  ...
  -- Chave oficial = cor_id|cod_produto|id_base|id_embalagem|personalizada (ids SAYER, via joins).
  CREATE TEMP TABLE _oficial_ativas ON COMMIT DROP AS
  SELECT f.id,
         (f.cor_id || '|' || p.cod_produto || '|' || b.id_base_sayersystem || '|'
          || e.id_embalagem_sayersystem || '|' || (CASE WHEN f.personalizada THEN 'true' ELSE 'false' END)) AS chave
  ...
  SELECT count(*) INTO v_total_ativas FROM _oficial_ativas;
  SELECT count(*) INTO v_chaves_snap  FROM _snap_keys;
  SELECT count(*) INTO v_desativariam FROM _oficial_ativas o
  ...
  -- 4) Desativa (soft) as ativas fora do snapshot. (Reativação é responsabilidade da promoção.)
  ...
  -- 5) Avança o marcador de ordem + fecha o run.
