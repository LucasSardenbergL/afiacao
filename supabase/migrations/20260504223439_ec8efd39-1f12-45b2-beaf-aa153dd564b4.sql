...
BEGIN
  ...
  WITH skus_necessitando AS (
    SELECT
      ...
      -- NOVO: soma quantidades já em pedidos efetivados/em rota recente (últimos 7 dias)
      -- para evitar duplicar sugestão antes do Omie sincronizar o estoque pendente.
      COALESCE((
    ...
    WHERE sp.empresa = p_empresa
      ...
      -- Estoque efetivo (físico + pendente Omie + em trânsito recente do app) abaixo do ponto de pedido
      AND (
        COALESCE(sea.estoque_fisico, 0)
        + COALESCE(sea.estoque_pendente_entrada, 0)
        + COALESCE((
