...
-- FASE 1d (money-path) — base pura EXPLÍCITA da fonte + endurecimento do vazio sem declaração.
-- Contraparte BANCO da fronteira do CONECTOR (connector/sayersync, mesmo PR): o conector deixou
-- de OMITIR slot inválido (passa a preservá-lo no payload p/ o Guard 4 barrar e logar) e passou a
-- DECLARAR is_base_pura=true quando a FONTE confirma fórmula sem corante (flat: 6 slots livres +
-- 12 flat cols resolvidos; child: formula_pk resolvida + 0 linhas na filha).
--
-- O que muda na promoção (corpo VERBATIM da 20260718170000/v4 + 4 mudanças marcadas):
--   [1d-A] _formulas_latest projeta is_base_pura (acompanha o header latest; NULL = não-declarado);
--   [1d-B] sinal contraditório barra: is_base_pura=true exige expected=0 E COUNT bruto=0 — declarada
--          com itens/expected NULL não promove nem limpa (fail-closed);
--   [1d-C] guard (c) vira (c'): a TRÍADE DECLARADA (is_base_pura=true AND expected=0 AND COUNT=0)
--          libera a transição legítima pigmentada→pura (a promoção LIMPA a receita: DELETE sem
--          re-INSERT) e a base pura NOVA; vazio SEM declaração agora barra TAMBÉM em chave nova
...
--
-- ⚠️ ORDEM DE DEPLOY: esta migration ANTES do deploy da edge tint-sync-agent (a edge nova INSERTa
-- is_base_pura; sem a coluna, PGRST204 e o sync de fórmulas PARA — mesma armadilha da 1c). O
-- binário do conector é ON-PREM (logística do founder) e é compatível em qualquer ordem: payload
