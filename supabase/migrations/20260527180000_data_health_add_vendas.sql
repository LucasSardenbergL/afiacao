-- Sentinela de Saúde de Dados — adiciona check de VENDAS (ponto cego exposto no incidente
-- de 2026-05-27: vendas ficou morto 8 dias e o Sentinela não viu, pq não tinha check de vendas;
-- foi o check de carteira, downstream, que denunciou por tabela).
--
-- Sinal: frescor do sync INBOUND de pedidos (omie-vendas-sync sync_pedidos) via fin_sync_log
-- (NÃO sales_orders.created_at, que é a data do pedido no Omie, não a hora de sync — decisão
-- revisada com codex). Worst-of entre oben e colacor (uma conta parada não é mascarada pela
-- outra). Crons vendas-sync-pedidos-{oben,colacor}-2h rodam de 2 em 2h → stale > 6h (3 ciclos),
-- broken se uma conta nunca completou.
...
BEGIN
  ...
  WITH checks AS (
    ...
    UNION ALL
    -- ── OMIE SYNC: status do ÚLTIMO sync financeiro ──
    ...
    SELECT 'vendas_pedidos'::text, 'vendas'::text,
      CASE WHEN v.oben_last IS NULL OR v.colacor_last IS NULL THEN 'broken'
           WHEN now() - LEAST(v.oben_last, v.colacor_last) > interval '6 hours' THEN 'stale'
           ELSE 'ok' END,
      ...
      'Cheque os crons vendas-sync-pedidos-{oben,colacor}-2h e fin_sync_log (action sync_pedidos)'::text,
      'critical'::text
    FROM (
      SELECT
        (SELECT max(l.completed_at) FROM public.fin_sync_log l
          WHERE l.action='sync_pedidos' AND l.status='complete' AND 'oben' = ANY(l.companies)) AS oben_last,
        (SELECT max(l.completed_at) FROM public.fin_sync_log l
          WHERE l.action='sync_pedidos' AND l.status='complete' AND 'colacor' = ANY(l.companies)) AS colacor_last,
        (SELECT l.error_message FROM public.fin_sync_log l
          WHERE l.action='sync_pedidos' AND l.status='error' ORDER BY l.started_at DESC LIMIT 1) AS last_err
    ) v
