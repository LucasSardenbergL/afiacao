...
BEGIN
  ...
  WHERE source IN ('vendas_pedidos','estoque_inventario','reposicao_sugestoes','carteira_scores',
                   ...
                   'reposicao_sayerlack_fabricado','omie_tipo_produto_oben','alert_channel');
...
-- Validação
SELECT 'MIGRATION 2b OK' AS status,
  (SELECT count(*) FROM public._data_health_compute() WHERE source='omie_tipo_produto_oben') AS check_cobertura_novo,
  (SELECT count(*) FROM public._data_health_compute() WHERE source='reposicao_sayerlack_fabricado') AS check_sayerlack;
