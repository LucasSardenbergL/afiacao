...
-- P0-B-bis Fatia 5 (final) — o check `vendas_cadastros` do Sentinela larga o espelho `omie_clientes`
-- e passa a medir o frescor pela proof account-correta `omie_customer_account_map`.
--
-- POR QUE AGORA (nao e so limpeza pro DROP — e conserto de um alarme PRESTES a disparar falso):
-- a Fatia 4 (#1428, deployada) matou o ULTIMO writer do espelho. Medido por psql-ro em 2026-07-18:
-- `max(omie_clientes.updated_at)` esta TRAVADO em 05:02:40 e nao anda mais. O check compara
-- `now() - LEAST(max_clientes, max_produtos)` contra 30h => em ~15h a partir da medicao ele vira
-- 'stale' e, dali em diante, NUNCA mais volta a 'ok'. Ou seja: hoje o check mede o frescor de uma
-- tabela MORTA. Sem esta migration o Sentinela ganha um vermelho permanente que nao corresponde a
-- falha nenhuma de sync — e alarme que mente e alarme que o operador aprende a ignorar.
--
-- POR QUE A PROOF (e nao o ledger): o check pergunta "o sync de CADASTRO rodou?". A proof
-- `omie_customer_account_map` e escrita pelo MESMO bulk que antes escrevia o espelho
-- (omie-analytics-sync :468/:482) => e a sucessora legitima do sinal. Medido em 18/07:
--   omie_clientes ....................... 05:02:40  (idade 14h56 — MORTO)
--   omie_customer_account_map ........... 19:57:43  (idade 1min — VIVO)
-- Ja o `carteira_membership_ledger` NAO serve para frescor: e um ACUMULADOR (upsert com
-- ignoreDuplicates) — `updated_at` so anda quando entra membro NOVO ou muda `identity_state`.
-- Uma semana sem cliente novo o congelaria e o check ficaria falso-vermelho. Frescor-de-sync e
-- membership-historica sao perguntas diferentes => fontes diferentes (proof x ledger).
--
-- /!\ POR QUE `WHERE account = 'oben'` E NAO `max()` GLOBAL da proof (correcao do /codex xhigh):
--     um `max()` global e mascarado pela conta MAIS FRESCA — oben atualiza a cada poucas horas e
--     esconderia colacor parado ha dias, deixando o Sentinela VERDE com uma conta morta. Medido em
--     18/07: oben 1,9h · colacor_sc 2,0h · colacor 16,5h — o global reportaria 1,9h.
--     `'oben'` e a EQUIVALENCIA ESTRITA do sinal que existia: o espelho era escrito pelo bulk do run
--     oben (`account === "vendas"`), entao `max(omie_clientes.updated_at)` ja era, na pratica, o
--     frescor de oben. Trocar a FONTE sem mudar o que o check AFIRMA e o escopo desta fatia.
--     ⚠️ O Codex propos tambem a alternativa mais ampla — `LEAST` entre as 3 contas, com `broken` se
--     faltar alguma. Ela e melhor como Sentinela e fica registrada como FOLLOW-UP explicito, NAO
--     feita aqui: colacor roda 1x/dia (05:20), o que contra o limite de 30h deixa so ~6h de folga —
--     cobrir as 3 contas exige threshold PROPRIO por conta, senao vira alarme falso recorrente. Isso
--     e design de check, nao migracao de fonte. O gap fica MEDIDO e nomeado, em vez de silencioso.
--
-- /!\ CREATE OR REPLACE de funcao QUENTE (5+ migrations recentes; qualquer sessao de sync/health a
--     recria). Corpo extraido da def EXATA da PROD via `pg_get_functiondef` em 2026-07-18 e conferido
--     byte-a-byte contra o repo (20260717160000): ZERO drift, 555 linhas identicas. As UNICAS
--     alteracoes sao 3 substituicoes dentro do bloco `vendas_cadastros` — a query real + as 2 strings
--     descritivas (`freshness_basis` e `probable_cause`), que passariam a MENTIR se ficassem citando
--     uma tabela que nao existe mais. Nenhuma outra branch tocada; diff estrutural vs PROD = so isso.
--     A ultima a recriar VENCE: se outra sessao recriar _data_health_compute depois desta extracao,
--     re-extraia a def e re-aplique as 3 trocas antes de colar no SQL Editor.
--
-- /!\ RAIO DE EXPLOSAO — por que esta migration vem ANTES do DROP, nao junto:
--     a funcao e `LANGUAGE sql` com um UNICO `UNION ALL` de 24 checks. Se `omie_clientes` sumisse
--     com a funcao ainda a referenciando, a funcao INTEIRA falharia — blackout dos 24 checks do
--     Sentinela, nao a degradacao de um. Consumida por 2 crons (`data-health-watchdog` */30 e
--     `fin-sync-heartbeat`), ambos com `vendas_cadastros` no IN-list.
--
-- Prova PG17 (com falsificacao): db/test-data-health-vendas-cadastros-proof.sh
-- Validacao pos-apply (SQL Editor) — logo abaixo do CREATE.
...
-- ============================================================================
-- Validacao pos-apply (cole no SQL Editor apos o Run; NAO use get_data_health(),
-- que tem gate `auth.uid() IS NULL -> RAISE 42501` e o SQL Editor roda sem JWT).
-- Espere: checks=24 · cita_espelho=f · cita_proof=t · status do check IN (ok,stale,broken)
-- ============================================================================
SELECT
  (SELECT count(*) FROM public._data_health_compute())                                    AS checks,
  pg_get_functiondef('public._data_health_compute()'::regprocedure) ~ '\momie_clientes\M' AS cita_espelho,
  pg_get_functiondef('public._data_health_compute()'::regprocedure) ~ 'omie_customer_account_map' AS cita_proof;

SELECT source, status, round(age_seconds/3600.0, 1) AS idade_h, freshness_basis, message
FROM public._data_health_compute()
WHERE source = 'vendas_cadastros';
