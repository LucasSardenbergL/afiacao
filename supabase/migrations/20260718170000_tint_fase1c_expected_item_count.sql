...
-- FASE 1c (money-path) — protocolo de staging como UNIDADE (integridade de TRANSPORTE edge→banco).
-- Fecha o RESÍDUO declarado no cabeçalho da 20260718140000 (v3): (R1) subconjunto VÁLIDO de itens
-- que sobra de falha de chunk + cleanup falho (tint-sync-agent, .delete() sem checagem) é
-- indistinguível de receita legítima → substituía receita íntegra; (R2) promoção concorrente lê
-- header antes dos itens e em chave oficial NOVA o guard (c) não barra (exige oficial COM receita)
-- → criava fórmula vazia/parcial ativa (o mal das 28.609 em miniatura).
-- MECANISMO: a EDGE declara no header quantas linhas de item RECEBEU e vai inserir
-- (expected_item_count, handleFormulas) e a promoção só aceita a fórmula quando o COUNT(*) BRUTO
-- ingerido BATE. NULL = protocolo legado/simulação → comportamento v3 EXATO (zero regressão nos
-- ~217k headers atuais; filtrar por tint_sync_runs.status segue PROIBIDO — 59% dos headers
-- legítimos vivem em runs 'error', lição da v2 DESCARTADA da 20260718140000).
-- COUNT BRUTO de propósito: o gate prova TRANSPORTE (o array que a edge recebeu foi persistido
-- inteiro); a VALIDADE do conteúdo segue nos guards 4a/4b.
--
-- CHALLENGE CODEX (gpt-5.6-sol xhigh, 2026-07-18 — 3 P1 + 4 P2 + 1 P3; fold completo):
--   · D4 original ("expected=0 declarado LIMPA a receita" / R3-base-pura) foi REJEITADO e REMOVIDO:
--     o CONECTOR (connector/sayersync/pg.go, shapes flat e child) OMITE slot com corante vazio ou
--     qtd não-parseável/<=0 ANTES do POST — expected medido na edge conta o array JÁ FILTRADO, então
--     0/ausente NÃO distingue "base pura confirmada" de "tudo filtrado por corrupção". Limpeza de
--     receita exige sinal SEMÂNTICO da fonte (is_base_pura) + conector que preserve/rejeite
--     inválidos → FASE 1d do programa. O guard (c) da v3 fica VERBATIM (vazio, declarado ou não,
--     segue barrado sobre oficial com receita — fail-closed).
--   · P1 edge: ausência de `itens` (regressão de serialização) NUNCA vira expected=0 — a edge grava
--     NULL (Array.isArray, sem `|| []`) → caminho legado, barra.
--   · P1 placeholders: N linhas placeholder com COUNT batendo NÃO limpam (guard (c) intacto barra).
--   · P2: id do header PRÉ-GERADO na edge (associação header→itens não depende da ordem do retorno
--     de .select()); erro carrega header_sync_run_id (o run DONO do header incompleto, não só o run
--     promotor — o re-log a cada promote é sinal honesto, padrão do Guard 4, mas a autoria fica
--     correta); CHECK expected_item_count >= 0 NOT VALID (writer service-role não congela chave com
--     negativo; NOT VALID = sem varredura dos 217k existentes, todos NULL).
--   · Furo PRÉ-EXISTENTE documentado (NÃO é desta fase): fonte manda {AX=10, VM=0} → o conector
--     FILTRA o VM → payload chega "limpo e íntegro" (expected=1, count=1) → guards nunca veem o
--     inválido → promove parcial. Existe hoje na v3 pela mesma via; fecha na FASE 1d (conector
--     preserva/rejeita + teste de contrato Go→edge).
-- Sem fallback ao header antigo completo quando o latest é incompleto (promoveria estado velho
-- como se fosse novo) e sem fallback ao perdedor da colisão (lição do Guard 4/v3). Mismatch
-- transitório de corrida com ingestão em curso se auto-resolve no promote seguinte (fail-closed).
...
-- Corpo: VERBATIM da 20260718140000 (pré-flight pg_get_functiondef da PROD 2026-07-18 = SEM
-- deriva, 663 linhas idênticas) + 2 mudanças marcadas [1c-A] [1c-B].
-- Provado: db/test-tint-promote.sh C24-C29 + falsificação F1c. Plano:
...
ALTER TABLE public.tint_staging_formulas DROP CONSTRAINT IF EXISTS tint_staging_formulas_expected_item_count_nao_negativo;
ALTER TABLE public.tint_staging_formulas ADD CONSTRAINT tint_staging_formulas_expected_item_count_nao_negativo
  CHECK (expected_item_count IS NULL OR expected_item_count >= 0) NOT VALID;
COMMENT ON COLUMN public.tint_staging_formulas.expected_item_count IS
  'Protocolo Fase 1c: nº de linhas de item que a EDGE recebeu e inseriu p/ este header (handleFormulas). NULL = protocolo legado/simulação/campo ausente (promoção usa o caminho v3, fail-closed no vazio ambíguo). A promoção só aceita a fórmula quando COUNT(*) bruto de tint_staging_formula_itens bate (integridade de TRANSPORTE — não autoriza limpeza de receita; base pura explícita é a Fase 1d).';

-- (2) CREATE OR REPLACE tint_promote_sync_run — corpo VERBATIM da 20260718140000 com 2 mudanças:
--   [1c-A] _formulas_latest projeta expected_item_count + header_sync_run_id (acompanham o header LATEST);
--   [1c-B] gate _eu_incompleta DEPOIS do DELETE do Guard 4 (pós-vencedor) e ANTES do guard (c),
--          que permanece VERBATIM (sem refino de limpeza — decisão pós-Codex).
...
BEGIN
  ...
  --   p/ base pura precisa de sinal explícito da FONTE (is_base_pura — Fase 1d). Pós-Codex 2026-07-18:
  --   expected_item_count NÃO autoriza limpeza (o conector OMITE itens inválidos antes do POST —
  --   expected 0/ausente não distingue "base pura confirmada" de "tudo filtrado por corrupção") →
  --   este guard fica VERBATIM da v3: vazio barra, declarado ou não.
