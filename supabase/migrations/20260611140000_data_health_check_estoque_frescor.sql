-- ============================================================================================
-- Migration — Sentinela: check `estoque_reposicao` (frescor do sku_estoque_atual que o motor lê)
-- ============================================================================================
...
AS $$
  WITH checks AS (
    ...
    SELECT 'estoque_reposicao'::text, 'estoque'::text,
      CASE WHEN max(se.ultima_sincronizacao) IS NULL THEN 'broken'
           WHEN now() - max(se.ultima_sincronizacao) > interval '30 hours' THEN 'broken'
           WHEN now() - max(se.ultima_sincronizacao) > interval '16 hours' THEN 'stale'
           WHEN (now() AT TIME ZONE 'America/Sao_Paulo')::time >= time '08:00'
            AND (now() AT TIME ZONE 'America/Sao_Paulo')::time <  time '18:00'
            AND now() - max(se.ultima_sincronizacao) > interval '4 hours' THEN 'stale'
           ELSE 'ok' END,
      EXTRACT(EPOCH FROM now() - max(se.ultima_sincronizacao))::bigint, (4*3600)::bigint,
      'sku_estoque_atual.ultima_sincronizacao (OBEN)'::text,
      CASE WHEN max(se.ultima_sincronizacao) IS NULL THEN 'Estoque de reposição nunca sincronizou'
           ELSE 'Estoque de reposição (motor de compra): sincronizado ' || to_char(max(se.ultima_sincronizacao) AT TIME ZONE 'America/Sao_Paulo','DD/MM HH24:MI') END,
      NULL,
      CASE WHEN max(se.ultima_sincronizacao) IS NULL OR now() - max(se.ultima_sincronizacao) > interval '4 hours'
           THEN 'A edge omie-sync-estoque parou de atualizar sku_estoque_atual. ARMADILHA: o cron marca "succeeded" mesmo quando a função dá 503/erro — a verdade está em net._http_response, NÃO em job_run_details. Estoque congelado faz o motor sugerir comprar o que já tem.'
           ELSE NULL END,
      ...
      'critical'::text
    FROM public.sku_estoque_atual se WHERE se.empresa = 'OBEN'
    UNION ALL
