-- ============================================================================================
-- Migration — Sentinela: VIGIA do sync de PEDIDOS DE COMPRA (2026-06-26) · eu + Codex (gpt high)
-- ============================================================================================
-- Adiciona 1 check ao _data_health_compute (22→23), domain 'estoque', PROMOVIDO ao PUSH
-- (watchdog 16→17 + heartbeat 17→18). Observabilidade do sync omie-sync-pedidos-compra, que alimenta
-- purchase_orders_tracking (leadtime + telas de acompanhamento = MONEY-PATH) e era um ponto cego total.
--
-- POR QUÊ NÃO frescor pela tabela (rejeitado na investigação de prod 2026-06-26):
--   • purchase_orders_tracking é MULTI-WRITER — 5 edges escrevem; nfes/ctes/sku-items setam updated_at.
--     max(updated_at) NÃO isola o sync de pedidos (pedidos quebrado + nfes rodando = tabela "fresca").
--   • Sinal ESPARSO: gaps de até 5 DIAS entre updates em operação normal (PesquisarPedCompra filtra por
--     previsão de entrega; o sync só toca linha quando há pedido na janela de 3d). Threshold pela tabela
--     daria falso-positivo ou seria largo demais (>5d) pra detectar a falha em tempo útil.
--   => A edge passou a gravar um HEARTBEAT 1-writer em sync_state (entity_type='pedidos_compra',
--      account='oben'). Este check lê ESSE marcador. (mudança de edge: supabase/functions/
--      omie-sync-pedidos-compra/index.ts — heartbeatRunning/heartbeatFim + retorno 502 em falha total.)
--
-- DESENHO do marcador (revisado pós-Codex challenge, 35 achados):
--   last_sync_at = horário do último SUCESSO (NÃO avança em falha total → preserva "última coleta boa").
--   updated_at   = heartbeat de execução (todo upsert o avança → detecta 'running' órfão = morte no meio).
--   status       = running → complete | partial | error. metadata = {trigger, dias} p/ auditoria.
--   'partial' (Codex #1/#26/#35, o achado-chave): coleta truncada (alguns pedidos + erro) NÃO pode virar
--     'complete' — seria o novo fail-open. → stale.
--   status desconhecido => 'broken' (fail-safe, Codex #21 — só 'complete'/'running'-fresco são saudáveis).
--
-- ⚠️ severity FIXO 'critical' (= vendas_pedidos/estoque_reposicao, money-path): o watchdog insere
--   fin_alertas com ON CONFLICT (company,tipo) WHERE dismissed_at IS NULL DO NOTHING; severity DINÂMICO
--   (warning→critical) no MESMO source NÃO re-emailaria a escalada (Codex #9/#10). Fixo critical evita isso.
--
-- ⚠️ OBEN-only: o cron omie-cron-diario só sincroniza OBEN (COLACOR=0 linhas, só POST manual). Se um dia
--   COLACOR entrar na esteira, ADICIONAR account='colacor' aqui (hoje apodreceria silencioso — Codex #12).
--
-- ⚠️ NÃO cobre (fora de escopo — observabilidade de FALHA DE SYNC, não integridade semântica): status
--   mapeado errado, pedido novo fora da janela de previsão, corrupção de payload (Codex #24/#25/#27).
--
-- ⚠️ PRE-FLIGHT OBRIGATÓRIO no SQL Editor ANTES de aplicar (drift repo×prod, §10/database.md §2):
--   recriar _data_health_compute + data_health_watchdog + fin_sync_heartbeat JUNTOS, partindo do corpo
--   de PRODUÇÃO (este arquivo foi gerado de pg_get_functiondef da prod em 2026-06-26 via psql-ro). A
--   ÚLTIMA a recriar vence — se outra worktree recriou o trio depois, REBASEAR este check sobre o corpo dela.
--
-- ⚠️ ORDEM DE DEPLOY (evita o check nascer 'broken' por marcador ausente — Codex #11/#4):
--   1) Deployar a edge omie-sync-pedidos-compra (chat do Lovable, verbatim do repo).
--   2) Disparar a edge 1x ({empresa:"OBEN"}) e confirmar sync_state pedidos_compra/oben = 'complete'.
--   3) SÓ ENTÃO aplicar esta migration (SQL Editor). Assim o check estreia 'ok', sem e-mail espúrio.
-- ============================================================================================
