...
-- Validação pós-apply (read-only): o check novo existe + o total de checks subiu pra 17 (16 + 1).
SELECT 'MIGRATION familia_ausente OK' AS status,
  (SELECT count(*) FROM public._data_health_compute() WHERE source='vendas_familia_ausente') AS check_novo,
  (SELECT status FROM public._data_health_compute() WHERE source='vendas_familia_ausente') AS status_atual,
