...
--
-- Fecha o ponto cego do Sentinela sobre o carteira-rebuild.
--
-- CONTEXTO (incidente 2026-07-28): o cron carteira-rebuild-nightly (07:30 UTC) enfileirou
-- o net.http_post e cron.job_run_details marcou 'succeeded' — mas o run morreu ANTES de
-- tomar o lease (sync_state.carteira_rebuild ficou 'complete' de 27/07, sem transicao para
-- 'syncing'; o claim_carteira_rebuild e a primeira chamada apos o gate de auth). Efeito:
-- 7.301 linhas de carteira_assignments congeladas por 24h. Auto-recuperou em 29/07.
-- Ninguem foi avisado: o unico check da familia carteira le farmer_client_scores
-- .calculated_at, que naquela manha estava fresco (calculate-scores rodou normal).
--
-- Este arquivo NAO altera nenhum check existente: acrescenta UM ramo UNION ALL. O corpo
-- foi extraido de pg_get_functiondef da PROD (nao do repo) — pre-flight do CLAUDE.md,
-- porque apply manual diverge do repo e a ultima a recriar VENCE. Diff conferido: os 20
-- check_keys da prod batem com os da 20260718220000 (ultima migration que recria a funcao).
--
-- Threshold 30h: o cron e diario; 30h da ~6h de folga apos a janela esperada, no mesmo
-- espirito do vizinho custos_produtos (30h) e carteira_scores (36h). severity 'warning'
-- para casar com carteira_scores — carteira congelada degrada, nao para o app.
--
-- Provado em PG17 local por db/test-data-health-carteira-rebuild.sh (asserts positivos,
-- negativos e falsificacao: sabotar o threshold/o ramo exige VERMELHO).
