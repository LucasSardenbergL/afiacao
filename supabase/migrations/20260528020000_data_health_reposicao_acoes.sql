-- Track A (checks de AÇÃO): 2 novos checks de "fila de trabalho parada" na pedido_compra_sugerido —
-- reposicao_disparo (pedido aprovado não despachado ao fornecedor) e reposicao_portal (pendente no
-- portal Sayerlack). Diferente dos checks de frescor: medem fila envelhecendo, não frescor de tabela.
-- DASHBOARD + HEARTBEAT, FORA do push do watchdog (v1): o backlog atual é de natureza incerta (pode
-- ter pedido abandonado), e empurrar email sobre isso = ruído. Aparecem no badge/página + digest diário;
-- promover pra push (adicionar ao IN do data_health_watchdog) é 1 linha quando o backlog for triado.
-- Thresholds calibrados em prod: disparo aprovado_em >2d=stale / >7d=broken; portal pendente >6h=stale /
-- >1d=broken (excluindo os com retry agendado). domínio 'estoque' (reposição) → sem mudança de frontend.
-- Corpo dos 11 checks = verbatim da 20260527260000; acrescento 2 UNION ALL antes do alert_channel.
  ...
  WITH checks AS (
    ...
    FROM (
      SELECT
        ...
        to_char(min(aprovado_em) FILTER (WHERE status='aprovado_aguardando_disparo'),'DD/MM') AS mais_antigo_txt
    ...
    FROM (
      SELECT
        ...
        to_char(min(atualizado_em) FILTER (WHERE status_envio_portal='pendente_envio_portal' AND (portal_proximo_retry_em IS NULL OR portal_proximo_retry_em < now())),'DD/MM') AS mais_antigo_txt
      FROM public.pedido_compra_sugerido
    ) rp
    UNION ALL
...
-- Heartbeat: inclui os 2 novos no resumo de saúde de dados (dashboard + digest diário; FORA do push).
