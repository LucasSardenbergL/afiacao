-- Triagem dos 45 crons (eu + codex, 4o consult): dos ~28 não-cobertos pelo Sentinela, só 4 são HIGH
-- (dano de negócio real + ninguém notaria rápido). NÃO cobrir por simetria. Desta leva entram os 2 HIGH
-- de FRESCOR DE DADO (limpos, sem precisar calibrar threshold com prod — daily → stale>30h, padrão CR/CP):
--   • custos_produtos    — compute-costs-daily escreve product_costs.updated_at (upsert seta updated_at
--                          explicitamente → avança a cada run). Custo velho = margem/valor/decisão de
--                          dinheiro errada. domínio 'estoque' (economia de produto), severity warning.
--   • vendas_cadastros   — sync-products-customers-daily (omie-analytics-sync sync_all account=vendas)
--                          loga em sync_state (entity_type customers/products, last_sync_at+status+erro).
--                          Morte calada = clientes/produtos novos somem do CRM/carteira/catálogo. domínio
--                          'vendas', severity critical (master data).
-- Os 2 HIGH de AÇÃO (disparar-pedidos-aprovados, sayerlack-portal) ficam DEFERIDOS: são "monitorar fila
-- de trabalho envelhecendo", exigem semântica de fila + calibração de threshold com dado de prod (qual
-- idade de "aprovado parado" é anormal?) — construir sem calibrar = falso-alarme. Backlog c/ spec próprio.
-- Ambos os checks reusam domínios existentes ('vendas'/'estoque') → sem mudança de frontend.
-- Corpo = verbatim da migration 20260527240000 (9 checks, com alert_channel); acrescento 2 UNION ALL.
...
AS $$
  WITH checks AS (
    ...
    UNION ALL
    -- HIGH (frescor): custos de produto (compute-costs-daily → product_costs.updated_at). Custo velho =
    -- margem/valor/decisão de dinheiro em dado velho. Daily → stale>30h.
    ...
    UNION ALL
    -- HIGH (frescor): sync de clientes/produtos do Omie (sync-products-customers-daily → sync_state).
    -- Morte calada = clientes/produtos novos somem do CRM/carteira/catálogo. Daily → stale>30h. broken se
    -- faltar entidade (uma nunca rodou), nunca sincronizou, ou última terminou em erro.
    SELECT 'vendas_cadastros'::text, 'vendas'::text,
      CASE WHEN ss.n_entidades < 2 OR ss.pior_last IS NULL OR ss.algum_erro THEN 'broken'
           WHEN now() - ss.pior_last > interval '30 hours' THEN 'stale' ELSE 'ok' END,
      EXTRACT(EPOCH FROM now() - ss.pior_last)::bigint, (30*3600)::bigint, 'sync_state.last_sync_at (customers/products)'::text,
      'Sync de clientes/produtos (vendas): ' || COALESCE(to_char(ss.pior_last,'DD/MM HH24:MI'),'nunca'),
      ss.erro_msg,
      CASE WHEN ss.n_entidades < 2 OR ss.pior_last IS NULL THEN 'sync_all (customers/products) nunca completou para alguma entidade'
           WHEN ss.algum_erro THEN 'Última sync de clientes/produtos terminou em erro' ELSE NULL END,
      'Cheque o cron sync-products-customers-daily (omie-analytics-sync sync_all account=vendas) e a tabela sync_state'::text,
      'critical'::text
    FROM (
      SELECT
        count(*)::int AS n_entidades,
        min(s.last_sync_at) AS pior_last,
        bool_or(s.status = 'error') AS algum_erro,
        (SELECT s2.error_message FROM public.sync_state s2
          WHERE s2.account='vendas' AND s2.entity_type IN ('customers','products') AND s2.status='error'
          ORDER BY s2.updated_at DESC LIMIT 1) AS erro_msg
      FROM public.sync_state s
      WHERE s.account='vendas' AND s.entity_type IN ('customers','products')
    ) ss
    UNION ALL
...
-- Watchdog: os 2 novos HIGH ENTRAM no push (alerta na transição ok→degradado). Não colidem com o
-- fin_sync_watchdog (tipos data_health_* distintos). custos_produtos='estoque'/warning→aviso/atencao;
-- vendas_cadastros='vendas'/critical→critico/urgente. Corpo verbatim da 20260527220000; só amplio o IN.
...
-- Heartbeat: inclui os 2 novos no resumo de saúde de dados. Corpo verbatim da 20260527240000; só amplio o IN.
...
BEGIN
  ...
  WHERE source IN ('vendas_pedidos','estoque_inventario','reposicao_sugestoes','carteira_scores',
                   'custos_produtos','vendas_cadastros','alert_channel');
  ...
  VALUES ('oben', 'outro', 'info',
          ...
          E'.\nChecks (vendas/cadastros/estoque/custos/reposição/carteira/canal):\n'||COALESCE(v_dh_resumo,'(sem dados)'),
