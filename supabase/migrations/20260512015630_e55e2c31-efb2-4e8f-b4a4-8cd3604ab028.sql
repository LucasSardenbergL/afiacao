-- Indexes para acelerar joins/filtros
CREATE INDEX IF NOT EXISTS idx_pedido_compra_item_sku
  ON public.pedido_compra_item (sku_codigo_omie);
CREATE INDEX IF NOT EXISTS idx_pedido_compra_item_pedido
  ON public.pedido_compra_item (pedido_id);
CREATE INDEX IF NOT EXISTS idx_pedido_compra_sugerido_emp_ciclo_status
  ON public.pedido_compra_sugerido (empresa, data_ciclo, status);

-- Otimiza a função: pré-agrega "em trânsito recente" em uma única CTE,
-- eliminando 3 subqueries correlacionadas por SKU.
...
BEGIN
  ...
  WITH em_transito AS (
    SELECT
      pcs2.empresa,
      pci.sku_codigo_omie::text AS sku_codigo_omie,
      SUM(pci.qtde_final) AS qtde
  ...
  preco_medio AS (
    SELECT
      slh.empresa::text AS empresa,
      slh.sku_codigo_omie::text AS sku_codigo_omie,
      AVG(slh.valor_total / NULLIF(slh.quantidade_recebida, 0)) AS preco_unitario,
      COUNT(*) AS n
  ...
  skus_necessitando AS (
    SELECT
      ...
      (COALESCE(sea.estoque_fisico, 0)
        ...
        + COALESCE(et.qtde, 0)) AS estoque_efetivo,
      (sp.estoque_maximo
        - (COALESCE(sea.estoque_fisico, 0)
           + COALESCE(sea.estoque_pendente_entrada, 0)
           + COALESCE(et.qtde, 0))) AS qtde_sugerida,
      COALESCE(pm.preco_unitario, 0) AS preco_unitario,
      (pm.n IS NULL) AS primeira_compra,
      fh.horario_corte_pedido,
      fh.valor_maximo_mensal,
      fh.delta_max_perc
    ...
    LEFT JOIN em_transito et
      ON et.empresa = sp.empresa AND et.sku_codigo_omie = sp.sku_codigo_omie::text
    LEFT JOIN preco_medio pm
      ON pm.empresa = sp.empresa AND pm.sku_codigo_omie = sp.sku_codigo_omie::text
    WHERE sp.empresa = p_empresa
      ...
      AND (COALESCE(sea.estoque_fisico, 0)
           + COALESCE(sea.estoque_pendente_entrada, 0)
           + COALESCE(et.qtde, 0)) <= sp.ponto_pedido
  ),
...
-- Margem extra para chamadas manuais via PostgREST
ALTER FUNCTION public.gerar_pedidos_sugeridos_ciclo(text, date) SET statement_timeout = '120s';