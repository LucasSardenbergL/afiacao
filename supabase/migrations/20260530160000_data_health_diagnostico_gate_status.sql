-- ============================================================
-- Gate de DIAGNÓSTICO por status nos checks de saúde de dados
-- ============================================================
-- Bug (irmão do fix de frontend mergeado no PR #464): um check com status='ok'
-- ainda retornava last_error/probable_cause/how_to_fix preenchidos. O caso clássico:
-- o check vendas_pedidos calcula o STATUS por completes recentes, mas o last_err é
-- uma subquery DESACOPLADA que pega o último error_message de TODO o histórico — então
-- carrega indefinidamente a flag 'orphaned_running_timeout' que o fin-sync-watchdog
-- grava ao varrer uma órfã de sync JÁ recuperada. Resultado: card verde exibindo erro
-- morto. (probable_cause já era NULL quando ok na maioria; last_err e how_to_fix escapavam.)
--
-- Fix: alinha a FONTE à apresentação (shouldShowDiagnostics no frontend) — anula os 3
-- campos de diagnóstico no SELECT final quando status='ok'. Gate genérico na projeção
-- (não toca as subqueries por-check) → cobre TODOS os checks (vendas_pedidos,
-- omie_sync_financeiro etc.) sem risco de transcrição. A RPC para de retornar erro
-- recuperado como atual — mesma filosofia tail-failing do fin_sync_watchdog (20260528000000).
--
-- SEGURO: data_health_watchdog e fin_sync_heartbeat só leem message/status e só agem
-- com status<>'ok' → anular diagnóstico em check ok tem ZERO efeito nos alertas/email.
-- Corpo (CTE de checks) = verbatim da 20260528140000; muda APENAS a projeção final.
...
AS $$
  ...
  SELECT c.source, c.domain, COALESCE(NULLIF(c.status, ''), 'unknown') AS status,
    ...
    -- Gate de diagnóstico: check saudável (status='ok') não expõe erro/causa/como-resolver
    -- (evita exibir falha transitória já recuperada). Espelha shouldShowDiagnostics no frontend.
    CASE WHEN c.status = 'ok' THEN NULL ELSE c.last_error END AS last_error,
    CASE WHEN c.status = 'ok' THEN NULL ELSE c.probable_cause END AS probable_cause,
    CASE WHEN c.status = 'ok' THEN NULL ELSE c.how_to_fix END AS how_to_fix,
    c.severity
  FROM checks c;
$$;
