...
> **Status: replay VALIDADO em 2026-08-08** (47.285 linhas; 2ª geração do dia, que incorpora o CHECK de finitude money-path — PR #1691 — e a `expirar_planos_taticos` — PR #1693; validação via `db/refresh-snapshot.sh`, provas 5/5, **paridade objeto-a-objeto contra prod 0 faltando / 0 sobrando**). Geração anterior: 2026-08-08, 1ª do dia (47.249 linhas, ATP fase 1.1 + fase 2). `db/verify-snapshot-replay.sh` roda prelude + stubs (`db/stubs-supabase.sql`) + snapshot num Postgres 17 descartável (transação única, `ON_ERROR_STOP`) → **`exit 0`**, restore **limpo**, contagens públicas (329/79/2/318/125/14/688) e **enforcement RLS amostrado OK** (own=1 / staff=2 / anon=0). _(A 1ª geração de 2026-06-19, public-only, falhava com `schema "private" does not exist`; resolvido incluindo `--schema=private`.)_ **Limitação:** prova ordem/dependência/sintaxe + RLS amostrada do `public`; **não** o runtime Supabase real completo (RLS/`auth`) — o "Gold" pede projeto Supabase vazio ou docker (ver `schema-rebuild-runbook.md` §Verificação). ⚠️ **O script imprime as contagens mas NÃO as asserta** (o "esperado" no `echo` é texto fixo, hoje defasado); a única asserção com `exit 1` é a de enforcement RLS. Comparar contagem é trabalho de quem lê.
