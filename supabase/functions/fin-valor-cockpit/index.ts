...
// entre células 'real' — combo "parece seguro, está frágil". NÃO min|folga| (folga negativa já é sinalizada por
// evp<0/perda_garantida; o |abs| a devolveria, escondendo o blind spot positivo — Codex xhigh 2026-06-24). É
// locator → guarda a receita do combo vencedor (combo ínfimo não vira alarme). Epsilon 1e-9 exclui break-even==k.
...
function montarCelulasComboEVP(input: { combos: ComboInput[]; capitalClientes: CapitalCliente[]; capitalSKUs: CapitalSKU[]; k: number | null; banda_hurdle?: number }) {
  ...
  const rollup = (keyFn: (c: Cel) => string) => {
    const m = new Map<string, { receita: number; quantidade: number; cm: number; cmNull: boolean; encargo: number; encargoNull: boolean; encargoTotal: number; encargoTotalNull: boolean; evp: number; evpNull: boolean; evpTeto: number; evpTetoNull: boolean; evpIncompleto: boolean; perdaGarantida: boolean; cmIncompleto: boolean; qtdSensiveis: number; folgaMin: { pp: number | null; receita: number | null } }>();
    for (const cel of celulas) {
      const key = keyFn(cel);
      const acc = m.get(key) ?? { receita: 0, quantidade: 0, cm: 0, cmNull: true, encargo: 0, encargoNull: true, encargoTotal: 0, encargoTotalNull: true, evp: 0, evpNull: true, evpTeto: 0, evpTetoNull: true, evpIncompleto: false, perdaGarantida: false, cmIncompleto: false, qtdSensiveis: 0, folgaMin: { pp: null, receita: null } };
