...
```
promocao_campanha (campanhas)
  ├─ promocao_item (SKUs da campanha, com preço e desconto)
  │    └─ desconto_extra_* (campos de negociação esporádica)
  └─ promocao_negociacao_evento (timeline de negociações)

fornecedor_aumento_anunciado (aumentos)
  └─ fornecedor_aumento_item (categorias + %)
       └─ categoria_aumento_familia_mapeamento (vínculo categoria → família Omie)

sku_leadtime_history.origem_compra  (normal | oportunidade_promo | oportunidade_aumento)
pedido_compra_sugerido.tipo_ciclo   (normal | oportunidade_promo | oportunidade_aumento)

fornecedor_email_polling (3 remetentes configurados)
  └─ fornecedor_email_polling_log (histórico de execuções)

fornecedor_alerta (central de notificações)

v_oportunidade_economica_hoje     (view unificada)
v_sku_aumento_vigente             (SKUs afetados por aumentos ativos)
v_promocao_avaliacao_hoje         (SKUs em promoção ativa)
v_promocao_item_efetivo           (desconto base + extra)
```
