...
// folga_hurdle_min_pp/_receita: a folga do combo 'real' MAIS PRÓXIMO do hurdle POR CIMA (menor folga positiva) —
// fecha o ponto cego da banda (combo logo FORA dela, ex.: break-even 0,355, é invisível em qtd_combos_sensiveis).
export type RollupCliente = { cliente: string; receita: number; cm: number | null; encargo: number | null; encargo_total: number | null; evp: number | null; evp_teto: number | null; evp_incompleto: boolean; perda_garantida: boolean; cm_incompleto: boolean; qtd_combos_sensiveis: number; folga_hurdle_min_pp: number | null; folga_hurdle_min_receita: number | null };
...
export type EmpresaEVP = {
  ...
  folga_hurdle_min_pp: number | null;      // menor folga POSITIVA (break_even − k > 0) entre células 'real' — o combo mais próximo do hurdle por cima
...
export function montarCelulasComboEVP(input: {
  ...
  const rollup = (keyFn: (c: CelulaEVP) => string) => {
    ...
    for (const cel of celulas) {
      ...
      acumularFolgaMinPositiva(acc.folgaMin, cel);                                                 // combo real mais próximo do hurdle por cima
      m.set(key, acc);
    }
    return m;
  };
