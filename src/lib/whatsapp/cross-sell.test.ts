...
function c(sku: number, afinidade: number | null, nome = `P${sku}`): CrossSellCand {
  return { omie_codigo_produto: sku, nome, afinidade };
}
...
describe('selecionarCrossSell', () => {
  ...
  it('ordena por AFINIDADE (adimensional, não R$) desc; null por último', () => {
  ...
  it('dedupe por SKU mantendo o de maior afinidade', () => {
    ...
    expect(sevens[0].afinidade).toBe(80);
  });
  it('capa em n', () => {
