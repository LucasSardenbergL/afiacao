...
- **Acesso ao banco:** **leitura/diagnóstico EU rodo direto** via `~/.config/afiacao/psql-ro` (role `claude_ro`; read-only é do **wrapper**, não do papel). **Escrita** só via SQL Editor do Lovable (founder cola). → `database.md` §1
...
- **Validação só conta com EVIDÊNCIA POSITIVA** — rode o comando autoritativo, confirme que **terminou** e capture `exit 0` **colado**. Ausência de sinal NÃO é aprovação: processo enfileirado, log sem linha de conclusão, `grep` sem ocorrência (**confira caixa/acento**) e linter que não tem a regra são **ausência de dado**. **as armadilhas de shell fabricam VEREDITO** (`| tail` engole o exit code; `$?` sobrescrito por `echo` no meio; `echo|jq` corrompe JSON no zsh; `grep` daqui é shim: dobra acento; `/private/tmp` morre no reboot; flag homônima BSD×GNU não falha, faz OUTRA coisa; wrapper (`heavy`/`timeout`) aborta com exit≠0 sem rodar nada; `cmd; echo exit=$?` zera o exit do COMPOUND) → `docs/historico/evidencia-positiva-shell.md`
...
- **Edge tem 4 gates no CI e nenhum cobre o outro:** `test:edges` (suíte Deno, `--no-remote` ⇒ teste de edge **não pode ter import remoto** — extraia a lógica PURA, **nunca afrouxe o flag**); `edges:typecheck` (a Deno **NÃO** type-checa); o **vitest**, que vigia a FORMA da edge lendo-a como TEXTO ⇒ refactor sintático reprova no `bun run test`; e **`sonda:bump`**. → `docs/historico/ci-testes-edge-deno.md`
