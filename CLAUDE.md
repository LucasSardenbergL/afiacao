...
- **TypeScript strict mode** — `tsconfig.app.json` tem `strict: false`, `noImplicitAny: false`. Resolve raiz de 1300 lint errors (97% `no-explicit-any`). **Infra incremental pronta**: `tsconfig.strict.json` lista files que passam strict (`strict: true` + `noImplicitAny` + `strictNullChecks` + `noUnusedLocals/Parameters`). Rodar via `bun run typecheck:strict`. CI bloqueia se regressão nos files migrados. Pra migrar mais files: garantir 0 `any` + tipos explícitos + adicionar ao `include` de `tsconfig.strict.json`. Convergência: quando 100% do `src/` estiver em strict, mover flags pra `tsconfig.app.json` e deletar `tsconfig.strict.json`. Progresso (2026-05-23 — **375 files no `include`**): os hooks de engine (`useBundleEngine`, `useTacticalPlan`, `useFarmerExperiments`, `useFarmerPerformance`, `useCrossSellEngine`, `useCopilotEngine`), o módulo de governança, o de loyalty/gamification, os componentes extraídos de `FinanceiroDashboard` (`components/financeiro/dashboard/*`) e os tabs do dashboard executivo (`des/*`) **já estão em strict**. Restam **44 `no-explicit-any` em arquivos de app** (+8 em testes +20 em Edge Functions Deno, ambos **fora do escopo** do `tsconfig.strict.json`). Maior bolsão: `AdminRoutePlanner` (13 — decomposto em #169, mas ainda com any); o resto são 28 arquivos com 1–2 `any` cada, espalhados em pages/hooks/components. Padrão das waves recentes: trocar `as any`/anotação `: any` por inferência ou tipo concreto, remover dead-code que o `noUnusedLocals` revela (cascata), e cuidar de files puxados transitivamente por lazy import. ⚠️ **COORDENAÇÃO (obrigatória — trabalho paralelo já causou retrabalho: a #161 decompôs `FinanceiroDashboard` enquanto outra sessão o tipava):** antes de QUALQUER migração strict, leia [`docs/strict-migration-lanes.md`](docs/strict-migration-lanes.md), rode `gh pr list --state open`, reserve uma lane livre (status 🔵 + sua branch) já no **primeiro commit** da sua branch, e **só toque arquivos da sua lane**. No `tsconfig.strict.json`, adicione paths **no fim do `include`** agrupados pela sua lane — **não reordene o array** (reordenar 375 paths = conflito com todo PR em voo). A reestruturação "um tsconfig por lane" é flag-day (ver claim file).
