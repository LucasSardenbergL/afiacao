...
- **Ritual de fecho** (ver bloco do topo): quando o Lucas pergunta "posso excluir a sessão?", além do resumo de fecho, rodar `wt:status` + `wt:clean` (+ oferecer `wt:prune` pra enxugar as fantasmas) e reportar MB/worktrees liberados. O worktree ATUAL fica de fora por padrão (sessão viva); use `--include-current` no `wt:clean` só quando ele confirmar que vai fechar.
