...
- **Validação só conta com EVIDÊNCIA POSITIVA** — rode o comando autoritativo, confirme que **terminou** e capture `exit 0` **colado**. Ausência de sinal NÃO é aprovação: processo enfileirado, log sem linha de conclusão, `grep` sem ocorrência (**confira caixa/acento**) e linter que não tem a regra são **ausência de dado**. **8 armadilhas de shell fabricam VEREDITO** (`| tail` engole o exit code; `$?` sobrescrito por `echo` no meio; `echo|jq` corrompe JSON no zsh; `grep` daqui é shim: dobra acento; `/private/tmp` morre no reboot; flag homônima BSD×GNU não falha, faz OUTRA coisa; wrapper (`heavy`/`timeout`) aborta com exit≠0 sem rodar nada; `cmd; echo exit=$?` zera o exit do COMPOUND) → `docs/historico/evidencia-positiva-shell.md`
