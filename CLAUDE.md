...
- **Validação só conta com EVIDÊNCIA POSITIVA** — rode o comando autoritativo, confirme que **terminou** e capture `exit 0` **colado**. Ausência de sinal NÃO é aprovação: processo enfileirado, log sem linha de conclusão, `grep` sem ocorrência (**confira caixa/acento**) e linter que não tem a regra são **ausência de dado**. **7 armadilhas de shell fabricam verde** (`| tail` engole o exit code; `$?` sobrescrito por um `echo` no meio; `echo|jq` corrompe JSON no zsh; `grep` daqui é shim que dobra acento; log em `/private/tmp` morre no reboot; flag homônima BSD×GNU não falha, faz OUTRA coisa) → `docs/historico/evidencia-positiva-shell.md`
...
- **Gate textual limpa comentário com o stripper COMPARTILHADO** (`removerComentarios` de `@/lib/gates/limpeza-fonte`, que entende string/template/regex) — **nunca** regex local: ela não sabe o que é string, e um `/*` dentro de string pareia com o `*/` seguinte e apaga o miolo do arquivo ANTES da medição (verde por CEGUEIRA). Gate novo herda o sentinela do maior bloco contíguo descartado — e escolhe a CAMADA do stripper pelo que MEDE: em markdown, `removerCodigo` (cerca+crase) zera o gate de citações, cuja citação NASCE entre crases ⇒ `removerCercas` de `scripts/lib/markdown-codigo.ts`, presa por teste-sentinela. → `docs/historico/gates-textuais-cegos.md`
- **Corte por ranking: o teto é o EIXO, não o tamanho.** `ORDER BY x DESC LIMIT n` só é seguro se `x` for o MESMO eixo da decisão que a tela serve — quando não é, aumentar `n` não corrige, só adia. Corrija por **UNIÃO com a fonte que conhece a decisão** (carteira/roteiro), nunca por limite maior. Corte deliberado e revisado **não** é corte inócuo. → `docs/historico/roteirizador-corte-cidades.md`
- **Fase N+1 exige SINAL da fase N** (a de PRODUTO da regra acima): exija ≥1 sinal POSITIVO de uso em prod **com denominador** — "no ar e ninguém reclamou" é ausência de dado. Superfície de uso nasce **com o sensor**; sem sensor, a fase N+1 é instalá-lo. "Quando medir" é **query**, não recado. → `docs/historico/fase-sem-sinal.md`
- **Edge tem 3 gates no CI e nenhum cobre o outro:** `test:edges` (suíte Deno, `--no-remote` ⇒ teste de edge **não pode ter import remoto** — extraia a lógica PURA, **nunca afrouxe o flag**); `edges:typecheck` (a Deno **NÃO** type-checa); e o **vitest**, que vigia a FORMA da edge lendo-a como TEXTO ⇒ refactor sintático reprova no `bun run test`. → `docs/historico/ci-testes-edge-deno.md`
