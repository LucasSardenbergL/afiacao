...
- **🪟 Contexto:** em sessões longas, **sugira `/compact foco: <próximo passo>` proativamente** (compact sem foco preserva mal; não há auto-compact por %). **No 2º compact da MESMA sessão → proponha split com `/handoff-sessao`** (1 entrega = 1 sessão). Subagentes têm janela própria. **"Retome" sem briefing → `scripts/onde-parei.sh` ANTES de reconstruir** (exit 3 = nada a retomar · 6 = não consegui consultar, nunca "nada").
...
- **Lovable: 3 deploys fora do merge** (Publish · edge · migration) — **merge ≠ produção**; custom **não** auto-aplica (SILENCIOSA); **nunca** tocar `supabase/migrations/` (=DR). **4ª: o SW só troca de build quando o cliente clica** — bytes = **disponibilidade, não adoção**. → `deploy.md`/`database.md`
- **Acesso ao banco:** **leitura/diagnóstico EU rodo direto** via `~/.config/afiacao/psql-ro` (role `claude_ro`; read-only é do **wrapper**, não do papel). **Escrita**: eu aplico no ENVELOPE (idempotente+transacional+postcondição, pré-voo 🟢, validada por fora); fora dele, founder no SQL Editor. Em script, sempre `-v ON_ERROR_STOP=1` + marcador positivo de fim (sem isso o wrapper sai **0 com ERROR**) — vigiado no CI por `test:hooks`. → `database.md` §1 · `docs/historico/psql-ro-exit-zero-em-sql-que-falhou.md`
...
- **Supabase RLS:** fechar função por `REVOKE` exige DUAS pontas (`PUBLIC` **e** `anon`), via `has_function_privilege`; SECURITY DEFINER bypassa RLS (gate na fronteira). Tabela nova **sempre** com RLS. → `database.md`
...
- **Teste SQL negativo** com `WHEN OTHERS THEN 'OK'` é teatro → capturar a SQLSTATE esperada + re-lançar o resto + **falsificar** (sabotar e exigir vermelho); o **gêmeo em TS** é `toThrow()`/`assertThrows` pelado — case a **marca do ramo**, não "lançou algo". RLS prova-se sob `SET ROLE authenticated` + GUC. **Falsificar em UM ambiente não prova a asserção** (#1483, locale) → rode nos DOIS (`LC_ALL=C` **e** `pt_BR.UTF-8`) e case string **ASCII, caixa fixa, sem `-i`**. E **COMMITE antes de falsificar** — `restaurar()` costuma ser `git checkout --`. Em profundidade, sabote **uma camada por vez**: a que fica VERDE é redundante ou inalcançada. E **sabotar sem CONTROLE verde na MESMA invocação do laço** (cópia/`LC_ALL`/override, abortando antes do 1º `sed`) é teatro: sempre-vermelha aprova TUDO, e a suíte crua do `test:hooks` é outra invocação — `--falsificar` rápido demais não rodou a suíte ([base](docs/historico/falsificacao-sem-linha-de-base.md)). → `money-path.md`
...
- **Deploy de edge: quem decide é `bun run pendencias:deploy` (ledger `deploy_atestacoes`), não o diff do PR.** Deploy ⇔ `(versao, fonte)` servido ≠ main; "o mapa mudou" **não** é motivo. Sonda humana só na 1ª atestação e **uma por leva** pós-deploy — nunca "para conferir de novo". **Cron de sonda ATIVO** (allowlist `sonda-cron-alvos.ts`; `sonda:cron-prova` exige zero efeito). → `docs/historico/deploy-redundante-ledger-e-cron-de-sonda.md`
...
Health stack (`/health`): typecheck · lint · test · `bunx knip` (deadcode) · `bun run lint:shell` (shellcheck·`scripts/`·hooks·**`db/`**). Worktree novo: `bun install`.
