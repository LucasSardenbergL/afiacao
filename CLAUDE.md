...
- ⚠️ **Deploy é manual via chat do Lovable após cada merge** — `omie-financeiro`, `fin-cashflow-engine` e as engines de valor/ação/regime (`fin-valor-engine`, `fin-valor-cockpit`, `fin-next-best-action`, `fin-regime-tributario`) leem o arquivo do repo (branch main). Sem isso, a main e a produção divergem.
...
- **🔴 RLS ampla em 5 tabelas-irmãs de carteira (follow-up de segurança PRIORIZADO, aberto em 2026-05-25 junto do PR #329)** — a mesma exposição `FOR ALL master-OR-employee` que o #329 fechou em `farmer_client_scores`/`customer_visit_scores` existe em **`farmer_recommendations`, `farmer_bundle_recommendations`, `farmer_calls` (a SELECT "own" é anulada pelo OR; contém conteúdo de ligações), `route_visits` (geo/visitas), `farmer_copilot_sessions`** → qualquer `employee` lê/gerencia a carteira alheia via PostgREST. (`farmer_performance_scores` JÁ está OK: view own-scoped + manage master-only — não mexer.) **Mesmo rito do #329**, mas com mapeamento de consumidores POR tabela (route planner, copilot, tabs Intelligence, dashboards de performance, telas Farmer) antes de apertar — NÃO batchar às cegas; reusar `carteira_visivel_para` + `pode_ver_carteira_completa`; `service_role` bypassa. Acima do "item 4" (RPCs view-as `_for(target)`, que já foi entregue em paralelo — `viewas_rpcs_for`/`viewas_access_targets`/`impersonation_audit` na main).
...
- ✅ **CI obrigatório no GitHub** — `.github/workflows/ci.yml` (job `validate`: typecheck + typecheck:strict + test + build **bloqueantes**; lint informativo) roda em PR+push pra main, e a branch protection da `main` **exige o check `validate`** (`enforce_admins: false`, sem review obrigatório). PR #4 não se repete.
