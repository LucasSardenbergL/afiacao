...
- **PostgREST:** capa em 1.000 linhas silencioso, **inclusive `.rpc()`** (`.range()` + `.order` estável). **`.or()` quebra em UPDATE** (42703 mesmo a coluna existindo) → RPC SQL-pura; negação é **NULL-blind**. **Nunca** interpolar input em `.or()` cru (ESLint `no-restricted-syntax` barra — usar helpers `@/lib/postgrest`). `.order()` estável só salva tabela PARADA: sob escrita concorrente o offset desloca — mas só se ela cruzar a **fronteira do RECORTE** (INSERT/DELETE sempre; UPDATE só na coluna do `.order()`/`WHERE`), o que INVERTE o ranking por volume de escrita ⇒ `fetchAllKeyset` (`docs/historico/paginacao-offset-janela.md`). → `database.md`
...
- **Sonda ausente: degradar é certo no SENSOR, errado no script que APAGA.** No `wt:clean`/`wt:prune`, `lsof` em 127 esvazia o `active_file` e a worktree de sessão VIVA vira `would -250 MB`; sem `md5`, `[ "" = "" ]` faz o `.env` com segredo único passar como descartável. ⇒ sonda de script destrutivo é **fail-CLOSED**, e **`command -v` não basta** (presente-porém-QUEBRADA esvazia o guard igual): exija resposta POSITIVA. E `set -e` é suspenso pelo CONTEXTO DE CHAMADA (`if ! f` sobrevive; `f;` morre com 141) ⇒ `| head` latente engana a varredura. → `docs/historico/sonda-ausente-em-script-que-apaga.md`
