...
- **Deploy de edge: quem decide é `bun run pendencias:deploy` (ledger `deploy_atestacoes`), não o diff do PR.** Deploy ⇔ `(versao, fonte)` servido ≠ main; "o mapa mudou" **não** é motivo. Sonda humana só na 1ª atestação e **uma por leva** pós-deploy — nunca "para conferir de novo". **Sem cron de sonda ativa** (rollback pré-sensor rodaria o fluxo real). → `docs/historico/deploy-redundante-ledger-e-cron-de-sonda.md`
