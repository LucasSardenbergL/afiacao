...
- **`bun run wt:prune`** (`scripts/wt-prune.sh`, 2026-06-14, desenho validado com o codex + teste de falsificação) — remove a worktree cuja **CONVERSA foi EXCLUÍDA** do histórico do app (e cujo trabalho está salvo): é o "excluir a worktree junto com a conversa", **diferido e seguro** (excluir a conversa nos 3-pontos **não** dispara hook capturável — confirmado na doc oficial; então em vez de reagir ao evento, o prune **reconcilia** pela ausência do transcript). O que o `wt:clean` não faz. **DRY-RUN por padrão**; `--yes` executa. Só remove quando TUDO vale: não-atual, **sem sessão/processo vivo** (lsof), não-`locked`, **conversa EXCLUÍDA** (sem `.jsonl` em `~/.claude/projects/<path>` — conversa **ainda viva no histórico é PRESERVADA**, mesmo na main; política escolhida pelo founder), **working tree limpo** (`git status --porcelain --untracked-files=all`), **arquivos ignored só na allowlist** (node_modules/dist/build/.vite/.turbo/coverage/*.log e `.env` byte-idêntico ao da worktree atual — **qualquer outro ignored BLOQUEIA**, ex. um `.claude/settings.local.json` próprio), e **trabalho 100% salvo** por **(A)** HEAD ancestral de `origin/main` **ou (B)** branch com PR mergeado cujo `headRefOid == HEAD` (⚠️ "PR mergeado" sozinho NÃO basta — a branch pode ter commit POSTERIOR ao merge). `git fetch origin` é **obrigatório** (aborta se falhar); **revalida** imediatamente antes do `remove`; **nunca `--force`**; **NÃO apaga a branch** (todo commit fica recuperável com `git worktree add <path> <branch>`). ⚠️ Reduz a `git worktree list`, **não** alivia RAM (worktree parada quase não come RAM — o gargalo é **sessão viva**).
- **`bun run wt:map`** / **`bun run wt:label "<assunto>"`** (`scripts/wt-map.sh`, 2026-06-14) — lista cada worktree com o **assunto** da sessão (▸ atual · ● viva · ○ parada · branch), pra achar qual é qual sem decorar o slug aleatório (`happy-satoshi-…`). Assunto = **rótulo manual** (`wt:label` grava no git-dir da worktree) **ou** fallback best-effort do **1º prompt** do transcript (`~/.claude/projects`, fail-soft — **não é contrato**, o formato interno pode mudar). Decisão eu+codex: **não** renomear worktree/branch pra casar com o título da conversa (acoplamento frágil com refs do Claude/Git) **nem** auto-capturar via hook no `settings.json` comitado (afetaria o boot de TODAS as sessões — fica como incremento opcional, não default).
...
- **Ritual de fecho** (ver bloco do topo): quando o Lucas pergunta "posso excluir a sessão?", além do resumo de fecho, rodar `wt:status` + `wt:clean` + `wt:reap` (+ oferecer `wt:prune` pra enxugar as fantasmas) e reportar MB/processos/worktrees liberados. O worktree ATUAL fica de fora por padrão (sessão viva); use `--include-current` no `wt:clean` só quando ele confirmar que vai fechar.
