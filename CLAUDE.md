...
- **~334 cores hardcoded restantes** (era ~370; o sweep converteu **36 status-semânticas** em 3 slices: #352 anterior + #374 slice 2 [badges de status, Tint dashboard/sync, Production/Productivity] + #375 slice 3 [TintReconciliation/Mapping/Integrations] — 2026-05-27). Padrão: converter **só** cor inequivocamente status → tokens `text-status-*`/`bg-status-*-bg`/`border-status-*/40`, espelhando o `PortalStatusBadge`; mapas `statusConfig`/`Record<Status,cor>` são o caso mais limpo. **O restante é majoritariamente decorativo/data-viz/categórico** (zonas de mapa do routePlanner, ranks/medalhas do AdminGamification, theming de coluna do Kanban, Star de avaliação) + **acoplado** (`ORDER_STATUS`/`EMPLOYEE_ORDER_STATUS` = chaves de lookup do `StatusBadge`, mudar quebra o `semanticMap`) → exige julgamento por ocorrência, **`/design-review`** em sessão dedicada. Top restante: call/SpinSuggestionCard (41×), Admin.tsx (30×), AdminGamification. ⚠️ **Lição:** ao checar se um teste asserta numa cor antes de converter, use `grep -r` (o glob `src/**/__tests__/` não expande recursivo sem `globstar` → falso-negativo; o CI pegou `tintImport/types.test.ts` assertando `'green'`).
