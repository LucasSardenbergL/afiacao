...
- **`CREATE OR REPLACE` função/view:** pré-flight `pg_get_functiondef`/`pg_get_viewdef` da PROD (apply manual diverge do repo); a última a recriar **vence**. VIEW só ACRESCENTA coluna no fim (preservar ordem exata). **Repita o `WITH (security_invoker=on)` em TODO replace** — omitir RESETA a opção → a view lê como OWNER e **bypassa RLS**: falha ABERTA, muda autorização e não comportamento, o CI não vê. Mas `off` **também é DESENHO** (view-gate `selfservice_*`; discriminante = `barrier=true` + `relacl` sem `anon`) — ligar `on` ZERA o customer. **`DROP FUNCTION`+`CREATE` RESETA o ACL** (`CREATE OR REPLACE` preserva): a função renasce com o default privilege, que concede EXECUTE a `anon` E `authenticated` — reemita o `REVOKE` **nomeando as roles** (de `PUBLIC` não basta); a Parte E do `authz:check` vigia as classificadas. Reescrita cirúrgica por `regexp_replace` não escreve `CREATE` ⇒ invisível como definição: em função do `AUTHZ_MANIFEST` é ERRO da Parte D do `authz:check` (use `CREATE OR REPLACE`, ou registre em `scripts/authz-reescritas-conhecidas.ts`). → `database.md`
...
- **PostgREST:** capa em 1.000 linhas silencioso (`.range()` + `.order` estável) — **vale para `.rpc()` também**: `fetchAllPages` só protege quem passa por ele, então RPC nova é o ponto cego (corrigir o truncamento das TABELAS não a alcança) e a resposta capada é indistinguível de "acabou". Num gate de EXCLUSÃO a cauda perdida não encolhe a saída, **zera** (#1785: `get_skus_margem_positiva` capada ⇒ lote vazio ⇒ o writer pula a gravação, sem erro, e a tabela congela). **`.or()` quebra em UPDATE** (42703 mesmo a coluna existindo) → RPC SQL-pura; negação é **NULL-blind**. **Nunca** interpolar input em `.or()` cru (ESLint `no-restricted-syntax` barra — usar helpers `@/lib/postgrest`). → `database.md`
...
- **Cliente do grupo = 2 cadastros Omie LEGÍTIMOS** (`servicos`/Colacor SC + `vendas`/Oben — CNPJs distintos por vantagem fiscal): os **1.633 users `@placeholder.local` sem `profiles`** são **aliases fiscais**, **NÃO lixo de import** → **nunca deleção ad-hoc** de `auth.users` (o CASCADE cobre só 14 tabelas — pedidos/endereços/scores viram uuid pendurado). Mas **canonicalizar o histórico é decisão legítima de produto** (migração completa preservando proveniência ERP, ≠ deleção). Discriminante = **ausência de `profiles`** (`@placeholder.local`/`ja_logou=0` valem p/ 6.910 dos 6.914); `eligible` **não** é fronteira estrutural. Já redescoberto 2×. → `database.md` §5
...
- **Teste SQL negativo** com `WHEN OTHERS THEN 'OK'` é teatro → capturar a SQLSTATE esperada + re-lançar o resto + **falsificar** (sabotar a migration e exigir vermelho); RLS prova-se sob `SET ROLE authenticated` + GUC (psql é superuser, bypassaria). Mas **falsificar em UM ambiente não prova a asserção** — a sabotagem fica vermelha no shell de quem escreveu e VERDE no do founder (#1483, acidente de locale) → rode nos DOIS (`LC_ALL=C` **e** `pt_BR.UTF-8`) e case string **exclusiva do ramo certo, ASCII, caixa fixa, sem `-i`**. E **COMMITE antes de falsificar** — `restaurar()` costuma ser `git checkout --`, que sobre trabalho não-commitado APAGA a implementação. → `money-path.md`
- **Validação só conta com EVIDÊNCIA POSITIVA** — rode o comando autoritativo, confirme que **terminou** e capture `exit 0`. Ausência de sinal NÃO é aprovação: processo enfileirado (`heavy` esperando vaga), log sem linha de conclusão, `grep` sem ocorrência e linter que não tem a regra (cada linter só enxerga a PRÓPRIA supressão) são **ausência de dado** — 3 instâncias numa sessão só. Armadilhas de shell: `cmd | tail` **ENGOLE o exit code** (`> log 2>&1; echo $?`); no zsh `echo "$json" | jq` **corrompe o JSON** (sempre `printf '%s' "$x" | jq`); o `grep` daqui é shim p/ `ugrep`, que dobra acento em TODO locale (`command grep` ao reproduzir). Comandos pesados (test/build/typecheck/vitest) → prefixar **`heavy`** (semáforo de RAM da M2 8GB). → `worktrees.md`, `deploy.md`
...
- **Edge tem suíte Deno PRÓPRIA e ela BLOQUEIA o CI** (`test:edges`, que roda `--no-remote`): teste de edge **não pode ter import remoto** — extraia a lógica PURA, **nunca afrouxe o flag** (jsr.io no caminho de entrega de TODO PR). E ela **NÃO type-checa**: quem checa é o `edges:typecheck`. → `docs/historico/ci-testes-edge-deno.md`
...
Todo PR não-draft **auto-mergeia (squash) quando o CI `validate` passa** (`.github/workflows/auto-merge.yml`, zero clique do founder). Para **segurar** um PR, deixe-o **DRAFT**. Nunca `gh pr merge --admin` de rotina (o auto-merge espera o verde — não bypassa o CI). **Ao criar/atualizar PR: arme `scripts/pr-watch.sh <nº>` em background** (Bash `run_in_background:true`) e, no desfecho, avise via PushNotification (mergeado/conflito/CI vermelho) — o founder não fica de poller. **Exit 6 ≠ 5:** 5 = consultei e o PR segue sem desfecho; **6 = NÃO consegui consultar** → estado DESCONHECIDO, confirme com `gh pr view <nº>` **antes** de avisar (reportar "não mergeou" num 6 é falso negativo). A janela conta **vigília**, não relógio de parede → watcher vivo além dos N min nominais é esperado, não travamento. Tabela de exit codes e o caso que a originou: cabeçalho do `scripts/pr-watch.sh`.
...
**Uma sessão Claude por working tree.** NUNCA 2 sessões no diretório principal (`/Users/lucassardenberg/Projetos/afiacao`) — o branch-flip vaza entre elas (risco de perda). Worktrees isolam (`bun run wt <branch>`). **Antes de tocar arquivo/função QUENTE:** conferir `origin/main` + `gh pr list` + migrations paralelas (timestamp colidido é o aviso) — **e RE-conferir imediatamente antes do `gh pr create`**: a checagem do início vence numa sessão longa, e o auto-merge fecha PR em minutos (um PR já viveu 6min — 26 arquivos de retrabalho). **Detecção tardia ≠ prevenção:** a rede desceu p/ o `git commit` — no `create` o trabalho JÁ está pronto (2 PRs descartados em 15/08); e commit sem PR é exposição pura. **Conflito de ARQUIVO é só UM eixo:** a tarefa pode já estar ENTREGUE na main sem colidir com arquivo seu — antes de implementar **e** antes de entregar, procure o ARTEFATO (símbolo/entrada que ia criar) com `git fetch && git grep <símbolo> origin/main`; busca por TÍTULO de PR é CEGA a isso (3× em 15/08: 0 hits no título, 1 no grep — quem entrega mergeia sob o tema DELE). **Sincronize antes de MEDIR** — número tirado de base defasada é achado falso. Higiene de RAM/Node na M2 8GB: `wt:status`/`wt:clean`/`wt:reap`/`wt:prune` (numa sessão Claude EU rodo `bun install` ao detectar `node_modules` limpo). MCPs enxutas via `.claude/settings.json` (Serena off por padrão — religar pontual).
