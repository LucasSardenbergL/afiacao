...
- 🟡 **Scaffolds pendentes de sprint próprio**: `submitOrder` offline (UnifiedOrder → PR2, rascunho-local), TouchPickingView **auto-detect mobile** (a confirmação offline de item já está integrada; falta só o roteamento automático mobile vs `/admin/estoque/picking/mobile`), segmentos de cliente / histórico NF-e em schema (hoje localStorage). Recebimento + picking offline já **integrados** (ver §6 item 1). Scaffolds órfãos (`useBulkSelection`, `useOptimisticMutation`, `useKeyboardShortcuts`, `tint-cache`) foram deletados em PR #25 — re-criar quando voltarem a ter consumidor real.
...
- **2 P2s das views SLA (follow-up do codex no #246)** — 3 views sem `security_invoker` (`score_recalc_pending`, `v_oportunidade_economica_hoje`, `visit_score_recalc_pending`) + JWT anon hardcoded no cron `sayerlack-portal-watchdog`; documentados em `supabase/schema-security-report.md`. Restaurar muda semântica em prod → decidir caso a caso (mesma natureza do #233).
