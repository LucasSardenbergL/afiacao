...
**✅ EXECUTADO (2026-07-08):** `db/test-selfservice-pr03-isolamento.sh` **VERDE 25/25** — aplica PR0.1+PR0.2a reais num PG17, espelha a RLS real das bases (omie_products/inventory staff-only, sales_orders own+staff, get_regua_preco gate staff), semeia A(oben,atacante)/B(colacor)/C'(irmão de grupo, oben, outro uid)/S(staff), e sob o GUC de A exige negado/0 em toda superfície de B/C'/cru. Cobre os 5 vetores do plano (Step 1 matriz, Step 2 contaminação staff, Step 3 anti-documento, Step 4 default privileges anon, Step 5 falsificação F1/F2/F3 com dente). Não é migration — é regressão executável (nada a aplicar no SQL Editor). Codex xhigh: _(review em andamento — incorporar antes do merge)_.
