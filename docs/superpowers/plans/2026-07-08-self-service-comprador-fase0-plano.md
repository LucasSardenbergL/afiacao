...
**✅ EXECUTADO + APLICADO EM PROD (2026-07-08; psql-ro confirmou pós-apply: resta só a policy `Staff can manage products`, ALL):** migration `20260708164211_selfservice_pr00_fechar_base_crua.sql` — **só o DROP** da policy `USING(true)` (Codex acatou a simplificação: staff lê pela policy `ALL` existente, sem criar policy SELECT redundante). Prova `db/test-selfservice-pr00-base-crua.sh` **VERDE 13/13** (SET ROLE + GUC + falsificação com dente, incl. embed/join). Codex gpt-5.5 xhigh incorporado. **Achados do pré-voo escalados (fora do escopo do PR0.0):** (a) 5 views `security_invoker=off` anon-readable → **✅ RESOLVIDO por #1246** (`20260708190000_fechar_views_invoker_off_p0.sql`: `security_invoker=on` na cadeia + `REVOKE SELECT FROM anon,PUBLIC`; prova PG17 25 asserts + Codex; psql-ro 2026-07-08 confirma `anon_sel=f`. O chip `task_0beb0615` foi deletado, mas o trabalho foi feito por outro caminho); (b) `omie_payload`/`omie_response` → PR0.0-bis, classificado **P1 obrigatório antes de self-service amplo** (Codex); (c) RPC SECDEF `get_tint_prices` (batch) sem gate staff → **CONFIRMADO leak** (retorna `custoBase`/`custoCorantes`, `auth_exec=t`; a singular `get_tint_price` já gateia por `v_is_staff`) → chip `task_5d3a2d9a`. Domínio Tintométrico, fora do self-service.
...
**✅ EXECUTADO + APLICADO EM PROD (2026-07-08; psql-ro confirmou: tabela + gate + flag='false' + RLS on + 3 policies [sem `customer_select`] + trigger anti-forje):** migration `20260708202033_selfservice_pr01_allowlist_gate.sql` + prova `db/test-selfservice-pr01-gate.sh` **VERDE 21/21 (pós-Codex)** (gate fail-closed com cada default OFF isolado, incl. `is_employee` P0#2; RLS de IUD → 42501; anti-forje; grants anon/authenticated; falsificação `DEFAULT true` + remover `is_employee IS FALSE`, com dente). Melhorias sobre o esboço: grants explícitos (`GRANT SELECT/IUD … TO authenticated`, RLS como barreira) + `DROP POLICY IF EXISTS` (idempotência). **Step 4 (front) DIFERIDO → PR0.2:** `useSelfServiceStatus`/`RequireSelfService` sem consumidor hoje = deadcode (knip) + a RPC nova não está nos types gerados até o apply; entram quando a 1ª tela consumir.
...
**✅ EXECUTADO (2026-07-08):** `db/test-selfservice-pr03-isolamento.sh` **VERDE 30/30** — aplica PR0.1+PR0.2a reais num PG17, espelha a RLS real das bases (omie_products/inventory staff-only, sales_orders own+staff, get_regua_preco gate staff), semeia A(oben,atacante)/B(colacor)/C'(irmão de grupo, oben, outro uid)/S(staff), e sob o GUC de A exige negado/0 em toda superfície de B/C'/cru. Cobre os 5 vetores (Step 1 matriz, Step 2 contaminação staff, Step 3 anti-documento, Step 4 default privileges anon, Step 5 falsificação com dente). Não é migration — é regressão executável (nada a aplicar no SQL Editor).

**Codex xhigh incorporado (DONE_WITH_CONCERNS):** (a) teatro corrigido — `S1c` agora `IS DISTINCT FROM` (NULL-safe), `S4c` filtra `table_schema='public'`; (b) falsificação ampliada — **F4** sabota o filtro central `customer_user_id=auth.uid()` de meus_pedidos (A vê pedido de C'), **F5** concede anon (S4b vermelho); (c) **Step 3b** prova anti-grupo por `pg_depend` (dependência real, não só texto) + regex ampliado (cpf/tax_id/omie_clientes/alias); (d) **lacuna do payload próprio tornada EXPLÍCITA** (⚠️ rastreado): a superfície self-service não projeta payload, mas a `sales_orders` crua ainda deixa o cliente ler o omie_payload do PRÓPRIO pedido → é o PR0.0-bis (não vazamento cross-cliente, sem custo/margem).

**Authz contract (insight P1 do Codex — inventário de superfície):** rodei a due-diligence via psql-ro (funções SECDEF exec-por-authenticated que leem custo/preço) e **achei 2 leaks reais fora da miniatura**: `get_tint_prices` (custo por fórmula → chip `task_5d3a2d9a`) e `fin_estimar_estoque_omie` (valor do estoque a custo → chip `task_61aeb818`). `medir_abaixo_piso_tier` = falso-positivo (gate `pode_ver_carteira_completa`). **Recomendação:** institucionalizar esse inventário como auditoria contínua (allowlist de objetos expostos a authenticated/anon que tocam tabelas de custo/preço/grupo, falhando em qualquer objeto novo fora dela) — a rede que pega o próximo leak que a miniatura PG17 não vê.
