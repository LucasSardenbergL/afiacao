...
**✅ EXECUTADO (2026-07-08):** migration `20260708202033_selfservice_pr01_allowlist_gate.sql` + prova `db/test-selfservice-pr01-gate.sh` **VERDE 17/17** (gate fail-closed com cada default OFF isolado, incl. `is_employee` P0#2; RLS de IUD → 42501; anti-forje; grants anon/authenticated; falsificação `DEFAULT true` + remover `is_employee IS FALSE`, com dente). Melhorias sobre o esboço: grants explícitos (`GRANT SELECT/IUD … TO authenticated`, RLS como barreira) + `DROP POLICY IF EXISTS` (idempotência). **Step 4 (front) DIFERIDO → PR0.2:** `useSelfServiceStatus`/`RequireSelfService` sem consumidor hoje = deadcode (knip) + a RPC nova não está nos types gerados até o apply; entram quando a 1ª tela consumir.
