...
> ⚠️ **RECONCILIAÇÃO com a persistência de extrações ([PR #802](https://github.com/LucasSardenbergL/afiacao/pull/802), já na main):** a aprovação agora também precisa **remover o rascunho** (`kb_extraction_drafts`) do documento. Como a aprovação passa a ser a RPC `aprovar_versao_boletim`, a RPC faz `DELETE FROM kb_extraction_drafts WHERE document_id = p_document_id` **na mesma transação** (transacional > o DELETE best-effort que o #802 faz hoje no front). Na Task A4, o `deleteDraft` best-effort do front é **removido** (a RPC cuida); a invalidação de `['kb-extraction-drafts']` **permanece** (pra a hidratação da fila refrescar pós-aprovação).
...
BEGIN
  ...
  -- 3) RECONCILIAÇÃO com #802: o rascunho cumpriu seu papel → remove na MESMA transação
  -- (substitui o DELETE best-effort do front; DEFINER bypassa a RLS master-only de drafts).
  DELETE FROM public.kb_extraction_drafts WHERE document_id = p_document_id;
