...
import { montarTermosBusca, refinarCandidatos } from '../code-normalize';
...
describe('refinarCandidatos', () => {
  ...
  it('NÃO casa por substring de outro código (catalisador citado)', () => {
    // descrição tem só o catalisador FC.6952, não o verniz buscado
  ...
  it('casa a variante separador-espaço (tingidor) via normalização do extrator', () => {
    // extrairCodigosSayerlack normaliza "TEH 3505.211FG" → "TEH.3505.211FG"
...
export interface CandidatoRefinado extends SkuCandidato {
  match: 'exato' | 'fraco';      // 'exato' = código do boletim ∈ tokens da descrição
  ambiguo: boolean;              // descrição tem >1 código distinto → exige escolha humana
  codigosNaDescricao: string[];
}

/** Termos LIKE pro pré-filtro SQL: o código inteiro + o miolo numérico (3-4 díg) estável,
 *  que casa a descrição mesmo quando o separador difere (espaço↔ponto). */
...
/** Refina candidatos brutos do pré-filtro: match-exato-por-token (reusa o extrator
 *  Sayerlack, que já normaliza espaço→ponto) + marca ambiguidade. Decisão Codex:
 *  só 'exato' E não-ambíguo é auto-confirmável; o resto é triagem humana. */
