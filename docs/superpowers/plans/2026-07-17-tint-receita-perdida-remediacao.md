...
**DESFECHO (2026-07-18, Codex xhigh no design):** implementada como **`expected_item_count`** (migration `20260718170000` + edge) com escopo REDUZIDO ao **transporte edge→banco** — o critério (b) foi **REJEITADO pelo Codex e movido pra Fase 1d**: o CONECTOR (`connector/sayersync/pg.go`, shapes flat e child) OMITE slot com corante vazio/qtd inválida ANTES do POST, então `expected` 0/ausente não distingue "base pura confirmada" de "tudo filtrado por corrupção"; ausência de `itens` no payload vira NULL (nunca 0); placeholders barram; id do header pré-gerado na edge (associação header→itens não-posicional); erro carrega `header_sync_run_id`. Provado em `db/test-tint-promote.sh` C24-C29 + falsificação F1c. O guard (c) da v3 segue VERBATIM.

## Fase 1d (NOVA — resíduo da 1c, pós-Codex) — fronteira do CONECTOR + base pura explícita

**Por quê existe:** o Codex (xhigh, 2026-07-18) provou que a fronteira REAL do fail-open de receita parcial é o **conector Go**, ANTES da edge: ele filtra silenciosamente slot com corante vazio ou qtd não-parseável/≤0 (`pg.go` flat `:262` e child `:317`) — fonte manda `{AX=10, VM=0}` → payload chega "limpo e íntegro" (`expected=1`, count=1) → NENHUM guard do banco vê o inválido → promove parcial `{AX:10}`. Furo PRÉ-EXISTENTE (a v3/1c não pioram nem fecham).

**Escopo:**
- Conector: **preservar** o slot inválido no payload (id + qtd crua + flag) OU **rejeitar a fórmula inteira** antes do POST — decisão de design com Codex; deploy do conector é on-prem (logística própria do founder).
- Sinal semântico **`is_base_pura`** vindo da FONTE (o conector sabe distinguir fórmula sem corante de fórmula filtrada) → só então a transição legítima p/ base pura limpa receita (`is_base_pura=true AND expected=0 AND COUNT=0`), com falsificação própria do gate semântico.
- Testes de CONTRATO: Go→payload e payload→edge p/ `{válido+inválido}`, `{todos inválidos}`, `itens` ausente, placeholders; + teste que fixa o `410/TINT_IMPORT_RETIRED` do tint-import (remoção exige revisão money-path).
- Follow-up menor: `source_item_index` único por header se um dia houver retry parcial de itens (hoje não há — `COUNT>expected` = bug e barra).

**DESFECHO (2026-07-20):** implementada com a opção **PRESERVAR** — o conector virou transporte FIEL (a fronteira de decisão é o BANCO, lição "preflight de cliente não é fronteira"; rejeitar no balcão mataria o sinal central em `tint_sync_errors` e cada regra nova custaria redeploy on-prem). `connector/sayersync`: slot com corante presente + qtd inválida → item emitido com qtd crua/`null` (flat e child); órfão emitido; slot flat livre segue omitido (senão C29 barraria ~100% do catálogo); `is_base_pura=true` derivado SÓ com fonte confiável (flat: 6 slots livres + 12 flat cols; child: pk resolvida + 0 linhas na filha); child com `formula_pk` não-resolvida manda `itens` AUSENTE (matou o `itens=[]` mentiroso que, com is_base_pura ingênuo, limparia o catálogo inteiro); hash-cache com append CONDICIONAL (goldens fixados — fórmulas normais não re-enviam; bases puras re-enviam 1× levando a declaração). Edge: builders puros `staging-rows.ts` (só literal `true` entra) + `staging-rows_test.ts`/`retired_test.ts` (Deno `--no-remote`; o 410 fixado). Banco v5 (`20260722113000`): tríade limpa (contador `receitas_limpas`), contradição barra, **C15a ENDURECIDO** (vazio sem declaração barra até em chave nova — fatos: 0 bases puras no fluxo SL em 1 mês, 0 headers `expected=0`, 0 erros de promoção em 7d) e **cap anti-limpeza-em-massa 50/run** (defesa contra filha esvaziada na origem). Prova: `fase1d_contrato_test.go` (3 sabotagens → 11 vermelhos dirigidos), `db/test-tint-promote.sh` C30-C33 + 3 falsificações F1d + restauração (40 cenários verdes). Base pura promovida NÃO vende (preço RPC NULL, fail-closed) — destravar é fase futura. Deploy: migration → edge → binário on-prem, NA MESMA JANELA (mesmo dia). **Challenge Codex xhigh no diff (2026-07-20) REPROVOU a v1 com 1 P0 + 4 P1 — todos acatados ou calibrados com racional no PR #1474:** cap virou ACUMULADO 24h/loja (a catraca 50+2 morreu — C34+F1d-4); child extrai pai+filha na MESMA tx REPEATABLE READ (fotografia vazia não declara mais base pura); ramo [1d-E] fecha as 3 parciais mascaradas (C35/C36/C29); `x-agent-version` no rastro do run (estado misto auditável — o furo do conector só fecha DE FATO com o binário novo no balcão); preflight por md5 exato. Follow-ups: `source_item_index`, `header_ready` por fórmula, property test do hash.
...
**DESFECHO (2026-07-19):** implementada. O simulador busca na CANÔNICA (`v_tint_formula_canonica`, mesma fonte do balcão — as 2 gerações não aparecem mais duplicadas) e precifica via `get_tint_prices` batch + `precoSimulador` (`src/lib/tint/simulador-preco.ts` — wrapper de `selectTintPrice`, paridade com `selectAltPrice` amarrada por teste). Sem preço → motivo honesto por caso (base sem custo/inativa · corante sem custo · receita VAZIA — distinta via `tem_receita` · motor indisponível), nenhum número exibido (o balcão bloqueia todas as fontes). Bônus de honestidade na seção 1: `tint_skus.imposto_pct/margem_pct` provados ÓRFÃOS (o promote lê os percentuais do STAGING SayerSystem, não de `tint_skus`) — coluna renomeada "Preço Venda"→"Referência" + nota na UI. `useOmieProductMap` (insumo do motor paralelo) removido.
...
**GATE DE PRÉ-CONDIÇÕES (2026-07-20 — a sessão PAROU sem implementar, como o plano manda):**
- ✅ **Fase 1d COMPLETA nas 3 peças → o bloqueio CAIU (2026-07-21).** Confirmado por evidência positiva via psql-ro: (a) migration `20260722113000` aplicada em prod (`is_base_pura`, `receitas_limpas` e o cap acumulado de 24h presentes no `pg_get_functiondef` de `tint_promote_sync_run`); (b) edge deployada; (c) binário do conector **0.2.0 rodando** — 664 runs nas 24h até 2026-07-21 20:31 UTC carregam `tint_sync_runs.metadata.agent_version='0.2.0'` (os 85 sem a tag são anteriores às 11:31 UTC, a transição). Uma v6 (`20260726120000`, só diagnóstico do log) entrou por cima.
- ✅ Fase 3 VIVA em prod: `tint_gate_revalida` + `tint_ultimo_preco_cliente` conferidas via psql-ro; edge `omie-vendas-sync` re-deployado verbatim em 20/07 (commit `f25fbe38` do bot nomeia a função).
- ✅ Guard 4 v3 + 1c + canônica vivos (marcadores conferidos no functiondef/reloptions de prod: `expected_item_count`, `_eu_incompleta`, `receita corrompida`, `security_invoker=on`, `preco_csv_legado`).
- 🔎 **Achado que muda o desenho do watchdog: a desativação JÁ é mecanismo vivo.** `tint_apply_keys_snapshot` (chave-fonte de 4 partes `cor|produto|base|personalizada`, CEGA a geração/embalagem; guarda de blast radius >20%; **nunca reativa**) já desativou 16.110 SAYERLACK + 45 SL (576 em 20/07, timestamp único 10:23 UTC — varredura automática de chaves que saíram da fonte). O **único reativador** é o upsert do `tint_promote_sync_run` (`ON CONFLICT` na unique key **com `subcolecao_id`** + `desativada_em = NULL`) → só ressuscita a geração que a FONTE mandar; o cenário do watchdog é a fonte voltar a mandar subcoleção `1`/SAYERLACK. A guarda de blast radius conta chaves DISTINTAS e as gêmeas compartilham chave → a desativação em massa da Fase 5 **não** interage com ela. As 4 cores exclusivas ACR MAX seguiam ativas em 20/07; a medição das 12 combinações (17/07) já está STALE — o catálogo se move diariamente, re-medir na implementação é OBRIGATÓRIO.
- ⚖️ **DECISÃO do founder (2026-07-20): `preco_csv_legado` = opção B — o CSV sobrevive à desativação.** Na Fase 5, a expressão do CSV na canônica passa a ler a geração antiga MESMO desativada (relaxa só o `desativada_em IS NULL` do max do `preco_csv_legado`, nada além), PRESERVANDO o filtro de proveniência não-SL do #1469 (as duas mudanças compõem). Raio medido: 464.855 ativas SAYERLACK = 100% com CSV; SL = 0. Implementar POR CIMA da versão do #1469 (pré-flight `pg_get_viewdef` de prod), com cenário novo em `db/test-tint-canonica.sh` + falsificação + `WITH (security_invoker=on)` repetido no replace.

**DESFECHO (2026-07-22, PR #1549 mergeado + APLICADO em prod):** implementada e **validada em produção via psql-ro**. Duas correções ao spec original, ambas por medição/challenge: (1) o "relaxar amplo o `desativada_em`" da decisão de 20/07 **não era no-op** — 16.958 linhas da geração '1' já estavam desativadas pela FONTE (via `tint_apply_keys_snapshot`), 1.704 delas em chave com SL ativa, e relaxar amplo lhes daria rótulo indevido + derrubaria o piso do gate. Daí o **carimbo `desativada_motivo='fase5_geracao_legada'`** (decisão do founder 21/07): o relaxamento lê SÓ o que a Fase 5 desativou. (2) **Challenge Codex xhigh REPROVOU a v1** (1 P0 + 7 P1): sem `BEGIN/COMMIT` explícito o UPDATE de 463.995 linhas commitaria antes do guard falhar (P0); `ADD COLUMN` em transação curta separada (ACCESS EXCLUSIVE); `CHECK (desativada_em IS NOT NULL OR desativada_motivo IS NULL)`; a expressão do piso virou `GREATEST(csv, COALESCE(max_ativo, csv))` com o CSV num LATERAL (I1/I2 por CONSTRUÇÃO). **Migration em 2 blocos** (DDL curto + BEGIN/COMMIT de dados+view) com 6 guards fail-closed. Prova: `db/test-tint-fase5-desativacao.sh` (37 asserts, EXIT=0, 4 falsificações + zona W que falsifica o validador). **Medição pós-apply (psql-ro 2026-07-22): 463.995 desativadas+carimbadas · 12 preservadas (as 4 ACR MAX × 3 embalagens, todas servindo na canônica) · I1/I2 valem · soma_csv=soma_piso=553953.50 IDÊNTICAS ao baseline pré-apply ⇒ NO-OP no preço, MEDIDO.** O PG17 pegou 2 bugs late-bound (`min`/`max` em uuid). ⚠️ Idempotente: re-rodar o BLOCO 2 aborta limpo em "alvo VAZIO" (as 12 restantes não têm gêmea SL válida). **Follow-ups 5b registrados no PR:** limpar carimbo no `tint_promote_sync_run` na reativação (hoje o CHECK faz FALHAR — fail-closed deliberado); watchdog "tombstone Fase 5 sem SL ativa válida" (é o cenário K10 do harness = P1-7 do Codex); snapshot reclassificar tombstone de chave que a fonte retirou depois; aposentar `import_tint_formulas` (órfã, SECURITY DEFINER, escreve preço).
