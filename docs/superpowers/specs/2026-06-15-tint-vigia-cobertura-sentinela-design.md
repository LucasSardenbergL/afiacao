...
- **Não é falso-positivo de importação:** o item divergente tinha `age_seconds ≈ 78 dias` (produto antigo) — a tolerância de 30h fez seu papel. Gatilho real = reclassificação de `familia` / reativação / perda da marca sobre produto **existente**, não base recém-importada. O alerta guarda só `age_seconds`, não o código do SKU (candidato a melhoria: anexar o código, como faz `vendas_familia_ausente`). **✅ FEITO (#1245)** — ver "Implementado" abaixo.
...
- **Matar o ruído (opcional):** Follow-up item 3 — acoplar `tint_marcar_bases_mixmachine()` ao fim de `omie-sync-metadados` (correção na fonte) fecha a janela antes de o watchdog alertar. Alternativa mais fraca: subir o cron 132 para `*/30`. **✅ FEITO (#1242)** — ver "Implementado" abaixo.

## Implementado — follow-ups de ruído (2026-07-08)

Os dois caminhos de "matar o ruído" da seção anterior foram entregues (ambos no ar + validados em PROD):

1. **Correção na FONTE (#1242 · edge).** `omie-sync-metadados` chama `tint_marcar_bases_mixmachine()` ao fim do sync da conta que grava produtos `oben` (label `'oben'`), fechando a janela de drift a cada ciclo (~2h) — bem antes das 30h do vigia. `try/catch` isolado (falha na marcação **não** derruba o sync); o cron 132 segue como rede de segurança. Idempotente/aditiva, nunca desmarca. `deno check`: 3 erros de tipo **pré-existentes** (supabase-js sem generics → `.upsert` vira `never`), **0 novos**. Resolve o **item 3 do Follow-up** (raiz da corrida) pela via da correção-na-fonte.
2. **Código no e-mail (#1245 · migration `20260708210000`).** Quando o Check A dispara, o e-mail anexa a lista dos divergentes (código + descrição + motivo: *sem is_tintometric* / *tint_type na aba errada*) via `_tint_cobertura_bases_lista_email(50)`, espelhando o `vendas_familia_ausente`. **Escopo mínimo anti-cascata:** NÃO toca `_data_health_compute` (arquivo que reverteu 5×) nem `fin_sync_heartbeat`; só a função nova + 1 ramo no `data_health_watchdog`. ⚠️ O corpo do watchdog partiu da **def VIVA (17 sources)**, não da migration `20260615130000` do repo (14 sources) — migrations posteriores (custos_proxy_conf_alta/custos_product_cost_revivido/pedidos_compra_sync) promoveram +3 ao push direto em prod; partir do repo teria revertido esses 3 (6ª cascata). Prova PG17 `db/test-tint-cobertura-lista-email.sh` (21 asserts + falsificação: remover o ramo tint → lista some do e-mail → vermelho). Validado em PROD via psql-ro (fn existe + watchdog referencia a lista + append família/17 sources intactos).

**Efeito líquido:** com a marca aplicada na fonte a cada sync, o drift raramente alcança 30h → o alerta quase não dispara; nas exceções (sync parado +30h), o e-mail já identifica o produto. O ruído auto-curável documentado acima (2× em 23 dias) deve cessar.
