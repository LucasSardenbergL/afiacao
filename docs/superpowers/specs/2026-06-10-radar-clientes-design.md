...
- **Dois modos de ataque (presets de 1 clique — decisão do founder, 2026-06-11):** "Novas do lote" (recém-abertas, abordar antes do concorrente) e **"Estabelecidas"** (ex.: >5 anos de `data_abertura` — empresas que sobreviveram e já consomem insumo de alguém; ordenáveis por porte/capital social). O dado já cobre os dois por construção: o dump da RFB é o cadastro COMPLETO de ativas (a idade é coluna, não limitação). Sinais de "saúde" disponíveis e seus limites: situação ativa + idade + porte (ME/EPP/demais) + capital social — a RFB não dá faturamento real (mesmo limite do Radar do Omie).
