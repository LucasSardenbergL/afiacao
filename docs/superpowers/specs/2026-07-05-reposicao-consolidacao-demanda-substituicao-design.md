...
- ⏳ Recompute refina 24/30→23/28 (auto, sem 'segurado').
- ✅ **Frente C (UX)**: botão "Consolidar demanda em outro SKU" no `SkuDetailSheet` (aba Ajuste manual) → `ConsolidarDemandaDialog` chama a RPC `consolidar_demanda_sku`; erros `ZR001-005` traduzidos (`src/lib/reposicao/consolidacao.ts`, teste vitest 8/8). typecheck/lint limpos (erros pré-existentes de `src/lib/mcp/*` = pacote `@lovable.dev/mcp-js` faltando, alheios a esta entrega).
