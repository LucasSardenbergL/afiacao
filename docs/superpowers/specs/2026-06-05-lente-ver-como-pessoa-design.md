...
## 14. Limitações conhecidas da Fase 1 (review final, 2026-06-06)

Fechadas no review final do codex; documentadas aqui pra não virarem "bug" depois. Detalhe e racional em `docs/superpowers/plans/2026-06-06-lente-fase-a-correcoes-review-final.md`.

- **`/meu-dia` não vira o dashboard da Farmer (Fase 2).** `CommercialDashboard`/`useMyCommercialRole` decidem Master vs Commercial dashboard pelo cargo **real** do master → na lente segue mostrando o `MasterDashboard`. Gap de **fidelidade** (a lente não reproduz o `/meu-dia` da Farmer), **não dano**. É uma tela interna não-migrada → entra na Fase 2 (migrar o gating de `useAuth` real → `display*` onde for `display`/`read-enabled`).
- **`displayCommercialRole` é o tipo legado de 4 valores (edge).** `CommercialRole` = `operacional/gerencial/estrategico/super_admin`; o banco tem também `farmer/hunter/closer/master`. `inferPersona` só casa os 4 no switch (passo 4), MAS o passo 2 (**department**) vem antes e resolve as personas reais (as Farmers têm `department='vendas'` → `vendedor`). Um alvo com cargo novo-estilo **e sem** department cairia em heurística/`geral` (degradação honesta). Ampliar o tipo rippla pra `useCommercialRole` e outros consumidores → fora de escopo desta fase.
- **Timing de entrada/troca/saída da lente (corrigido na re-verificação do codex).** As RPCs de auditoria (`log_impersonation_start`/`end_impersonation`) rodam num client **sem guard** (`supabaseUnguarded`, `client.ts`) — são bookkeeping legítimo do master e não podem ser bloqueadas pelo guard de RPC. Com isso o guard **permanece ativo** durante toda a entrada, a troca direta de alvo (A→B) e a saída: no `stop`, o guard só desliga + limpa o `target` DEPOIS do `await end_impersonation`, fechando a janela em que um flush offline escreveria como master. (Detalhe e os outros 2 achados do codex — RPC de leitura PT na allowlist; `fetch` cru money-path no `DispararAgoraButton` — em `docs/superpowers/plans/2026-06-06-lente-fase-a-correcoes-review-final.md`.)
