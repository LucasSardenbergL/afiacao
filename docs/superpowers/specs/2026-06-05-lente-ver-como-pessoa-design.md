...
- **Timing de saída da lente (aceito por design).** Em `stopImpersonation`, o `setLensActive(false)` roda ANTES do `await end_impersonation` — **obrigatório**, senão o guard de RPC (read-only) bloquearia o próprio `end_impersonation` (RPC mutante). A janela curta (guard off + UI ainda na lente durante o await) não carrega risco: é o caminho "Sair", nenhuma escrita do usuário é iniciada ali. No reload, o effect defensivo re-sincroniza o guard; o flash de "master real" no 1º render é leitura.
