...
6. **Leitura caso a caso dos 16 do estrato (founder rodou a query de detalhe, 2026-06-10):** os 11 expirados são ~2 decisões reais procrastinadas ~5-6 dias cada (mesma necessidade re-gerada idêntica — #301/#306 com o MESMO valor R$ 8.709 em dias consecutivos — até a aprovação do #324 em 02/06); os valores R$ 8–10k do estrato são artefato do acúmulo durante a espera (no regime novo, compra ao cruzar 3k e o monte não se forma). Os 2 vetos têm motivo registrado: #341 "Lead time errado de alguns itens" (= **discordância de DADO**, não de lógica — vira regra do piloto §4.7) e #355 "Rejeitado inline no Cockpit". O overlap 18>16 da agregação: #341/#355 são "aprovado e depois cancelado" (contam nas duas categorias). Múltiplos pedidos Sayerlack/dia por **grupo** (03/06: 3 pedidos de R$ 10.1k/6.8k/7.2k) → motivou o delta por grupo (§4.2.7). **Taxa de veto projetada pelo histórico: 0% (semanas 07 e 12/05) a ~40% (semana turbulenta de 04/06)** → o piloto é genuinamente decisivo, não formalidade.
...
7. **Delta ao vivo ≤ `delta_max`** (config nova, default 0.30): existe pedido anterior do mesmo **(empresa, fornecedor, `COALESCE(grupo_codigo,'')`)** com `criado_em < p.criado_em` **e `criado_em > now() − interval '90 days'`** (referência stale não vale) e evidência de compra real (`omie_pedido_compra_numero IS NOT NULL` OR `status IN ('disparado','concluido_recebido')`), pego o mais recente; `abs(p.valor_total − ant.valor_total) / ant.valor_total ≤ delta_max`. **Sem anterior do MESMO GRUPO → NÃO elegível** (a primeira compra de cada grupo é sempre humana; cada disparo manual semeia o grupo pro futuro — o universo cresce sozinho). Racional do grupo: os dados reais mostram múltiplos pedidos Sayerlack por dia, um por grupo, com valores distintos (03/06: R$ 10.1k, 6.8k, 7.2k) — delta contra o fornecedor inteiro compararia maçã com banana. Computado no tick — não depende da coluna morta `pedido_anterior_valor` e não congela na geração;
...
- **Disciplina do veto (contrato com o founder):** todo veto registra motivo na `justificativa_cancelamento` (já é hábito — #341/#355 comprovam). **Veto por dado errado (ex.: "lead time errado") obriga corrigir a FONTE** (`sku_parametros`/SLA do fornecedor) — senão o motor re-sugere o erro no ciclo seguinte e o veto vira rotina, contaminando a métrica.
- Enquanto o piloto roda, o founder recebe 1 e-mail informativo por auto-aprovação (volume esperado: ~2-4/semana no arranque — o universo por grupo começa estreito e cresce a cada disparo manual que semeia referência).
