...
- ~~Estado de feedback persiste numa tabela leve nova `suggested_action_feedback`~~ → **REVISTO pós-Codex Fase 2 (2026-06-04): ZERO MIGRATION.** Ao mapear o código pra implementar a Fase 2, descobri que **as 3 fontes JÁ têm motor de feedback completo** (a premissa "mix-gap não tem dono" estava errada): tarefa → `useTarefaMutations.concluir/adiar`; rota → `useRegistrarContato` (RPC `registrar_contato_rota`) + componente `OutcomeMenu` pronto; mix-gap → `useMarkMixGapFeedback` (RPC `mark_mixgap_feedback`, optimistic no mesmo `['my-mixgap']` que a fila lê). **O loop fecha reusando os 3 motores, sem tabela nova.** O Codex (challenge, 2026-06-04) bateu o martelo: criar `suggested_action_feedback` agora gera **estado duplicado** (a fonte real continua dizendo que o item existe → conflito de verdade — "pior que não ter"). A tabela só cobriria um "snooze/dispensar de fila pessoal" que é **hipótese, não requisito** no piloto → **adiada** (junto da materialização de `suggested_actions`), construída só se o uso real provar necessidade. O "não é pra agora" do v1 é **estado de sessão (React) + `track('fila.nao_util_agora')`** (fricção medida, não estado persistido) — se aparecer muito no piloto, vira a evidência pra snooze persistente.
- **Payload tipado obrigatório (Codex P1):** o formato comum (`AcaoSugerida`) ganha um **discriminated union** `payload` por fonte — `{kind:'tarefa',tarefaId} | {kind:'rota',customerUserId,dataRota,bucket,valor} | {kind:'mixgap',customerUserId,familia}` — pra cada CTA chamar o motor de origem com os args certos **sem parsear strings** (`entidadeId`/`dedupeKey` deixam de ser dados estruturais). É também o schema forward-compatible da futura `suggested_actions`.
...
Todo item tem **Fazer** (botão primário, abre a execução) + um **outcome por fonte** que escreve no motor de origem (§5, zero migration). Mapeamento (revisto pós-Codex Fase 2):

- **Tarefa** (prazo): `Fazer` + **`Concluir`** (1 toque → `useTarefaMutations.concluir(id,'manual')`) + **`Abrir tarefa`** (link `/tarefas`). **NÃO** oferecer `Adiar` na fila — `adiar()` muda o **prazo de negócio** (founder vê + alimenta a cobrança/escalonamento); adiar de verdade fica na tela dedicada. Vendedora não dispensa ordem do chefe.
- **Rota** (esperado): `Fazer/Ligar` (`tel:`) + plugar o **`OutcomeMenu`** pronto (`Fechou pedido`/`Falou, vai pensar`/`Não atendeu`/`Não quer ser ligado` → `registrar_contato_rota`, com undo de 5min). Precisa de `dataRota`/`bucket`/`valor` (vêm do `payload.rota`).
- **Mix-gap** (esperado): `Fazer/Montar pedido` + **`Ofertei`/`Vendi`/`Sem fit`** → `markFeedback({customerUserId,familia,status:'ofertado'|'convertido'|'recusado'})` (optimistic; sai da fila na hora).
- **Genérico (qualquer fonte):** **`Não é pra agora`** → esconde na **sessão** (estado React, não DB) + `track('fila.nao_util_agora',{fonte,dedupeKey})`. Válvula de escape medida, sem estado persistido (Codex: forçar outcome real, medir a fricção).

"Piscar" (item sai no refetch após invalidate, em rota/tarefa) é aceitável no v1 (Codex P2); mix-gap já é instantâneo (optimistic nativo). É este loop que gera as métricas do piloto **e** o sinal pra refinar.
...
- ações **mostradas / aceitas (Fazer) / outcome registrado / "não é pra agora"**;
  - ⚠️ **`fila.exibida` 1×/dia/sessão, NUNCA por render** (Codex P2 — senão mede re-render, não exposição). Guardado em `sessionStorage` por `spBusinessDate`; helper puro testável.
  - cada CTA/outcome loga um evento **de fila** próprio (`fila.acao_fazer`/`fila.outcome`/`fila.nao_util_agora`) **além** do que o motor loga — pra atribuir "% veio da fila".
...
2. ~~Migration da tabela de feedback~~ → **RESOLVIDO pós-Codex Fase 2: zero migration** (reuso dos 3 motores; ver §5/§7). Sem ritual `lovable-db-operator` na Fase 2.
...
### 2ª consulta — Codex challenge da Fase 2 (loop de feedback, 2026-06-04)

Brief: dado que as 3 fontes têm motor de feedback próprio (descoberto no mapeamento), vale a migration `suggested_action_feedback` ou fecho o loop só com os motores (zero migration)? **Veredito: "ok, sem tabela."** Achados incorporados: **(P1)** tabela nova = estado duplicado → conflito de verdade com a fonte; **(P1)** `AcaoSugerida` precisa de `payload` discriminado por fonte (senão parsear `entidadeId`/`dedupeKey` vira gambiarra); **(P1)** não oferecer "Adiar tarefa" na fila (muda prazo de negócio que o founder vê); **(P2)** `fila.mostrada` não pode inflar (1×/dedupeKey/dia, não por render); **(P2)** "dispensar genérico" é lixeira sem semântica → forçar outcome real + medir fricção via `track('fila.nao_util_agora')`; **(P2)** rota "pisca" no refetch (aceitável; se incomodar, pending local por `dedupeKey`, não tabela); mix-gap já alinhado (optimistic no mesmo `['my-mixgap']`). Escopo mínimo correto do v1: enriquecer `AcaoSugerida` com payload, `Concluir` em tarefa, `OutcomeMenu` em rota, `markFeedback` em mix-gap, instrumentar `fila.*`. **Adia `suggested_action_feedback` até o piloto provar necessidade real de snooze pessoal.**
