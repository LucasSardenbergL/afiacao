...
- [✅] **B2b** PROVADO + revisado (PR a abrir). Trigger UPSERT `AFTER INSERT OR UPDATE OF owner_user_id ON carteira_assignments` → provisiona+reconcilia `farmer_client_scores`. **Deploy split:** trigger ungated (futuro); conserto one-time (= B1) gated. Migration `20260619120000` + harness PG17 15/15. Per-row dispara N× num reimport (aceitável, op única).
- [ ] **B2c (recalcOne grava farmer_id) — ELEVADA por /codex #1.** `scoring-recalc-client:438` grava o `farmer_id` do payload da fila → pode **re-stalar** um cliente ATIVO reatribuído mid-flight (fila com dono do enqueue, antigo) até a passada noturna do `scoring-recalc-batch` (resolve dono via ownerMap `:102`) curar em ≤24h. Inativos (1425) não têm fila → imunes. Fix companheiro do B2b: recalcOne resolver dono em carteira_assignments no write (espelhar o enqueue). EDGE, deploy/teste à parte. **Recomendado como próximo.**
...
- [ ] **P1** calls filter customer-only (`recalcOne` remove `.eq('farmer_id',…)`) — latente (0 impacto hoje), PR testado à parte. (Distinto do B2c: aqui é o FILTRO de leitura das calls; lá é o WRITE do farmer_id.)
- [ ] **Leitores** (`generate-tactical-plan` edge + `useTacticalPlan`/`useFarmerCopilot`/`useFarmerTacticalPlan`/`useFarmerExperiments` + leitor futuro de history/priority_log por farmer) filtram score por `farmer_id=user.id` → vazio pra cliente de outro dono. Gatear por posse ao vivo (view/RPC). Money-path-adjacente (Codex), frente separada.
