...
- [✅] **DELETE-policy CONSTRUÍDA** (PR delete-policy, migration `20260622120000`). `CONSTRAINT TRIGGER ... DEFERRABLE INITIALLY DEFERRED` + `NOT EXISTS` guard em `carteira_assignments` → remove o score órfão na remoção DEFINITIVA; delete+insert do mesmo cliente na mesma txn (replace) PRESERVA o score (dispara no commit, carteira existe → guard falso). Blindagem (0-impacto hoje: nada deleta carteira_assignments). PG17 8/8 + falsifica deferred E guard. /codex 2026-06-22 recomendou A (build) + o design. Follow-up: monitor `sem_carteira>0` no data-health (defesa em profundidade).
...
- [✅] **Cobertura nos dropdowns — [PR #980](https://github.com/LucasSardenbergL/afiacao/pull/980).** `useFarmerTacticalPlan` + `useFarmerCopilot` passam a usar `ownerIds = lente ? [alvo] : [eu, ...cobertos]` (via `useMyActiveCoverage`, paridade com `useMyCarteiraScores`) → gestor vê clientes cobertos nos dropdowns de plano/copiloto. **Experiments fica OWN-BOOK** de propósito (cobertura transitória não polui grupos A/B de experimento longo). Guard de lente atualizado (coverage-aware + casos: cobertura incluída fora da lente; NÃO vaza na lente). Deploy: Publish frontend.
