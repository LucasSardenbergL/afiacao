...
| corante | `corante.id` (numérico 1..16) — ⚠️ a RECONCILIAÇÃO de prod (12/06) PROVOU: o app tem `id_corante` `1..16` e **0** dos `WP01` da v0.1.6 casavam. A coluna "codigo" do `CORANTES.CSV` É o banco `corante.id` (o export renomeia id→codigo); o banco `corante.codigo`=`WP01` é só descrição. Slots `corante1..6` são FK→`corante.id` → CoranteIdent id→id (slot cru = identidade). v0.1.7 (revertido da v0.1.5) | `3`, `12` |
| cor padrão | `padraocor.codigo` **VERBATIM** — ⚠️ o GABARITO (12/06) provou que o `" - BS"` (Base Solvente, founder) **JÁ VEM DENTRO do codigo** (`"001B - BS"`, `"01 - ACRIL BS"`); o compose+sufixo das v0.1.4/5 DUPLICARIA (`"… - BS - BS"`) → v0.1.6 envia verbatim, **SEM TRIM** (a era-CSV preservou espaços nas pontas — chave byte-a-byte; ex. personcor `"0105 IVE "`). Sem codigo → id cru. | `001B - BS` |
...
**§13.1 — VEREDITO DO GABARITO (12/06, export oficial: 42 CSVs, 485.447 linhas de fórmula):** regra de 3 entre embalagens **PERFEITA** (desvio máx **0,0023 ml** em 1,3 mi de comparações = arredondamento de 4 casas; ~63% das qtds são múltiplos exatos de onça/384 — herança das receitas em oz, mas a proporcionalidade segura); `volume_finalml = volume_embalagem + Σ qtds` em **0/485.447 falhas** (CONFIRMA: volume_final inclui corantes; o conector manda o volume da EMBALAGEM — razões preservadas, fator same-package=1); identidades cruas confirmadas (id_base=91, id_embalagem=1/2/3/38 c/ volumes 810/3240/18000/405 ml, cod_produto="JO05.7796", corante slots no espaço CODIGO 1..16, cor_id/codigo_cor verbatim COM sufixos e espaços DENTRO); preco_final presente no export (fonte = banco de preço ainda não localizado; v0.1.7). Counts cruzados com o 1º ciclo do conector: corantes 14=14 ✓, produto_base_embalagem 220=220 ✓, produtos 20=20 ✓, cores 8.171 c/ fórmula (≤8.174 sincronizadas ✓). **Correções que o gabarito forçou (v0.1.6):** cor_id verbatim (anti dupla-sufixação) · sem trim em identidade · corante=codigo. **Staging precisa de limpeza** (fórmulas v0.1.4/5 com cor_id duplo-sufixado) + **full re-scan** (deletar state.json) antes da reconciliação.

**§13.2 — RECONCILIAÇÃO de prod (12/06, staging × oficial, pós full re-scan v0.1.6):** bases **56/56** · embalagens **4/4** · produtos **20/20** · cores **8.238/8.388** (98%; as ~150 restantes = cores novas da Sayerlack desde o último CSV manual = ganho do sync) · **corantes 0/14 → BUG**: o app oficial tem `id_corante` `1,2,...,16` (a coluna "codigo" do `CORANTES.CSV` = banco `corante.id`), mas a v0.1.6 mandou `corante.codigo`=`WP01..WP88` (só descrição) → **v0.1.7 reverte o corante pro id** (como na v0.1.5; cor/produto/base/embalagem da v0.1.6 ficam). LIÇÃO: identidade só se prova por RECONCILIAÇÃO staging×oficial — o gabarito sozinho enganou (a coluna "codigo" do CSV ≠ a coluna codigo do banco). `bs_duplicado=0` confirmou o fix do " - BS" da v0.1.6.

## 14. BUG DO LOOP DE RE-ENVIO (16/06) — fórmula sem timestamp → full sync permanente

> Diagnóstico fechado + plano do conserto. Registrado também no chip `task_08c4d3fd` (mas chips são efêmeros — **ESTE doc é o registro durável**). Conserto vai pra sessão dedicada.

**Sintoma (prod, via psql-ro):** conector em LOOP — 398+ runs/hora de `formulas` (escalando 1.227→6.195/dia entre 12-16/06), `tint_staging_formulas` com **3,3M linhas** acumuladas, `keys_snapshot` rodou só 1×. Cada full sync das ~485k fórmulas leva ~1h; com `interval=10m`, vira full sync atrás de full sync. (Reinícios com versões diferentes v0.1.3-0.1.7 agravaram a acumulação, mas NÃO são a causa.)

**Causa raiz (confirmada no `raw_data` do staging):** a tabela FORMULA do SayerSystem **NÃO preenche o campo de data** — `data_alteracao` E `data_atualizacao` vêm VAZIOS. A query delta (`connector/sayersync/pg.go:ExtractDelta`, `WHERE <dataCol> > $1 OR <dataCol> IS NULL`) traz TUDO sempre por causa do `OR IS NULL`; o HWM (`state.go`) nunca avança. O raw_data da fórmula só tem CONTEÚDO (`cor_id, id_base, id_embalagem, subcolecao, cod_produto, volume_final_ml, itens[id_corante,qtd_ml,ordem]`) — zero timestamp/versão. Cores (padracor) não sincronizam como entidade separada (`tint_staging_cores_catalogo`=0; vêm embutidas na fórmula).

**Conserto (chip `task_08c4d3fd`, sessão dedicada):** detecção por **HASH de conteúdo** — o conector calcula hash estável por fórmula (cor+base+embalagem+subcoleção+produto+volume+itens ORDENADOS), guarda cache local (~485k×50B≈24MB; arquivo dedicado ou SQLite, escrita atômica como `SaveState`), envia só hash novo/alterado + detecta removidas (pra keys-snapshot). CONFIRMAR se as entidades pequenas (corante/base/embalagem/produto) têm `data_alteracao` preenchido (mantêm delta por timestamp) ou precisam de hash também. TDD nos testes Go → recompilar `GOOS=windows GOARCH=amd64` → redeploy via `update.go` (auto-update, manifest versão+sha256, anti-downgrade) ou reinstalação manual. Verificar: queda drástica de runs/hora em `tint_sync_runs`.

**Fase seguinte (pós conector saudável):** faxina/purge do staging (3,3M + lixo de versões) → full re-scan limpo → reconciliação formal (`tint_run_reconciliation` via tela `TintReconciliation`, divergência ~zero) → flip `integration_mode` `shadow_mode`→`automatic_primary` (`tint_integration_settings`, store M01) → catálogo 100% automático.

**Decisões money-path (eu + Codex, 16/06):** (1) o PREÇO do balcão vem do **OMIE** (RPC `get_tint_price`/`get_tint_prices`, base+corantes), **NÃO do sync** — `precos_base` do SayerSystem nunca foi localizado (outro banco/schema) e o app calcula via Omie. O sync alimenta só o CATÁLOGO; zerar `preco_final_sayersystem` na promoção é inofensivo. (2) Founder gerencia o preço no Omie (com margem ~10%); o app reflete com o delay do sync de produtos Omie (~2h). (3) Aposentar o CSV via código foi DESCARTADO (baixaria ~19k cores onde calc<CSV — decisão comercial, não técnica); o CSV de preço já está aposentado de fato (Omie é a fonte). (4) O catálogo automático vale o conserto: a Sayerlack atualiza fórmulas diariamente.

**Estado em 16/06:** `integration_mode=shadow_mode` (seguro; o flip prematuro do founder foi revertido sem dano — a promoção não chegou a zerar o CSV). Balcão honesto (preço via Omie, Passos 1/2/1b, PR #868) **publicado e no ar**. PRD03657 resgatado (founder corrigiu no Omie → sincronizou: base R$403,50). Paliativo até o conserto: `interval` do conector 10m→24h + garantir 1 instância; CSV manual como ponte pras mudanças diárias.
