...
- **Repo:** `compute-costs-daily` = `'0 7 * * *'` (1×/dia) em [`cron_baseline.sql:41`](../../../supabase/migrations/20260527230000_cron_baseline.sql) e [`crons_timeout_fix.sql:41`](../../../supabase/migrations/20260527170000_crons_timeout_fix.sql). NENHUMA migration de 2h versionada no repo.
- **Registro do cron (`cron.job`):** schedule **`45 */2 * * *`** (a cada 2h), `active=t` → a mitigação FOI aplicada **direto no SQL Editor** de prod (sem arquivo, padrão Lovable). ✅ premissa do "2h" confirmada *no registro do scheduler*.
