...
- **D — UI `Projecao13Card`:** abaixo dos badges existentes, uma linha: "Caixa inicial da projeção: R$Y (de DD/MM) · saldo bancário atual R$Z · Δ R$W" + nota "(Δ esperado se o snapshot é de dias atrás — o caixa se move)". Quando `!disponivel`: "Caixa inicial da projeção indisponível neste snapshot" (parcial/ausente). Recebe `caixaInicialProjecao` + `saldoAtualBanco` (= `totalCC`) + `cohorteCompleta` (= `!parcial`) por props.
