...
**Data:** 2026-07-14
**Status:** design aprovado no chat; **Fase 0 (spike-A) executada em 2026-07-14: GO** (3 critérios provados no portal real — ver §Achados do spike-A)
...
## Achados do spike-A (2026-07-14, portal real, sessão do founder — GO)

Executado via browser na sessão logada do founder (evidência: screenshots na transcrição da sessão). Os 3 critérios do gate passaram — **com upgrade**: o preço aparece ao **selecionar** o item no select2, ANTES de gravar a linha; a captura pode operar sem gravar nada.

1. **URL real:** `http://portal.sayerlack.com.br:9092` (HTTP puro, porta 9092; a raiz e o :443 respondem 403/404 — browser força https na URL sem porta; a edge usa a URL com porta).
2. **Fluxo:** `/order-creation` → "Novo Pedido / Proposta" → cliente select2 `46269 [COLACOR COM] - OBEN COMERCIO LTDA` (auto-preenche: tipo 89, canal 30-DISTRIBUICAO, ANTECIPADO, taxa −2%, FOB, SPMINAS) → "+ Incluir Item" → select2 do item por `sku_portal`.
3. **Colunas da linha** (mapa p/ o scrape): `UN`=L · `Cap Emb`=litros/embalagem (QT 0,8100 · GL 3,2400 — versão base) · `Qtd Fat`=**litros** (arredonda p/ múltiplo da embalagem; digitar 1 aqui no GL vira 0!) · `Qtd UN`=**embalagens (é aqui que se digita 1)** · `Preço Fat`=R$/litro tabela · `Preço UN`=R$/embalagem tabela · `Prz Ent`=dias · `% Desconto` (POR EMBALAGEM: QT 13,8678% ≠ GL 31,0942%) · **`Preço Venda`=R$/embalagem LÍQUIDO — o número que alimenta a tela/motor**.
4. **Prova money-path:** `Preço Venda = Preço UN × (1−desconto)` e `Preço Fat = Preço UN ÷ Cap Emb` fecham nas 2 embalagens; QT `74,4348` ≈ manual de hoje `74,43`; GL `306,4977` ≈ manual `306,50`.
5. **Item inexistente/inativado:** select2 responde **"Nenhum resultado encontrado"** (mensagem `.select2-results__message`) → sinal do `nao_encontrado`.
6. **Limpeza:** X vermelho cancela linha em edição; **lixeira remove linha gravada SEM dialog de confirmação**; "← Voltar" sem "Salvar Proposta" → **Propostas Abertas permanece vazia** (nada persiste; a linha gravada vive só na sessão via `order-creation/form/save-tab-preco-session`).
7. **Zero PO:** "Efetivar Pedido" nunca clicado; Propostas Abertas vazia antes/depois; `purchase_orders_tracking` inalterado (681 / máx `2026-07-15 00:15 UTC` antes e depois).
8. **Bônus p/ Fase 1:** existe botão **"+ Incluir Múltiplos Item"** (inclusão em lote — candidato a varrer os 28 SKUs em poucas interações; explorar no spike-B). Rodapé calcula a data prevista (emissão + Prz Ent úteis).
9. **Risco residual da Fase 1** = o já conhecido do Browserless (flakiness/timeout), não o portal — o fluxo login+select2+gravar já roda em produção na edge de envio.
