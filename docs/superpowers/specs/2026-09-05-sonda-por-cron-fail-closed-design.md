...
2. **materializa** cada closure (`git archive <sha> supabase/functions/<edge> supabase/functions/_shared`) num diretório temporário e gera o import map: locais resolvem sozinhos; todo especificador remoto (`npm:`, `https://`, `jsr:`) mapeado para um stub do **catálogo** (`supabase/harness-sonda-rollback/stubs/`). Especificador fora do catálogo = closure **`INVERIFICAVEL`** (nunca "passou"). Medido (2026-09-05, 3.226 closures): **16 especificadores remotos distintos em 5 famílias** — `@supabase/supabase-js` (8 variantes `npm:`/`esm.sh`), `deno.land/std@*/http/server.ts` (2), `npm:@anthropic-ai/sdk`, `npm:web-push`, `resend` (4) — o catálogo casa por família e versão-agnóstico; qualquer família nova reprova até ganhar stub;
