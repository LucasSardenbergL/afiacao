...
- Vetores fixos para os testes: RFC 4231 #2 = `5bdcc146bf60754e6a042426089575c75a003f089d2739839dec58b964ec3843`; `sonda-de-versao:v1:monthly-report` sob a chave de teste `Jefe` = `04855b66e0237a22cb2039fa2859d597b90adf29ee61ce9cd2f3d03c37e7ce42` (Deno 2.9.2, conferido contra o valor que o Codex calculou de forma independente) — asserido no relé e na alvo.
...
6. O relé também está na allowlist e se atesta pelo mesmo caminho (`OPTIONS` em si mesmo). **A prova histórica do relé é diferente da das alvos** (Codex, rodada 3): para cada closure histórico do relé, o runner executa também o **POST operacional do cron** (`x-cron-secret` + `{alvo, tick}`) e captura o `Request` de saída, exigindo `OPTIONS`, headers exatos, corpo nulo e `redirect: "manual"` — um relé antigo com bug no POST recebido nunca vira candidato aprovado a rollback.
...
1. **Enumeração por ponto fixo** (Codex P1): `F ← fecho da edge no HEAD`; repete { `C ← ∪ sobre cada f ∈ F de git log --follow --name-status -- f` (o `--follow` aceita UM pathspec por vez; dos diffs de rename saem os pares antigo/novo, e ambos entram em `F`; qualquer subprocesso com erro = exit 2); para cada `c ∈ C` calcula o fecho **naquele sha** (parse de imports locais em `git show c:path`, recursivo); `F ← F ∪ (arquivos de todos os fechos)` } até `F` estabilizar. Dependência removida da main (ex.: um `_shared/helper-antigo.ts` que só um `index.ts` velho importava) entra porque aparece no fecho do sha antigo. Identidade do closure = hash dos conteúdos do fecho naquele sha (o cálculo do `sonda:fingerprint`). Medido: ~3,2 k closures (aprox. pré-ponto-fixo) nas 54 edges.
...
3. Executa `deno run --no-remote --import-map=<gerado> runner.ts` com `Deno.serve`/`Deno.env.get`/`globalThis.fetch`/`EdgeRuntime.waitUntil` substituídos ANTES do `import()`. **Contador que suba durante o `import()` = `FALHA`** (IO top-level), mesmo que o import lance; só erro de import com contadores zerados é `INVERIFICAVEL` (Codex P2). O runner instala um **relógio virtual**: `setTimeout`/`setInterval`/`queueMicrotask` e `EdgeRuntime.waitUntil` são interceptados e enfileirados; depois de cada chamada ao handler, executa **toda** tarefa agendada (e as que elas agendam) até a quiescência, com teto explícito de 10 k tarefas (estourar = `FALHA`) — só então lê os contadores. Trabalho agendado antes do `return`, com qualquer atraso, conta (Codex, rodada 3). Chamadas ao handler:
...
O bloco gerado passa a disparar o **relé** por edge (`net.http_post(…/sonda-relay, body {alvo})`, com `x-cron-secret`), que é inócuo em **todo** bundle da alvo. Enquanto o relé não estiver no ar (F1 pendente de deploy), o bloco legado (POST `{"probe":true}` na alvo) só sai com a opção explícita **`--permitir-efeito-legado`**, bloqueada por padrão (Codex, rodada 3: é o único caminho concreto de efeito que sobra, e ele é humano); sem a opção, o gerador imprime o motivo e o one-liner `deploy_sonda_disparar(ARRAY[…])`.
...
**Rodada 3 — veredito `ROLLBACK-TEST: PASSA`, `[P1] Nenhum`** (parecer cru em `scratchpad/codex-challenge-3.out`). P2/P3 e o que fiz:

| achado (rodada 3) | severidade | decisão minha |
|---|---|---|
| rollback do próprio relé não é provado pelo `OPTIONS` (um relé antigo com bug no POST recebido) | P2 | aceito: prova histórica do relé executa o POST do cron e inspeciona o `Request` de saída (§4.3 item 6) |
| drenagem assíncrona finita (`setTimeout(fn, 50)` escapa a dois `setTimeout(0)`) | P2 | aceito: relógio virtual até quiescência com teto (§4.4 passo 3) |
| `git log --follow -- F` não aceita conjunto | P2 | aceito: `--follow` por caminho, pares de rename entram no ponto fixo, erro = exit 2 |
| fallback legado do `sonda:sql` é caminho concreto de efeito | P2 | aceito: `--permitir-efeito-legado`, bloqueado por padrão (§4.7) |
| §7 ainda dizia "por amostra" | P3 | aceito: removido; os 3 conjuntos exatos ficaram nomeados |
| vetor da mensagem real não estava fixado | P3 | aceito: literal em §4.1, conferido no Deno contra o valor dele |
| `authorizeCron` compara com `===` no relé | parcial dele | residual nomeado (§7 vii): padrão do repo, entrega própria |
| dependência remota não reproduzível (limite ii-b) | aceitável como limite operacional | mantido como limite nomeado; a formulação honesta dele ("prova do código local sob o contrato explícito das famílias remotas") entrou no §7 |
| ambiente único (residual vi) | aceitável | mantido; se nascer 2º ambiente, `project_ref` vira vínculo mecânico antes do rollout |
| ordem F1→F4 | sem exposição | confirmado: F2 só ativa as 3 provadas; F4 exige prova antes do `INSERT` |
