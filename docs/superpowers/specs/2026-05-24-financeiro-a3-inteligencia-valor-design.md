...
- **Engine Deno**: `supabase/functions/fin-valor-cockpit/index.ts` — lê `order_items`+`product_costs`+`fin_contas_receber`+`inventory_position`+WACC(A2) pra Oben, monta as células, devolve rankings + recomendações + confiança. **Gate: gestor comercial (`commercial_roles.commercial_role='gestor'`) + master.** Vendedor individual NÃO acessa (não vê margem de todos os clientes). O engine valida o papel via JWT (gestor ou master) antes de responder.
