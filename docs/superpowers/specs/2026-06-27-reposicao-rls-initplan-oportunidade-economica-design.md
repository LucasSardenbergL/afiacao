...
> **STATUS 2026-06-27 — CICLO COMPLETO (aplicada + medida em prod, gate FECHADO):** PR #1106 mergeado, migration **aplicada** no SQL Editor (36/36 policies wrapped, confirmado via `psql-ro`), `schema_migrations` reconciliado + `schema-snapshot.sql` re-dumpado (DR fiel, policies wrapped). **Gate fechado** sob `authenticated` em prod: buffers **495.426 → 11.419** (~43×, = a ordem estrutural sem-RLS), Execution **2.936ms → 880ms** quente, cache frio **>8s → ~1,3s** (lê 89 MB, não 3,8 GB) → **HTTP 500 ELIMINADO**; `Heap Fetches: 0` (sem o 2º gargalo de vis-map que mordeu o tint). Frontend (#1108: degradação honesta + dedupe) mergeado. **Fase 2 (materializar) ADIADA** — os 880ms residuais (estrutural puro do `generate_series`) passaram no gate (warm <1s, cold <3s); gatilhos de reabertura: estrutural quente >~1,5s, cockpit/badge voltar a pesar, ou a view crescer (mais SKUs em promo/aumento). Decisão de escopo do founder (2026-06-27): **faseado por evidência**; Fase 1 fechou o 500, encerrado no verde.
