...
- **Upsert `onConflict(user_id,account)` × `UNIQUE(codigo,account)` — VIVO, e falha RUIDOSA.** Verificado em `omie-analytics-sync/index.ts:812`; as duas UNIQUEs coexistem em prod (`uq_ocam_user_account`, `uq_ocam_codigo_account`). Quando um código migra de user1→user2 o upsert insere (não há conflito em `(user_id,account)`) e bate na `UNIQUE(codigo,account)` da linha antiga → `23505`. **Não corrompe em silêncio: `:813` faz `throw` e derruba o sync.** O comentário do writer (`:803-806`) descreve essa UNIQUE como *proteção* ("barra colisão cross-account"), não como este modo de falha — daí ela passar despercebida em leitura casual. É disparado exatamente pelo cenário do P1-a: mesmo follow-up.
- **P2 (unsafe-integer) — metade fechada.** Lado do **snapshot**: fechado (guard do parser acima). Lado do **pedido**: aberto — `codigo_cliente?: number` chega como número do JSON do Omie (`omie-vendas-sync/index.ts:45,107,129`), então um código `>2^53` já teria sido arredondado antes de qualquer guard. Sem impacto prático (códigos Omie reais são `<10^10`); o guard end-to-end segue follow-up.
