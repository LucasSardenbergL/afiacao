...
> ## ⚠️ Atualização 2026-08-13 — a premissa do §1.2 caiu pela metade
>
> Este spec ficou 3 semanas numa branch órfã (`claude/jolly-wescoff-d36818`, nunca em `origin`).
> Nesse intervalo prod andou. **Re-medido via `psql-ro` em 2026-08-13 20:51 UTC:**
>
> | Tabela | §1.2 dizia (22/07) | Estado real (13/08) |
> |---|---|---|
> | `public.omie_products` | aberta, fecho em branch draft | **FECHADA e aplicada** — `anon` fora do `relacl`, `authenticated=r`, 1 policy `omie_products_select_staff` |
> | `public.product_costs` | aberta, fecho no PR #1520 draft | **ainda aberta** — `anon=arwdDxtm`; #1520 segue OPEN+DRAFT |
>
> O fecho de `omie_products` mergeou no **PR #1558 em 2026-07-23** — um dia depois deste spec —
> pela migration `20260727140000_authz_preco_fecha_omie_products.sql`. A allowlist ficou 3 semanas
> declarando `fechadaPor: null` para uma tabela já fechada: **exatamente o `ANCORA_NAO_DECLARADA`
> do §5.2**, que este documento escreveu para impedir. O desenho não envelheceu — ele previu o
> próprio caso e passou 3 semanas demonstrando-o.
>
> **Onde o desenho PRECISOU mudar** (§3, "Gate de CI"): o plano mandava injetar a Parte C dentro de
> `auditAuthz`. Isso só funcionava enquanto **todas** as entradas tinham `fechadaPor: null`. Com uma
> âncora declarada, todo fixture da Parte A/B — que naturalmente não contém a migration-âncora —
> passa a disparar `ANCORA_AUSENTE` (12 testes pré-existentes quebraram). A Parte C julga *quais
> arquivos existem no repo*; A/B julgam *definições de função*. Foram separadas: `auditGrants` é
> própria, e `auditCompleto` (usada pelo `main()`) compõe as três. O comando de CI continua um só.
>
> Entregue nesta rodada: Tasks 1–3 (allowlist + gate estático + Parte C no `authz:check`).
> Pendente: Tasks 4–7 (audit de prod sob `psql-ro` + harness PG17).
