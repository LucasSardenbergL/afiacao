...
## 13. Fatia 5B — o DROP (2026-07-19). ÉPICO ENCERRADO.

Aplicado depois da 5A estar nas 3 camadas e **verificada por comportamento** em produção — não só por deploy.
...
**Placar final do épico:** Fatia 0 (ledger + backfill) → 1 (rebuild lê o ledger) → 2 (`identity_state`) → 3 + PR-C (leitores; 2 funções de clone deletadas) → 4 (writers → RPC; hotfix #1444 passou o ledger à document-first) → **5A** (2 objetos SQL + últimas leituras) → **5B** (DROP). O que era um espelho de conta única com rótulo falso (`empresa_omie` 100% no DEFAULT) virou três fontes com responsabilidade separada: **membership** no `carteira_membership_ledger` (acumulador), **identidade account-correta** em `omie_customer_account_map`, **aliases fiscais** em `customer_canonical_alias`.
