...
No ramo de consumo (2ª metade do UNION ALL), o guard é uma **allowlist de CFOP de venda de saída**: `AND v.quantidade > 0 AND v.cfop IN ('5101','5102','5108','6101','6102','6108')`.
...
**Refinamento (Codex 2026-07-12).** `LIKE '5%'/'6%'` era largo demais — aceitava `6202` (devolução de *compra*: é saída, mas NÃO é venda; 5 linhas reais em prod, **0 de pai com ficha** hoje). Trocado por **allowlist positiva** dos CFOPs de venda: produção própria (`5101/6101`, p/ TINGIMIX fabricado internamente), revenda de terceiros (`5102/6102` — os **únicos** que os 37+31 pais usam hoje) e consumidor final (`5108/6108`). Confirmado em prod: **0 pais usam CFOP fora da allowlist** (não subestima nenhuma venda real). Sentinela em §5.
...
- **Fix #10** (devolução) — provado com 2 fixtures de quantidade **positiva**: devolução de venda (CFOP `1201`, entrada) e devolução de compra (CFOP `6202`, saída-não-venda), ambas NÃO explodem consumo (asserts C1/C2). A sabotagem SAB1 remove a allowlist → **as duas** vazam (prova que o guard cobre ambas).
...
- **Limitações declaradas (não bloqueiam; o founder está no loop + curadoria):**
  ...
  - **Não-regressão de SKU-ambos (Codex 2026-07-12):** a promessa "produto vendido não muda de classe" vale para **não-insumos**. Um SKU-ambos (insumo que também é vendido direto) **muda XYZ por design** — passa a somar o consumo (é o objetivo do PR). ABC fica estável (o sintético tem `valor NULL`). O harness prova um NÃO-insumo intacto (assert B); a mudança dos SKU-ambos é intencional e coberta pela curadoria.
  - **Devolução infla demanda DIRETA do acabado (Codex, pré-existente):** o ramo de venda direta não filtra CFOP — uma devolução positiva conta como venda do acabado. É comportamento **pré-existente** (as 4 views já liam `v_venda_items_history_efetivo` assim); o PR-2 não o introduz nem o corrige (filtrar a venda direta mudaria a classe de TODOS os produtos = a regressão que evitamos). 0 casos hoje. Vigiado.
  - **Sentinela de CFOP de venda novo:** se um pai passar a vender com CFOP fora da allowlist, o consumo do insumo é subestimado. Vigilância: `SELECT DISTINCT v.cfop FROM v_venda_items_history_efetivo v JOIN v_pcp_malha_oben m ON m.pai_oben=v.sku_codigo_omie WHERE v.empresa='OBEN' AND v.cfop NOT IN ('5101','5102','5108','6101','6102','6108')` → deve ser vazio; se aparecer CFOP de venda, adicionar à allowlist.
