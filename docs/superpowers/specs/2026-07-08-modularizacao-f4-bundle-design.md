...
3. **Prova de ganho**: rebuild + medição antes/depois no PR. **Ganho pífio (<5 kB gzip no boot) → reverter o fix** e reportar honestamente (tooling fica mesmo assim).
   - **EMENDA (2026-07-08, pós-medição — às claras, não silenciosa):** o antes/depois deu **-2.85 kB gzip** no entry (72.94→70.09), abaixo do chute original de 5 kB. Porém a composição fina mostrou que o teto teórico da cirurgia-componentes era ~3 kB gzip (o grosso dos 48 KB de negócio no eager vem dos HOOKS do sino, deliberadamente fora desta fase) — o fix atingiu ~100% do próprio teto, com custo ~zero (idioma lazy já existente no shell) e eliminou governanca do eager. **Decisão revisada: manter o fix**; o critério de 5 kB permanece para cirurgias futuras cujo teto o comporte (os hooks: ~30 KB fonte restantes).
