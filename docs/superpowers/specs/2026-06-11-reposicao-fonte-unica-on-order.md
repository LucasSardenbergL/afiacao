...
- ⚠️ **Passo 4** (bump no disparo: `disparar-pedidos-aprovados` chama `omie-sync-estoque {only_pending, esperar_codints}`) · **Passo 5** (Sentinela `estoque_reposicao` usa o marcador `reposicao_pendente_po`/`reposicao_estoque_full` em vez de `max(ultima_sincronizacao)`) — PENDENTES.
