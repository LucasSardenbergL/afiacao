...
## Estado: TODOS OS 5 PASSOS FEITOS (Caminho B). Falta o GATE.
- **Codex adversarial xhigh** (esgotado 11/06; retroativo quando voltar) **ANTES de qualquer deploy**.
- **Ordem de deploy** (fail-closed): (1) migration passo 1 (`aplicar_snapshot_pendente`) via SQL Editor; (2) deploy edge `omie-sync-estoque` (passo 2) + `disparar-pedidos-aprovados` (passo 4) via Lovable; (3) forçar 1 sync OBEN → confirmar `sync_state(reposicao_pendente_po, complete)`; (4) **só então** migration passo 3 (motor — senão a barreira aborta tudo) + passo 5 (Sentinela). Preflight `pg_get_functiondef` antes de cada `CREATE OR REPLACE`. Smoke: FUNDO PU (8689734299) `estoque_pendente_entrada = 3`.
