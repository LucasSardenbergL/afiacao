...
- ✅ **PR-4 — Proposta 1-toque** (#1332, mergeado 2026-08-06 · `250754cd`): enviar a cesta de `/rota/propostas` via template com recotação Omie no envio (RPC `get_whatsapp_proposta_cotacao`: praticado▸tabela▸NULL); linha sem preço/estoque/unidade/prazo trava a proposta INTEIRA (ausente≠zero); envio grava orçamento com o elo do funil. Spec: `docs/superpowers/specs/2026-07-13-whatsapp-pr4-proposta-1-toque-design.md`. 🟥 prove-sql (2 falsificações) + Codex adversarial. ⚠️ **Ficou 3 semanas parado em conflito** (não-draft + `CONFLICTING` = o auto-merge nunca dispara) e o merge da `main` expôs 2 gates estruturais criados DEPOIS dele — um deles achado real de money-path: a leitura do dono da conversa descartava o `error`, e `donoConversa=null` apaga a trava `conversa_de_outro_cliente` (proposta com preços sairia no fio de outro cliente). Detalhe em [prs-parados-2026-08-06.md](prs-parados-2026-08-06.md). **Deploy pendente:** migration `20260713050000_whatsapp_proposta_cotacao_v2.sql` no SQL Editor (só a v2 — `CREATE OR REPLACE` da mesma assinatura da v1, e traz o `ALTER TABLE sales_orders` + índice de dedupe) + Publish.
