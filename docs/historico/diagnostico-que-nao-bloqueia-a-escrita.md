...
Prova depois do r3 (corpo estrito, plano aprovado, página única na escrita): módulo **79/79** (Deno) · mutcheck **79/79 pegas**, 0 sobreviventes, 0 inválidas, controle+ 79/79 · harnesses das ferramentas de operação verdes depois do r4 (veredito 54 casos, agregador 25, manifesto 13, varredura 8, conferência 16, gerador 20, preâmbulo do condutor e decisão) e **22 sabotagens de checador vermelhas com 7 controles verdes** · `sonda:cron-prova --backfill omie-desconto-backfill` **8/8 PASSA** com o controle novo (as 2 versões novas eram INVERIFICAVEL) · `test:edges` **1140/0** · `edges:typecheck` 0 erros de classe-crash (114 de outras classes tolerados, a dívida conhecida) · 14 gates vitest que leem a edge como texto **204/204** · eslint, knip, `authz:check`, `sonda:bump`, `sonda:fingerprint` (60 edges), `mutcheck-all --seco` (28 contratos), `docs:links` e `docs:indice` verdes.
