...
| [authz-custo-fu4f.md](authz-custo-fu4f.md) | **FU4-F "vendedor não vê custo"** (#1434/#1465/#1473/#1485) — o que fechou (`inventory_position`+view operacional, `cmc_snapshot`, `get_tint_price(s)`, `pedido_compra_item`) e o que **NÃO** fechou (espelho `product_costs`, régua, oráculo do cockpit). Lições: fechar a fonte não fecha o dado se há CÓPIA; permissivas combinam com OR (fechamento fantasma); assert com `ok` nos dois ramos; `CREATE OR REPLACE VIEW` não remove coluna; validar prod contra o diff de um PR **draft** produz diagnóstico falso |
