...
- Contadores com unidade explícita: `consultas_tentadas` (NFes com ≥1 request), `requisicoes_omie`
  (requests físicos), `consultas_detalhadas`, `consultas_adiadas_por_limite`, `consultas_falhas`,
  `controle_marcacoes`.
- Status do run numa decisão pura ([`adiamento.ts`](../../supabase/functions/omie-sync-sku-items/adiamento.ts)),
  ...
  recompute.
- Sensor `results.fila_parada_48h`: NFe consultável, elegível há mais de 48h, que o run não tratou
  (adiada ou não alcançada pelo guard) ⇒ `error` "fila não anda". Independe da vazão das outras NFes.
- O motivo gravado no controle diz o que aconteceu: `upsert_falhou`, `ok_sem_itensRecebimento` e
  `ok_itens_sem_nIdProduto` deixam de se esconder atrás de `ok_0_itens`.
...
3. PR da parte B pronto ⇒ merge ⇒ deploy do `omie-cron-diario` ⇒ o jobid 52 responde sem
