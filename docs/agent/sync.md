...
## Dedup do pull: identidade ESTÁVEL da fonte, nunca hash mutável

O `sync_pedidos` dedupava por Set em memória de `sales_orders.hash_payload` — campo **mutável, nullable e MULTI-WRITER**: o `sync-reprocess` o reescrevia como hash estrutural (`hashObject({cab,itens})`), de-namespeando o `omie_<conta>_<cod>`, e linhas legadas com hash NULL eram invisíveis ao Set → **re-inserção → duplicata** que infla positivação/OTE/comissão (só satura em 2 cópias porque o `.maybeSingle()` do reprocess quebra silencioso em 2 linhas; achado Codex 2026-06-17, 528 oben + 6 colacor). **Regra:** dedup de sync **sempre** pela **identidade estável da fonte** (`omie_pedido_id` = `codigo_pedido` do Omie — sempre setado no pull, nunca tocado pelo reprocess), **nunca** por hash que algum writer recalcula; change-detection vai em **coluna dedicada** (`reprocess_content_hash`), separada do campo de identidade (1 writer por coluna de sinal — money-path). Guard de fronteira = índice **parcial** discriminado por `checkout_id` (`UNIQUE (account, omie_pedido_id) WHERE checkout_id IS NULL`): o pull tem `checkout_id` NULL, o push (envio do app) NOT NULL → barra 2 linhas-pull do mesmo pedido **sem** quebrar a dualidade push/pull intencional (por isso o `UNIQUE(account,omie_pedido_id)` pleno foi removido na `20260613120000`). Conserto sequenciado (edge→limpeza→índice) em voo: PR #928 / `docs/superpowers/plans/2026-06-17-vendas-dup-guard-pull-identity.md`.
