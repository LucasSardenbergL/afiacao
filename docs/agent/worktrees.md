...
Limite conhecido (fase 1): não pega a *race fria* (duas sessões, nenhum arquivo escrito ainda), nem `ALTER TABLE`/`DROP+CREATE`. Testes: `scripts/lib/migration-objects.test.ts` · `scripts/test-preflight-migration.sh` · `scripts/test-migration-collision-guard.sh` — as três rodam no CI.
...
- **Worktree nasce pronto:** `bun run wt` roda `bun install` na criação; para worktree criado pelo app (`.claude/worktrees/*`), o hook `vigia-worktree.sh` (SessionStart) dispara `bun install` em background e avisa a sessão. ⚠️ **typecheck vermelho com `Cannot find module`/dep `@lovable/*` ausente = deps não instaladas, NÃO é CI vermelho** — o CI real se confere com `gh pr checks`. Com o install **EM VOO** o falso vermelho troca de cara e fica MUITO mais caro: a árvore parcial derruba o React em RUNTIME (`Cannot read properties of null (reading 'useRef')` de dentro de um componente), e isso **parece bug do seu código** — custou um ciclo de TDD inteiro em 2026-09-06. Discriminador (roda em segundos): `bun install --frozen-lockfile`; **verde depois dele, com o lockfile intacto, = era a árvore, não o código**. O gatilho a montante é `node_modules` ter sido APAGADO (o hook só dispara com o diretório ausente) — `wt:clean` alheio é o suspeito, mas a causa NÃO foi provada; no macOS o bun usa `clonefile`/CoW, **não** hardlink (inodes distintos entre worktrees, `nlink=1`), então install de OUTRA worktree não corrompe a sua. O mesmo hook alerta swap alto (>6GB) e >6 sessões Claude vivas — a alavanca real de RAM é FECHAR sessões (`wt:clean` num parque de sessões vivas libera 0MB).
- **`node_modules` VAZIO não dá erro — dá as deps do checkout PRINCIPAL.** A worktree do app mora dentro dele (`.claude/worktrees/*`), e a resolução sobe o diretório: medido em 2026-09-25, com o `node_modules` da worktree existente e vazio, `yaml` resolveu de `afiacao/node_modules` e `bun run tsc --version` respondeu `Version 5.8.3` — teste verde com dep de OUTRA árvore, sem aviso (e o hook acima só olha se o diretório EXISTE). Sonda de deps é pelo caminho LOCAL com resposta positiva (`node_modules/.bin/<b> --version` → rc 0 + versão), nunca pelo nome: é a guarda 13 do `exclusividade:medir` ([histórico](../historico/exclusividade-media-outra-coisa.md)).
...
- **Vigia acusou `worker-service.cjs --daemon` (claude-mem) com CPU alta + prompts bloqueados por "claude-mem worker unreachable for N consecutive hooks":** é o worker VIVO-MAS-SURDO — o plugin NÃO se recupera sozinho (só distingue pid morto de vivo; `stop`/`restart` falam HTTP com um surdo) e o hook sai `exit 2` por desenho após 3 falhas, em TODAS as sessões. Desde 2026-09-05 a própria linha do vigia já traz o discriminador (`[claude-mem: health SURDO …; contador=N …]` + a receita quando surdo e contador ≥ 3; sonda ausente vira `nao sondei`, nunca ok). **Desde 2026-09-25 a receita é um script:** `bun run claude-mem:reanimar -- --so-olhar` (diagnóstico, não toca em nada) e, sem a flag, a recuperação — olha antes (`ps` + `/api/health` + contador), só mata processo do PRÓPRIO claude-mem e só após 3 sondas falhando, pede confirmação, sobe pelo CLI do plugin e só diz `RECUPERADO` com health 200 + hook `context` rc=0 + contador 0. Worker jovem (< 60 s) nunca é tocado; porta de outro programa, sonda que não responde de forma interpretável ou endereço incoerente = PARA sem matar. Provado pelo laboratório `scripts/lab-claude-mem-reanimar/` (plugin falso, Linux e macOS) no `test:hooks`. O plugin ≥ 13.24.18 falha em SILÊNCIO depois do 1º bloqueio — quem avisa é o bloco 6 do vigia (`scripts/claude-mem-saude.sh`). Histórico e receita manual em `docs/historico/claude-mem-worker-vivo-mas-surdo.md`.
