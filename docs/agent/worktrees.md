...
⚠️ **`~/.local/bin/heavy` é CÓPIA, não symlink: mergear na `main` não atualiza o semáforo em uso** (mesma armadilha do Lovable — repo ≠ produção). Instalar é passo manual e o `cp` tem de ser **atômico** (`cp` para tmp + `mv`), senão substituir o arquivo por baixo de um `heavy` em execução faz o bash reler o script novo a partir do offset antigo. Convivência de versões é segura (o antigo ignora o subdir `fila/`; só não entra no FIFO).
