...
⚠️ **CLASSE — artefato JSON versionado lido com cast lê o formato ALHEIO como o de hoje** ([base](../historico/exclusividade-media-outra-coisa.md), 2026-09-25). Assinatura: `JSON.parse(...) as <Tipo>` sobre arquivo COMMITADO que carrega `schemaVersion` — o campo gravado e nunca conferido vira TypeError (exit 2) no melhor caso e, no pior, veredito calculado sobre dado de outro formato. Leia por uma porta que confere a versão ANTES da forma e devolve RECUSA com código: `lerMatriz` (matriz do `exclusividade`, gate e motor) e `avaliarCarimbo` (authz). Gate: o bloco "a CLASSE" de `scripts/exclusividade-gate.test.ts` cobra zero `as Matriz` em `scripts/`, com sentinela contra cegueira. Varrido em 2026-09-25: o único leitor ainda afetado é `db/authz-carimbo-gravar.ts` (relê o carimbo anterior sem conferir a versão) — decisão de desenho própria, em chip.
...
| `generate-tactical-plan` | `{"canary":true}` | `v1.1-paginacao-eof-e-cursor` ⚠️ servido no campo **`versao`**, não `contrato` (é o `VERSAO` da sonda; o `canaria:bump` **enxerga** essa forma e resolve o símbolo) | margem ausente degrada em vez de fabricar (velho: NULL→`?? 0`→R$0/h; #1498) |
...
Prova executada (PG17, 21 asserções + 15 sabotagens com controle verde): `bash db/test-canaria-veredito.sh [--falsificar]`. **Está no `nucleo-ci.txt`** desde o #2403 — o job `provas-sql` carrega o runtime `bun` (sem `bun install`) e `fetch-depth: 0` por causa dela. O SQL sai do gerador DESTE disco por `db/lib/gerar-canaria-fixture.ts`, e não pela CLI: a CLI tem guard de sincronia com a `origin/main`, que num PR é impossível de satisfazer (`sonda:bump` e `sonda:fingerprint` exigem justamente divergir — #2414). O artefato de fixture é INERTE por construção (o SQL inteiro é o valor de uma variável dentro de um `DO` que só faz `RAISE`), e as duas pontas disso são medidas na própria suíte. ⚠️ **Gerar o SQL de outro lugar que não este disco cega a prova**: a tentativa de rodar a CLI num worktree de `origin/main` (#2405) deixou a suíte **19 ok / 0 fail** com o gerador sabotado — as sabotagens `GERADOR …` do `--falsificar` existem para reprovar essa volta. **A fixture NÃO é o artefato operacional**, e o que garante que ela representa o que a CLI emite é uma asserção, não a leitura do código: `EQUIVALENCIA_CANARIA` (em `scripts/sonda-versao-sql.test.ts`) exige que o stdout do EXECUTÁVEL da fixture seja a moldura FIXA envolvendo, byte a byte, o que a CLI emitiu para os mesmos insumos. Sem ela a prova fica verde com a CLI divergente — MEDIDO em 2026-09-09: janela trocada só no ramo da CLI, prova 21 ok / 0 fail; e verde também com a CLI emitindo ZERO bytes. Falsificação no `scripts/mutcheck.d/sonda-versao-sql.mut`.
...
- **13ª leva — `whatsapp-inbound`, a única edge do canal WhatsApp que tinha ficado fora do mapa** (2026-09-26), em `v1.0-sensor-inicial`. O critério é "não existe caminho de prova": quem chama é a 360dialog, e o log próprio (`whatsapp_webhook_events`) é gravado por toda versão. Na entrada, a versão servida era INDETERMINADA entre três bundles, e um deploy sem sensor seria inatestável ([histórico](../historico/whatsapp-inbound-sem-prova-edicao-de-tipo.md)). A forma é a da `omie-webhook`: a sonda fica ANTES do gate `x-whatsapp-secret`, com `authorizeCronOrStaff` próprio (`GATE_PROPRIO`). O harness executado mostrou que os caminhos que não são sonda respondem byte a byte igual ao bundle anterior, e que sondar o pré-sensor devolve 401 sem I/O (BARATA). Fica **fora** da sonda por cron de propósito: o relé exige bloco `OPTIONS` com CORS, e webhook servidor-a-servidor não tem preflight.
...
⚠️ **Herdar o `sonda:bump` cobre o DIRETÓRIO da edge, não `_shared/` — e a diferença foi MEDIDA.** Sabotando a `generate-tactical-plan` sem bumpar o marcador: mexer em `plano-helpers.ts` (dentro do diretório) faz o `sonda:bump` reprovar (exit 1); mexer em `_shared/tactical-margem.ts` — a `margemConhecida` fabricando zero, que é a decisão money-path que essa canária EXISTE para atestar — passava nos **dois** gates. A causa era de DESCOBERTA, não de régua: o `canaria:bump` achava canária por regex de `contrato:` e não via essa edge, então respondia "7 conferidas" com e sem o buraco (a classe do #2363). Ele agora reconhece o marcador servido em `versao` por símbolo e resolve o literal no `versao.ts` — o marcador continua **um**, vigiado por dois gates em eixos complementares: diretório da edge (`sonda:bump`) e superfície da canária, `_shared/` incluído (`canaria:bump`). E uma edge que EMITA `canary: true` sem que o gate localize canária nenhuma agora **reprova por não conseguir medir**, em vez de seguir verde.
...
- ⚠️ **`NUNCA_ATESTADA` de edge RECÉM-instrumentada: a 1ª sonda vem DEPOIS do deploy, nunca antes (2026-09-25, #2540).** Até o deploy, o bundle servido é o velho, que não conhece a sonda. Como o POST do `sonda:sql` leva credencial, esse bundle passa no gate e executa o fluxo real: no `omie-cron-diario`, seriam os 5 steps. Além disso, o `pendencias:pacote -` não emite pacote para `NUNCA_ATESTADA`, porque não é estado de deploy. O caminho é outro:
  1. Decida por medição **passiva**. Se a resposta do cron que já chama a edge, em `net._http_response`, vem sem `edge` no topo, o bundle servido é o velho e difere da main.
  2. Monte o pacote nomeando a edge: `bun scripts/pendencias-pacote.ts <edge>`.
  3. A 1ª atestação vem do eco do próximo tick, ou de uma sonda, que a essa altura já é segura.

  Caso completo em `docs/historico/sku-items-consumo-redundante-no-ciclo.md` §9.
- A skill **`lovable-deploy-verify`** confere se o bundle servido bate com o esperado (bytes/comportamento). Use após Publish/deploy — não confiar cegamente no "deployed" do Lovable. **N2 de edge (prova de versão) é automático quando `~/.config/afiacao/supabase-pat` existe** (Access Token do Supabase, `chmod 600`, padrão psql-ro): `verify-edge.sh` resolve env `SUPABASE_PAT` > arquivo e consulta a Management API; sem o arquivo, cada verificação de versão vira handoff manual na UI (custou 3 retomadas de sessão p/ confirmar 1 deploy). Teste: `scripts/test-verify-edge-pat.sh`. A varredura por bytes é **paralela** (`xargs -P`, halt-on-hit) — o bundle passou de 300 chunks e o modo 1-a-1 estourava o timeout.
  - ⚠️ **NÃO PEÇA O PAT AO FOUNDER: o projeto roda em Lovable Cloud e o Supabase é da org do LOVABLE** (**MEDIDO 2026-09-07** na sessão logada do founder — até então isto era **lembrança** dele, de 2026-07-23, e `scripts/test-verify-edge-pat.sh` **stuba o curl**, então a Management API real nunca tinha sido chamada: `git grep` por 401/403 nesse contexto dava **zero**). A conta GitHub do founder no `supabase.com` tem **UMA** organização (`LucasSardenbergL's Org`, `ikmcoarorcprkhwhbksv`) com **2 projetos, ambos PAUSADOS e com uso zerado**: `Afiacao` (`lkotrsfdvnwxqyevhffh`) e `Sardenberg` (`eylbxymqucglkarplgto`) — o **Supabase standalone**, a armadilha de maio, não produção. O ref `fzvklzpomgnyikkfkzai` **não aparece em org nenhuma dela**, logo **não existe Access Token para ele gerar** e o N2 é estruturalmente indisponível. ⚠️ **Trocar de identidade NÃO resolve** (Google, e-mail, outro GitHub): o projeto não está atrás do login errado, está em **outra organização** — e o doc do Lovable (conferido 2026-09-07) não oferece conectar/transferir para um Supabase próprio: a única saída listada é *Remove Lovable Cloud*, que **apaga** a instância. **Não re-teste isto** — a pergunta está fechada por medição — o arquivo `supabase-pat` continua válido como mecanismo, só que ninguém pode preenchê-lo neste setup. A escada real de prova de edge aqui é: **N1** (`verify-edge.sh`, OPTIONS → servida) **+ rastro do commit do bot** na `main` (`Deployed …`/`Redeployed …` — evidência de que o deploy rodou, não de qual versão) **+ canária comportamental** quando a edge tiver uma (a ÚNICA prova de versão disponível). Edge sem canária: declare "N1 + rastro; versão não provada" — nunca "no ar". Se a entrega for money-path e a prova importar, **crie a canária junto do fix** (padrão `identidade_probe`/`credito_gate_probe`), porque depois não haverá como provar.
