...
- ⚠️ **`computeCosts` (recompute) cobre todo o catálogo ATIVO _com preço_** (`.eq(ativo,true)` + `fetchAll` paginado fura o cap de 1.000; pula `valor_unitario<=0`), mas **não** inativos nem ativos sem preço. Os **2 seed writers** (`omie-analytics-sync`/`syncInventory`, `sync-reprocess`) inserem produto novo como `CMC@0.7` **sem** o ladder (sem anti-lixo `cmc/price`, sem flag `CMC_MARGEM_ATIPICA`) → reconciliados só no próximo `computeCosts`. O **número não é mascarado** (`cost_price=cmc` é o custo real), mas há janela até o recompute; mitigada subindo o cron `compute-costs` p/ 2h (decisão Claude+Codex jun/2026). Passivo (inativo/sem-preço/seed não-reconciliado) limpa-se por **SQL**, não pelo recompute ([#977]).
