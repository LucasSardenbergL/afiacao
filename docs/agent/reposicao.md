...
- ⚠️ **Seed de `product_costs` não passa pela escada** (dívida de governança ADIADA, não-urgente): os 2 writers de produto NOVO (`omie-analytics-sync:847` bulk, `sync-reprocess:587` N+1) gravam `CMC`/0.7/`cost_price=cmc` **cru** — sem o anti-lixo `[0.01,5]` nem a classificação `CMC_MARGEM_ATIPICA`; só o `computeCosts` reconcilia no ciclo seguinte. O número é **real** (não mascarado), só `cmc`-lixo raro vaza na janela. Se um dia aplicar o ladder no seed: **sem-preço (`valor_unitario<=0`) MANTÉM `CMC`+`cost_price=cmc`** (o recompute pula `price<=0` → `UNKNOWN` viraria permanente, regrediria ~21 ativos sem preço mas com venda). Decisão/tarefa: `docs/superpowers/specs/2026-06-22-cost-source-seed-writers-debt-spec.md`.
