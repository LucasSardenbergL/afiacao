...
**Follow-ups (Codex, não-urgentes — custo/precos_base não existem hoje):** (a) E4/precos_base usam loop-por-fórmula → migrar p/ set-based se algum dia vier custo em massa; (b) E1 atualiza `tint_corantes.volume_total_ml` mas preserva `preco_litro` (incoerência se algo ler direto); (c) ✅ **FEITO 22/06** — `idle_in_transaction_session_timeout=30s` no role `authenticator` (`ALTER ROLE`; cluster segue `0`, cirúrgico; confirmado via `psql-ro`) — mata conexão presa antes que trave o lock (causa-raiz dos 2 lock-timeouts); (d) higiene: `DROP TABLE tint_formulas_backup_preflip` (103 MB) após estabilizar.

**✅ RESOLVIDO (22/06) — nome_cor NULL das personalizadas:** fix (1) aplicado — migration `20260622130000_tint_promote_nome_cor_fallback.sql` (CASE quando `nome_cor` vazio → `cor_id`, preserva nome verbatim; PG17 + Codex; PR #992). Prod (`oben`, 22/06): **776 personalizadas ativas, `nome_cor` NULL = 0** (23502 morto). `TESTE CLAUDE` aparece correta (`cor_id="TESTE CLAUDE"`, `nome_cor="TESTE CLAUDE"`). Sobrou **1 órfã** do 1º envio falho (`cor_id=4602073`, do timing em que a `personcor` estava sem código/descrição) → some no próximo keys-snapshot (a fonte não emite mais esse id). **Conector NÃO precisa do fix (2):** o discovery (22/06) provou o mapeamento `personcor` correto (`id`/`codigo_cor`/`descricao` → lookup `CorPerson` em `sync.go`); o `cor_id` numérico era timing transitório do 1º envio, já auto-corrigido quando a fórmula re-emitiu. Demais `cor_id` numéricos (4848/6269/7585…) são cores SEM código na origem (`codigo_cor` em branco → id numérico + nome da descrição) = **correto**. O re-scan semanal (domingo, `clearAllHWM`) é a rede contra updates de nome que não tocam `data_atualizacao` — torna desnecessário o "Fix A" (re-emitir na hora) salvo se a latência semanal incomodar.

**🗑️ Aposentadoria do CSV no app (22/06, Opção A — break-glass):** com o catálogo automático no ar, a importação manual por CSV saiu do uso normal. Na `TintIntegracao` o default da tela virou "Integrações" e a aba "Importar" virou **"Produtos Omie"** (mantém só o `SyncCard` do Omie, que segue vivo); o `ImportCard` + histórico ficam atrás de break-glass `?csv=emergencia` (a rota `/tintometrico/importar` redireciona pra lá) com banner de aviso. **Prazo de remoção total: 2026-07-13** — guardado por deadline test no CI (`src/test/csv-aposentadoria.deadline.test.ts`), que fica vermelho na data e trava o auto-merge até a limpeza. Não depende de memória de ninguém.

**✅ REMOÇÃO TOTAL CONCLUÍDA (13/07, PR #1314):** o break-glass foi excisado — diff **100% frontend** (0 migration, 0 edge): `useDirectTintImport` (−765), `ImportCard`, `HistoryTable`, `preflight-files` e o próprio deadline test **apagados**; `TintImport` reduzido ao `SyncCard` do Omie. A aba **"Produtos Omie" foi MANTIDA** (renderiza o `TintImport` enxuto) e o redirect `/tintometrico/importar` → `?tab=importar` **permanece coerente** (aponta pra aba viva). **Verificado no ar por bytes** (`lovable-deploy-verify`): entry `index-aC2DQL7B.js`; banner `"Importação CSV em modo emergência"` **ausente** nos 319 chunks + controle `"produtos tintométricos sincronizados"` **presente** em `TintImport-D85nebFE.js`. Auto-merge do repo **destravado** (deadline test fora da `main`, PRs abertos verdes). O corte SayerSystem (Blocos A/B abaixo) fica como histórico da operação.
...
3. **Edge `tint-sync-agent` em PROD** — ✅ **já confirmado por evidência**: 129 promoções `sync_agent` rodaram em prod (última 16/06 11:54), logo o gate existe e funciona (`supabase/functions/tint-sync-agent/index.ts:330`<!--cita: path === "catalogs"--> `/catalogs`, `supabase/functions/tint-sync-agent/index.ts:433`<!--cita: path === "formulas"--> `/formulas`; desde 2026-09-25 esses endpoints só ENFILEIRAM — a promoção roda no cron `tint-promocao-tick`, migration `20260925210000`). **Revalidar com `lovable-deploy-verify` só se a edge for redeployada** antes do flip.
