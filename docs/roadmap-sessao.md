...
## 7. "Buddy" (inspirado na UPOPS/PMBuddy do Itaú) — Crítica da Fila v1
...
- ✅ **Spec + plano escritos e aprovados.** `docs/superpowers/specs/2026-06-04-critica-da-fila-design.md` + `docs/superpowers/plans/2026-06-04-critica-da-fila.md`. v1 = bloco **"Por que agora"** nos top-5 cards do Meu Dia: timeline de fricção + **4 badges de contradição determinísticos** (`recorrente_sumiu`, `tarefa_feita_sem_prova`, `sem_resposta_repetido`, `alto_valor_fora_rota`) + feedback (PostHog). **100% frontend, zero backend** (customer_metrics_mv client-readable; thresholds reusam o `useAiOps`). WhatsApp-voz adiado. **Critério de morte de 2 semanas** embutido.
- ✅ **Build concluído (subagent-driven) + revisão em 2 etapas (conformidade ✅ + qualidade ✅, opus) + 2 fixes P3.** Verde: typecheck strict 0, **2194/2194 testes**, lint 0, build OK. Arquivos: `src/lib/fila/critica/*` + `useCriticaFila.ts` + `PorQueAgora.tsx` + `FilaDoDia.tsx`.
- ✅ **PR [#585](https://github.com/LucasSardenbergL/afiacao/pull/585) aberto com auto-merge** (squash no CI verde). **Merge com a main resolvido** (conflito com o #580 G1-Fase-2: integrei `PorQueAgora`/`useCriticaFila` com o `AcaoOutcomeMenu`/`fila.exibida` deles — complementares no mesmo `FilaDoDia`).
- ⏳ **Depois do merge: Publish no Lovable + piloto de 2 semanas.** Calibrar `altoValorFat90dMin` (R$5.000) / `altoValorDiasQuietoMin` (45d) com dado real.

---

### Encerramento da sessão (housekeeping recorrente)
- Manter este roadmap atualizado a cada mudança (reflete **#559 + #562 mergeados** + a feature de voz + a frente 5 dos tingidores '04' + o programa **Buddy/Crítica da Fila** [§7]).
- PRs de doc/fix abertos com auto-merge quando o CI passar.
- **O que depende de você:** (1) verificação visual da Fase 1 (libera o build da Fase 2) · (2) **revisão do spec da criação por voz** (libera o plano) · (3) **rodar a query de diagnóstico do sinal '04'** (frente 5) · (4) **Publish + iniciar piloto da Crítica da Fila** após o [#585](https://github.com/LucasSardenbergL/afiacao/pull/585) mergear · Publish no Lovable pro preview · SQL no SQL Editor quando houver migration (Fase 2 e a feature de voz terão).
