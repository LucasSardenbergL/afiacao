...
- ✅ **Build concluído (subagent-driven).** 10 tasks TDD; revisão em 2 etapas (conformidade ✅ + qualidade ✅, opus) + 2 fixes P3 (texto honesto do badge alto-valor + config morta removida). Estado verde: typecheck strict 0 erros, **2194/2194 testes**, lint 0 erros, build OK. Arquivos: `src/lib/fila/critica/{types,montar,build-inputs}.ts` (+`__tests__`), `src/hooks/useCriticaFila.ts`, `src/components/fila/{PorQueAgora,FilaDoDia}.tsx`. Bug do plano pego pelo subagente: `RouteContactItem` usa `customerUserId` (camelCase).
- ⏳ **PR aberto (auto-merge no CI verde) → depois: Publish no Lovable + piloto de 2 semanas.** Calibrar `altoValorFat90dMin` (R$5.000) / `altoValorDiasQuietoMin` (45d) com dado real no piloto.
...
- **O que depende de você:** (1) verificação visual da Fase 1 (libera o build da Fase 2) · (2) **revisão do spec da criação por voz** (libera o plano) · (3) **Publish + iniciar piloto da Crítica da Fila** depois que o PR mergear (calibrar limiares com dado real) · Publish no Lovable pro preview · SQL no SQL Editor quando houver migration (Fase 2 e a feature de voz terão).
