...
- 🧭 **Escolha de execução** (subagent-driven vs inline) → começa o build.
- ⏳ **Build** — **F1** (função+view+selo no inbox+card/badge+painel `/whatsapp/sla`, sem edge) → CHECKPOINT visual → **F2** (digest cron+e-mail, edita `dispatch-notifications`). Migration via SQL Editor (ritual `lovable-db-operator`).
