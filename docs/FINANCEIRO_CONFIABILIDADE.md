...
**Limitações v1 / o que ficou pro v2:** versionamento/snapshot histórico do forecast = v2; impostos regime-aware (compõe `fin-regime-tributario`) = v2; decomposição de variância volume/preço/mix = v2; consolidado cross-CNPJ = v2. **Drill de variância por categoria = passo focado SEPARADO** (decisão do founder): precisa de atribuição por **CÓDIGO** (`fin_categoria_dre_mapping`: `omie_codigo→dre_linha`), NÃO por descrição (`getAnaliseDimensional` agrega por descrição → casar por descrição arrisca mis-atribuir o desvio). ⚠️ **Follow-up de regime:** a página exibe badge "Regime de Caixa" mas chama `getDRE` no default `competencia` — v1 manteve consistência (competência explícita); confirmar com o founder qual é o correto e alinhar as duas telas.
