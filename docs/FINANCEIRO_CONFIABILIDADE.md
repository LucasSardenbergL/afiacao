...
**Limitações v1 / o que ficou pro v2:** versionamento/snapshot histórico do forecast = v2; impostos regime-aware no forecast (compõe `fin-regime-tributario`) = v2; decomposição de variância volume/preço/mix = v2; consolidado cross-CNPJ = v2. ✅ **Follow-up de regime RESOLVIDO (PR #418):** a badge dizia "Regime de Caixa" mas a página sempre calculou **competência** (correto p/ orçado×realizado; o forecast depende dela p/ reconciliar). Era rótulo errado → corrigido p/ "Regime de Competência".
