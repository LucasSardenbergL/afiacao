...
```sql
-- V1. security_invoker preservado nas 4 views. ATENÇÃO: as views recriadas por este PR
--     (WITH security_invoker = true) gravam o reloptions como 'security_invoker=true' — o
--     PG17 preserva o LITERAL. (A view antiga v_venda_items_history_efetivo tem '=on' porque
--     o #1292 a escreveu com '= on'.) Casamos ambos os literais p/ robustez.
...
**Esperado:** V1=4 · V2 demanda > 0 · V3=0 · V4 classes ≈ baseline · V5 = os insumos com lead time.

> **⚠️ V4 tem ruído temporal — não é a prova rigorosa.** O baseline (A=68/B=90/C=164) foi capturado num dia; a janela ABC é **90d móvel**, então 1 dia de diferença já move um SKU na fronteira B/C sem ter a ver com o religamento. A prova RIGOROSA de não-regressão é estrutural, rodada em prod 2026-07-12 (ambas = **0**):
> 1. **Fonte no-op**: `v_sku_demanda_efetiva` `EXCEPT ALL` `v_venda_items_history_efetivo` (bidirecional) para não-insumos = **0** → a troca de fonte é idêntica para os vendidos.
> 2. **População ABC**: recomputar a classe só com quem tem faturamento (`valor_total_90d IS NOT NULL`) vs a classe atual = **0 SKUs movidos** → os insumos (receita NULL) não deslocam nenhum vendido no Pareto (`sum()` ignora NULL).
