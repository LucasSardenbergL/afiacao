...
| **Linear** | Quase-neutro que aguenta jornada longa; densidade alta como **escolha**, não como aperto | Decisão A1 e D1 (§5) · `src/index.css:306`<!--cita: densidade alta ali é INTENCIONAL--> |
| **Notion** | Navegação por seções, Recentes/Favoritos na sidebar, stagger na expansão | `05-revisao-skill.md` SB1/SB2 · `docs/ux-audit/03-roadmap.md:143`<!--cita: Notion sidebar--> |
| **Polaris** (Shopify) | Componentes de dado B2B: EmptyState compacto, ResourceList | `docs/ux-audit/03-roadmap.md:223`<!--cita: Polaris EmptyState compact--> · `docs/ux-audit/03-roadmap.md:270`<!--cita: bulk actions bar do Retool table--> |
| **Carbon** (IBM) | **Rigor de sistema, não estética** — (a) *design tokens*: todo sinal semântico via token, nunca cor crua; (b) *touch target spec*: 44×44 mínimo, 56×56 para uso com luva | A regra `text-status-*` (em vez de `text-emerald-600`) vem de `03-roadmap.md:332`<!--cita: Carbon Design tokens system-->; `<Button size="touch">` 44px e `balcao` 56px vêm de `03-roadmap.md:94`<!--cita: Carbon Touch Target spec--> |
| **Retool** | O vocabulário de **ferramenta interna**: a tabela densa é a superfície principal — bulk actions, atalhos `j/k`, e tela de gestão pode ser desktop-only sem se desculpar | `src/index.css:306`<!--cita: densidade alta ali é INTENCIONAL--> (32px densos em ponteiro fino, decisão explícita) · `docs/ux-audit/02-heuristica.md:133`<!--cita: Apenas desktop--> · `docs/ux-audit/02-heuristica.md:416`<!--cita: navega linhas--> · `03-roadmap.md:270`<!--cita: bulk actions bar do Retool table--> |
