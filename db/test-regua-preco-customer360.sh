...
# ════════ ZONA 1 — pré-requisitos (order_items COM omie_codigo_produto; sales_orders COM created_at) ════════
P -q <<'SQL'
...
CREATE TABLE public.sales_orders (id uuid PRIMARY KEY, account text, order_date_kpi date,
  created_at timestamptz, deleted_at timestamptz, customer_user_id uuid);
...
# ════════ ZONA 3 — seeds ════════
# CA=cliente do 360. 7001 caso feliz (banda=qty da última venda); 7002 abaixo do piso; 7003 sem_produto;
# 7004 sem_preco; 7005 sem_quantidade (qty 0); 7007 empate no mesmo dia (desempate por created_at).
P -q <<'SQL'
...
INSERT INTO public.inventory_position VALUES
  ('a1111111-1111-1111-1111-111111111111','oben',100),  -- P1: piso 125
  ('a2222222-2222-2222-2222-222222222222','oben',100),  -- P2: piso 125
  ('a4444444-4444-4444-4444-444444444444','oben',100),  -- P4
  ('a5555555-5555-5555-5555-555555555555','oben',100),  -- P5 (sem_quantidade)
  ('a7777777-7777-7777-7777-777777777777','oben',100);  -- P7 (empate mesmo dia)

INSERT INTO public.sales_orders (id, account, order_date_kpi, created_at, deleted_at, customer_user_id) VALUES
  ('d0000000-0000-0000-0000-000000000001','oben', current_date-40, now()-interval '40 days',         NULL, '11111111-1111-1111-1111-111111111111'), -- CA P1 ANTIGO
  ('d0000000-0000-0000-0000-000000000002','oben', current_date-10, now()-interval '10 days',         NULL, '11111111-1111-1111-1111-111111111111'), -- CA P1 RECENTE
  ('d0000000-0000-0000-0000-000000000003','oben', current_date-5,  now()-interval '5 days',          NULL, '11111111-1111-1111-1111-111111111111'), -- CA P2
  ('d0000000-0000-0000-0000-000000000004','oben', current_date-7,  now()-interval '7 days',          NULL, '11111111-1111-1111-1111-111111111111'), -- CA P4 (preço 0)
  ('d0000000-0000-0000-0000-000000000005','oben', current_date-6,  now()-interval '6 days',          NULL, '11111111-1111-1111-1111-111111111111'), -- CA P5 (qty 0)
  ('d0000000-0000-0000-0000-000000000071','oben', current_date-3,  now()-interval '3 days 5 hours',  NULL, '11111111-1111-1111-1111-111111111111'), -- CA P7 mesmo dia, created_at CEDO
  ('d0000000-0000-0000-0000-000000000072','oben', current_date-3,  now()-interval '3 days 1 hour',   NULL, '11111111-1111-1111-1111-111111111111'), -- CA P7 mesmo dia, created_at TARDE
  ('e0000000-0000-0000-0000-000000000001','oben', current_date-10, now()-interval '10 days',         NULL, '55555555-5555-5555-5555-555555550001'), -- comparável C1
  ('e0000000-0000-0000-0000-000000000002','oben', current_date-10, now()-interval '10 days',         NULL, '55555555-5555-5555-5555-555555550002'); -- comparável C2

INSERT INTO public.order_items (customer_user_id, product_id, omie_codigo_produto, quantity, unit_price, sales_order_id) VALUES
  -- CA P1 (7001): antigo qty20/128, recente qty10/130 -> preco_atual=130 @ qty10 -> banda 5..20
  ...
  -- CA P4 (7004): unit_price 0 -> sem_preco
  ('11111111-1111-1111-1111-111111111111','a4444444-4444-4444-4444-444444444444',7004, 8,  0,'d0000000-0000-0000-0000-000000000004'),
  -- CA P5 (7005): unit_price>0 mas quantity 0 -> sem_quantidade (não fabrica banda 0..0)
  ('11111111-1111-1111-1111-111111111111','a5555555-5555-5555-5555-555555555555',7005, 0,100,'d0000000-0000-0000-0000-000000000005'),
  -- CA P7 (7007): mesmo order_date_kpi, created_at distintos -> preco_atual=210 (o created_at mais TARDE)
  ('11111111-1111-1111-1111-111111111111','a7777777-7777-7777-7777-777777777777',7007,10,200,'d0000000-0000-0000-0000-000000000071'),
  ('11111111-1111-1111-1111-111111111111','a7777777-7777-7777-7777-777777777777',7007,10,210,'d0000000-0000-0000-0000-000000000072'),
  -- comparáveis de P1 (banda 5..20, 180d, outros clientes)
...
# A3 — comparaveis íntegro (banda 5..20 inclui C1 qty10/120 e C2 qty12/122; leave-one-out tira o CA)
...
# A7 — preco_atual é o ÚLTIMO real, não o mais antigo
...
eq "A11b 7007 qty_ref = 10"                 "$(fnum 7007 qty_ref 10)" "t"
...
if [ "$(fnum 7007 preco_atual 200)" = "t" ]; then ok "F3 tiebreaker sabotado (created_at ASC) → 7007 vira 200 (A11 tem dente)"; else bad "F3 sabotagem não mudou o desempate → A11 é teatro [veio $(ftxt 7007 preco_atual)]"; fi
P -q -f "$MIG360"  # restaura
