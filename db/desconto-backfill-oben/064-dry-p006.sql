...
-- disparo 64 · etapa dry · página 6
-- Passada dry automática — página 6 (conduzir.sh). Passo 2 REFEITO no dia UTC do canário (retomada da pág. 4 após o defeito do condutor no ramo CONTINUAR_EM_PARE). Alvo congelado 2026-09-14T14:28:21Z: 10505 linhas / 4826 pedidos, sha256(ids ordenados)=520efbf3edc97e16d31bfa5ae4d6ba170642bd7b9315cd2792279ba77c9e9280.
--
...
DECLARE
  ...
  c_arquivo constant text  := 'db/desconto-backfill-oben/064-dry-p006.sql';
  c_etapa   constant text  := 'dry';
  c_corpo   constant jsonb := '{"dry_run": true, "account": "oben", "meses": 12, "pagina": 6, "max_paginas": 1}'::jsonb;
  v_segredo    text;
  v_request_id bigint;
  v_exec       uuid;
  v_json       jsonb;
