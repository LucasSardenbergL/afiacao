...
PORT=5436
DATA="$(mktemp -d /tmp/pgtest-precoped-acc.XXXXXX)/data"
export LC_ALL=C LANG=C
MIG="$REPO_ROOT/supabase/migrations/20260606190000_reposicao_preco_pedido_cmc_account.sql"
...
"$PGBIN/pg_ctl" -D "$DATA" -o "-p $PORT -k /tmp" -l /tmp/pg-precoped-acc.log -w start >/dev/null
...
P -v ON_ERROR_STOP=1 -tA -c "SELECT 'tem_cmc_account_aware=' || (CASE WHEN pg_get_functiondef('public.gerar_pedidos_sugeridos_ciclo(text,date)'::regprocedure) ILIKE '%ipc.account = ANY%' AND pg_get_functiondef('public.gerar_pedidos_sugeridos_ciclo(text,date)'::regprocedure) ILIKE '%ipc.cmc > 0%' THEN 'sim' ELSE 'NAO' END);"
...
DELETE FROM sku_parametros        WHERE empresa='OBEN' AND sku_codigo_omie::text IN ('100','200','300','500');
DELETE FROM inventory_position    WHERE omie_codigo_produto::text IN ('100','200','300','500');
DELETE FROM sku_leadtime_history  WHERE empresa='OBEN' AND sku_codigo_omie::text IN ('100','200','300','500');
DELETE FROM sku_estoque_atual     WHERE empresa='OBEN' AND sku_codigo_omie::text IN ('100','200','300','500');
DELETE FROM omie_products         WHERE account='oben' AND omie_codigo_produto::text IN ('100','200','300','500');
DELETE FROM fornecedor_habilitado_reposicao WHERE empresa='OBEN' AND fornecedor_nome IN ('FORN A');

-- omie_products: 4 SKUs Produto NORMAL ('01'); a presença de tipo_produto NOT NULL satisfaz o guard
INSERT INTO omie_products (omie_codigo_produto, account, codigo, descricao, ativo, tipo_produto)
VALUES (100,'oben','PRD100','PROD 100',true,'01'),(200,'oben','PRD200','PROD 200',true,'01'),(300,'oben','PRD300','PROD 300',true,'01'),(500,'oben','PRD500','PROD 500',true,'01')
...
VALUES ('OBEN',100,'PROD 100','FORN A',true,'automatica',10,100),
       ...
       ('OBEN',500,'PROD 500','FORN A',true,'automatica',10,100);

-- inventory_position: 100 cmc=536 ('oben') ; 200 cmc=0 ; 300 cmc=-5 ; 500 cmc=700 SÓ na 'vendas' (sem 'oben')
...
VALUES (100,'oben',5,536.00,now()),(200,'oben',5,0,now()),(300,'oben',5,-5.00,now()),
       (500,'vendas',5,700.00,now());   -- [ACCOUNT] cmc só na convenção canônica → motor antigo perdia

-- sku_leadtime_history: média de compras — 100→436,80 ; 200→100 ; 300→200 ; 500→300
...
VALUES (gen_random_uuid(),'OBEN',100,now(),10,4368.00),
       ...
       (gen_random_uuid(),'OBEN',500,now(),10,3000.00);
...
VALUES ('OBEN',100,5,0),('OBEN',200,5,0),('OBEN',300,5,0),('OBEN',500,5,0);
...
UNION ALL SELECT 'B5 sku100 valor_linha = 95*536 = 50920: ' || (valor_linha = 50920.00)
  FROM pedido_compra_item WHERE sku_codigo_omie::text='100'
UNION ALL SELECT 'B6 gerou 4 itens (filtros sobreviveram): ' || ((SELECT count(*) FROM pedido_compra_item)=4)
UNION ALL SELECT 'B7 [ACCOUNT] sku500 cmc SÓ na vendas (700) — motor agora pega (NÃO média 300): ' || (preco_unitario = 700.00)
  FROM pedido_compra_item WHERE sku_codigo_omie::text='500';
SQL
echo ""
