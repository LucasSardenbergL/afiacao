-- ============================================================================================
-- APLICAÇÃO via `bun run db:aplicar` — sync_reprocess_saude, o 22º check do push
--
-- Mesmo EFEITO da migration `20260918200000_data_health_sync_reprocess_saude.sql`, SEM os
-- envelopes `BEGIN;`/`COMMIT;`: a transação é do `db:aplicar`, que executa o corpo via
-- `aplicar_sql()` — e lá comandos de transação são proibidos. A migration com envelope é a que se
-- cola no SQL Editor; este arquivo é o mesmo conteúdo pelo caminho do executor, com recibo no
-- ledger `db_aplicacoes`.
--
-- A PARIDADE é derivada, não digitada: este arquivo é a migration menos as duas linhas de
-- envelope. Conferir com
--   diff <(sed '/^BEGIN;$/d;/^COMMIT;$/d' supabase/migrations/20260918200000_data_health_sync_reprocess_saude.sql) \
--        <(sed '1,/^-- FIM DO CABEÇALHO DESTE ARQUIVO$/d' db/aplicar-data-health-sync-reprocess-saude.sql)
-- tem de sair VAZIO. Duplicar 1.500 linhas de função à mão é como as duas pontas divergem.
--
-- As três funções do trio acoplado vão JUNTAS (docs/agent/sync.md): o watchdog filtra
-- `WHERE t.source = ANY (v_sources)` — source fora do array é check que existe e NUNCA é avaliado.
-- FIM DO CABEÇALHO DESTE ARQUIVO
-- ============================================================================================
-- sync_reprocess_saude — 22o check do _data_health_compute: o reprocesso da Omie passa a ter sensor
--
-- POR QUE: de 2026-09-08 20:15Z a 2026-09-18 o reprocesso da Oben falhou em 122 ciclos
-- `operational/orders` e 10 `strategic/orders`, e os estagios seguintes (inventory/products)
-- pararam de gravar. DEZ DIAS, ZERO alerta — porque nenhum dos 21 checks lia `sync_reprocess_log`:
--   · `vendas_pedidos` mede o sync INCREMENTAL (fin_sync_log/sync_pedidos), que estava verde;
--   · `sync_state_saude` veria erro autodeclarado, mas a edge nao escreve `sync_state` (o marcador
--     `orders` foi removido como fossil na migration 20260824232212);
--   · `data_health_watchdog` so consome o compute (estado em 14/09 22:30Z: 21 avaliados, 0 falhos);
--   · `fin_sync_heartbeat` so resume fin_alertas/fin_sync_log;
--   · a acao `get_health` da propria edge le a tabela, mas nao tem limiar, nao escreve e ninguem a chama;
--   · `cron.job_run_details` diz `succeeded` — isso prova o ENQUEUE, nao o HTTP (a verdade estava em
--     `net._http_response`, HTTP 500, que retem poucas horas).
-- Diario do incidente: docs/historico/reprocess-oben-parado-seis-dias.md. O conserto do reprocesso em
-- si e o #2496 — este PR conserta a CEGUEIRA, que sobreviveria aquele conserto.
--
-- AS TRES FUNCOES VAO JUNTAS (docs/agent/sync.md): registrar o bloco no compute NAO basta. O
-- `data_health_watchdog` filtra `WHERE t.source = ANY (v_sources)` — source fora do array e check que
-- existe e nunca e avaliado (nem verde nem vermelho), e o array tambem define o esperado do dead-man.
-- O `fin_sync_heartbeat` tem sua propria IN-list para o RESUMO do e-mail: o `sync_state_saude` ficou
-- 5 dias alertando fora do resumo por exatamente esta omissao (corrigido em 2026-08-29).
--
-- Os corpos abaixo partem do `pg_get_functiondef` da PROD (lido via psql-ro em 2026-09-18), nao do
-- repo: apply manual diverge, e a ultima a recriar VENCE (docs/agent/database.md §4). A unica
-- diferenca em relacao a PROD e o bloco novo + as duas listas.
--
-- ACL: `CREATE OR REPLACE` PRESERVA o ACL (so DROP+CREATE o reseta). Estado medido antes, que a
-- postcondicao reconfere: _data_health_compute = {postgres, service_role, sandbox_exec} — SEM
-- `authenticated` e sem PUBLIC/anon; watchdog e heartbeat = os mesmos + `authenticated`.
-- ============================================================================================
        ...
        COALESCE(string_agg(
          d.reprocess_type || '/' || d.entity_type || ' (' || d.account || '): '
            || CASE WHEN d.veredito = 'unknown' AND d.nao_catalogada THEN 'chave nao catalogada'
                    ...
                    WHEN d.ultimo_status = 'running' THEN 'travado em running'
      ...
      FROM (
        SELECT cat.account, cat.reprocess_type, cat.entity_type, cat.nao_catalogada,
               ...
               CASE
                 ...
                 WHEN u.status IS NULL THEN 'broken'
                 WHEN u.status IN ('error','failed') THEN 'broken'
                 WHEN u.status = 'running'
                      AND u.created_at < now() - interval '2 hours' THEN 'broken'
                 -- dialeto desconhecido NUNCA vira ok (a tabela tem 3 vocabularios de escritores
                 -- diferentes; um 4o chegando nao pode passar por saudavel)
                 WHEN u.status NOT IN ('complete','running') THEN 'unknown'
                 WHEN s.ultimo_sucesso_em IS NULL
                      OR s.ultimo_sucesso_em < now() - make_interval(hours => cat.sla_h) THEN 'stale'
                 ELSE 'ok'
               END AS veredito
        FROM (
...
BEGIN
  ...
  WHERE source IN ('estoque_reposicao','vendas_pedidos','estoque_inventario','reposicao_sugestoes','carteira_scores',
                   ...
                   'sync_reprocess_saude');  -- [VIGIA tint 2026-06-15] +A no resumo (B fica fora)
...
-- ============================================================================================
-- POSTCONDICAO — EXECUTA o compute. Criar a funcao nao prova nada: SQL/PL-pgSQL sao late-bound e
-- so falham em RUNTIME (CLAUDE.md). Um erro de referencia dentro do bloco novo passaria pelo CREATE
-- e so apareceria na proxima rodada do cron, em silencio.
-- ============================================================================================
DO $post$
DECLARE
  v_n        int;
  ...
  v_vazado   int;
BEGIN
  -- (1) as 3 funcoes existem
  IF (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public'
         AND p.proname IN ('_data_health_compute','data_health_watchdog','fin_sync_heartbeat')) <> 3 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: as 3 funcoes do trio acoplado nao estao todas presentes — '
                    'recriar so uma parte desincroniza o push e cega o check novo.';
  END IF;

  -- (2) o compute EXECUTA e o check novo sai — 1 LINHA, status no vocabulario que o watchdog aceita
  SELECT count(*), count(*) FILTER (WHERE status IN ('ok','stale','broken','unknown'))
    INTO v_n, v_ndist
    FROM public._data_health_compute() WHERE source = 'sync_reprocess_saude';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude devolveu % linha(s), esperado 1. '
                    'O watchdog trata count(*) <> count(DISTINCT source) como "compute quebrado" e '
                    'NAO executa o laco — este bloco cegaria os outros 21 checks.', v_n;
  END IF;
  IF v_ndist <> 1 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude devolveu status fora de '
                    'ok|stale|broken|unknown — o watchdog levanta 22023 e o check nunca e avaliado.';
  END IF;

  -- (3) o contrato GLOBAL continua de pe (1 linha por source no compute INTEIRO)
  SELECT count(*), count(DISTINCT source) INTO v_n, v_ndist FROM public._data_health_compute();
  IF v_n <> v_ndist THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: o compute devolveu % linhas para % sources distintos — '
                    'source duplicado faz o watchdog abortar o laco INTEIRO.', v_n, v_ndist;
  END IF;
  -- 30 = os 29 sources que o corpo de PROD devolvia em 2026-09-18 + o novo. Nao confundir com os
  -- `checks_avaliados=21` do watchdog: aquele numero e o tamanho do v_sources (o compute produz
  -- MAIS checks do que o push avalia — os de fora sao dashboard-only). Este assert e a defesa
  -- contra a regra "a ultima a recriar VENCE": se outra sessao acrescentou um check entre o
  -- pre-voo e o apply, este bloco aborta em vez de REVERTER o trabalho dela em silencio.
  IF v_ndist <> 30 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: o compute tem % sources, esperado 30 (29 medidos em PROD '
                    'no pre-voo + o novo). O corpo de PROD divergiu do que foi lido — outra sessao '
                    'recriou a funcao no intervalo e este apply a estaria REVERTENDO. Releia o '
                    'pg_get_functiondef da PROD e remonte a migration sobre o corpo atual.', v_ndist;
  END IF;

  -- (4) o source esta nas DUAS pontas: sem isso o check existe e nunca e avaliado (silencioso)
  SELECT count(*) INTO v_fonte FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'data_health_watchdog'
     AND pg_get_functiondef(p.oid) LIKE '%''sync_reprocess_saude''%';
  IF v_fonte <> 1 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude nao esta no v_sources do '
                    'data_health_watchdog — o check existiria sem NUNCA ser avaliado (nem verde nem '
                    'vermelho), e o dead-man ainda esperaria 21.';
  END IF;
  SELECT count(*) INTO v_resumo FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fin_sync_heartbeat'
     AND pg_get_functiondef(p.oid) LIKE '%''sync_reprocess_saude''%';
  IF v_resumo <> 1 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: sync_reprocess_saude nao esta na IN-list do '
                    'fin_sync_heartbeat — alertaria sem aparecer no resumo do e-mail.';
  END IF;

  -- (5) ACL preservado: o compute NAO pode ter virado executavel por anon/PUBLIC no replace
  SELECT count(*) INTO v_vazado FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = '_data_health_compute'
     AND (has_function_privilege('anon',   p.oid, 'EXECUTE')
       OR has_function_privilege('public', p.oid, 'EXECUTE'));
  IF v_vazado <> 0 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: _data_health_compute ficou executavel por anon/PUBLIC — '
                    'e SECURITY DEFINER e bypassa RLS.';
  END IF;

  SELECT status, message INTO v_status, v_msg
    FROM public._data_health_compute() WHERE source = 'sync_reprocess_saude';
  RAISE NOTICE 'OK: 22 checks, sync_reprocess_saude=% — %', v_status, v_msg;
END
$post$;
