...
-- Único delta vs a def de produção: FROM v_sku_demanda_efetiva→ FROM v_sku_demanda_efetiva.
-- security_invoker=true OBRIGATÓRIO em cada CREATE OR REPLACE. Versão CANÔNICA destas 4 views
-- (aplicar DEPOIS de db/reposicao-consolidacao-demanda.sql). NÃO vai em supabase/migrations/.

-- ==== v_sku_candidatos_primeira_compra (prod) ====
...
-- ==== v_sku_demanda_estatisticas (prod) ====
