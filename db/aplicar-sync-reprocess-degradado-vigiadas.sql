-- ============================================================================================
-- APLICAÇÃO via `bun run db:aplicar` — a degradação conta só as chaves VIGIADAS
--
-- Mesmo EFEITO da migration `20260920233000_sync_reprocess_degradado_so_das_vigiadas.sql` (#2526),
-- SEM os envelopes `BEGIN;`/`COMMIT;`: a transação é do `db:aplicar`.
--
-- Paridade derivada, não digitada:
--   diff <(sed '/^BEGIN;$/d;/^COMMIT;$/d' supabase/migrations/20260920233000_sync_reprocess_degradado_so_das_vigiadas.sql) \
--        <(sed '1,/^-- FIM DO CABEÇALHO DESTE ARQUIVO$/d' db/aplicar-sync-reprocess-degradado-vigiadas.sql)
-- tem de sair VAZIO.
-- FIM DO CABEÇALHO DESTE ARQUIVO
-- ============================================================================================
-- sync_reprocess_saude: a contagem de DEGRADAÇÃO passa a olhar só as chaves VIGIADAS
--
-- Medido em prod (2026-09-20, logo após o apply do conserto do retry): o check dizia
-- "estagios no ar, com falha por pedido registrada em 1 estagio(s)" — mas NENHUMA das 7 chaves
-- vigiadas tinha falha. O contador pegava `oben/manual/orders`, um `complete` com error_message de
-- **94 dias atrás**, numa chave DISPENSADA do catálogo (sla_h NULL).
--
-- O status seguia `ok`, então não havia alarme falso; o dano era na MESSAGE — que é o que o founder
-- lê no /health, e exatamente o ruído de fóssil que o catálogo existe para barrar. Uma chave
-- dispensada não empresta degradação ao conjunto, pela mesma razão que não empresta `broken`.
--
-- Recria SÓ o `_data_health_compute` (o conjunto de sources não muda); a postcondição confere que
-- o source segue nas duas pontas.
-- ============================================================================================
...
BEGIN
  ...
  IF v_n <> v_ndist OR v_ndist <> 30 THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: compute devolveu % linhas / % sources (esperado 30/30).', v_n, v_ndist;
  END IF;
  ...
  IF v_status IS NULL OR v_status NOT IN ('ok','stale','broken','unknown') THEN
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: status % fora do vocabulario aceito.', COALESCE(v_status,'<NULL>');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
    ...
    RAISE EXCEPTION 'POSTCONDICAO FALHOU: o source sumiu de uma das duas pontas.';
  END IF;
