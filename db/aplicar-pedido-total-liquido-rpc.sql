...
--
-- Mesmo EFEITO da migration de DR `20260914181500_pedido_total_liquido_acervo.sql`, SEM o envelope
-- `BEGIN;`/`COMMIT;`: a transação é do `db:aplicar`, que executa o corpo via `aplicar_sql()` — e
-- lá comandos de transação são proibidos. A migration com envelope é a que se cola no SQL Editor.
-- Este arquivo é o ESTADO ATUAL; o histórico fica nas migrations e no ledger `db_aplicacoes`. A
-- paridade é PROVADA, não prometida: db/test-pedido-total-liquido-acervo.sh aplica este arquivo num
-- banco à parte e exige o mesmo corpo, a mesma ACL e a mesma tabela.
--
