-- ============================================================
-- APLICAÇÃO via `bun run db:aplicar` — pedido_total_liquido_acervo — a passada que converte o
-- cabeçalho do acervo de bruto para líquido onde as linhas provam o desconto
--
-- Mesmo EFEITO da cadeia de DR, na ordem:
--   1. `20260914181500_pedido_total_liquido_acervo.sql`            (registro, classificador, relatório, conversor)
--   2. `20260914193000_pedido_total_liquido_acervo_mes_entre_contas.sql` (o gate de mês atravessa as contas)
-- SEM os envelopes `BEGIN;`/`COMMIT;`: a transação é do `db:aplicar`, que executa o corpo via
-- `aplicar_sql()` — e lá comandos de transação são proibidos. As migrations com envelope são as
-- que se colam no SQL Editor. Este arquivo é o ESTADO ATUAL; o histórico fica nas migrations e no
-- ledger `db_aplicacoes`. A paridade é PROVADA, não prometida: db/test-pedido-total-liquido-acervo.sh
-- aplica este arquivo num banco à parte e exige o mesmo corpo, a mesma ACL e a mesma tabela que a
-- cadeia instala.
--
-- Isto só CRIA as funções e a tabela de registro. Não converte nada: a conversão é uma chamada
-- explícita a `pedido_total_liquido_converter(p_aplicar => true, ...)`, depois do ensaio.
-- ============================================================
...
-- ════════════════ 2/2 — 20260914193000_pedido_total_liquido_acervo_mes_entre_contas.sql ════════════════
